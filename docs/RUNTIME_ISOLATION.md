...
Lab Tester Portable은 MT5 build 6140 실행 파일과 US100·US30·US500 실틱 캐시를 로컬 복제해 구성한다. Live Portable에는 같은 build, 동결 V7 패키지, 계정·브로커 캐시와 V7 전용 상태가 있다. V6 상태나 포지션은 복사하지 않았다. 2026-08-25 CP1+CP2 승격은 부모 V7을 pre-window flat에서 정상 정지한 뒤 동일 execution/Portfolio/Magic/state 계약으로 entries-disabled recovery·restart와 최종 `0/0 → 1/1`을 통과했다. Exact patch release만 현재 Live owner다.
