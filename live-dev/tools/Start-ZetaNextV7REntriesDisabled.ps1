...
if (-not $ConfirmEntriesDisabled) {
    throw 'Next V7R entries-disabled start requires -ConfirmEntriesDisabled.'
}

$commonModule = Join-Path $PSScriptRoot 'ZetaNextOperatorCommon.psm1'
if (-not (Test-Path -LiteralPath $commonModule -PathType Leaf)) {
    throw "Next V7R operator module is missing: $commonModule"
}
Import-Module $commonModule -Force

$contract = Get-ZetaNextOperatorContract
$authorization = Get-Content -LiteralPath $contract.StatePath -Raw
if ($authorization -notmatch 'Next V7R return entries-disabled preflight:\s+`(?:ENABLED|PASSED)`') {
    throw 'CURRENT_STATE.md does not authorize the Next V7R return entries-disabled runtime.'
}
if ($authorization -notmatch 'Next V7R return new-entry authorization:\s+`DISABLED`') {
    throw 'Entries-disabled mode requires Next V7R return new-entry authorization to remain DISABLED.'
}
...
try {
    ...
    Write-Output "Next V7R entries-disabled runtime started (PID $($process.Id)); waiting for a new 0/0 flat snapshot after sequence $priorSequence."
    ...
    if ($null -eq $status) {
        throw 'Next V7R entries-disabled runtime did not prove exact identity, 0/0 entries, flat exposure, zero margin/risk, and receipt continuity.'
    }
    Write-Output ("Next V7R entries-disabled handshake passed: release={0} portfolio={1} entries=0/0 positions=0 orders=0 margin={2:N2} balance={3:N2} equity={4:N2} sequence={5}." -f
        $status.release_id,
        $status.portfolio_id,
