...
$liveStartPath = Join-Path $PSScriptRoot 'Start-ZetaNextV7Live.ps1'
...
if ($authorization -notmatch 'Next Live-Dev authorization:\s+`ENABLED`' -or
    ...
    throw 'CURRENT_STATE.md does not authorize Next V7 Live new entries.'
}
...
if ($null -eq $terminal) {
    Write-Output 'Next V7 0/0 preflight 후 1/1 Live를 시작합니다...'
    & $liveStartPath -EnableNewEntries -ConfirmLiveDev
    $inventory = Assert-ZetaNextExclusiveTerminalBoundary -Contract $contract -AllowExactLive
    $terminal = @($inventory.ExactLive) | Select-Object -First 1
    if ($null -eq $terminal) {
        throw 'Next V7 starter returned without an exact Live-Dev terminal process.'
    }
} else {
    $liveConfigPath = Join-Path $contract.RuntimeRoot 'Config\terminal-next-v7-live.ini'
    ...
    if ([string]::IsNullOrWhiteSpace([string]$processInfo.CommandLine) -or
        ...
        throw "Next PID $($terminal.Id) is not the exact V7 Live runtime; it was left untouched."
    }
    ...
    if (-not [bool]$status.healthy -or -not (Test-ZetaNextLiveStatus -Status $status) -or
        ...
        throw 'The existing Next V7 terminal failed its identity/Magic/account/1/1 snapshot handshake; it was left running to manage any owned risk.'
    }
    Write-Output "기존 Next V7 Live-Dev 터미널 PID $($terminal.Id)을 확인했습니다. 중복 실행하지 않습니다."
}
...
if ($terminalWindows.Count -gt 0) {
    ...
    Write-Output "Next V7 EA가 부착된 MT5 창을 복원했습니다 (PID $($terminal.Id))."
} else {
    Write-Warning "Next V7 MT5 PID $($terminal.Id)은 실행 중이나 창을 찾지 못했습니다."
}

$dashboardWindowTitle = 'Project Zeta Terminus Next V7 Live-Dev 대시보드'
...
if ($existingDashboard.Count -eq 1 -and $existingDashboard[0].MainWindowHandle -ne 0) {
    ...
    Write-Output '기존 Next V7 대시보드를 복원했습니다.'
} else {
    $null = Start-Process `
        ...
        -WindowStyle Normal `
        -PassThru
    Write-Output 'Next V7 한국어 대시보드를 열었습니다. 로컬 스냅숏을 5초마다 갱신합니다.'
}
