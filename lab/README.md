...
현재 active research family는 없다. Program 7 / meso→macro Unit 107 `dual-portfolio-formula-interaction-spillover-causality-v1`은 두 exact interaction의 E1-E3 개선이 majority-direct였음을 확인했지만, E4와 continuous June-July에서 changed-component 경제 자체가 역전되어 `VALID_NO_TRANSFERABLE_DIRECT_FORMULA_INTERACTION_NO_SEED`로 닫혔다. grid·neighbor·retune·MQL·Tester·MT5는 없었고 fixed candidate도 바뀌지 않았다. 최신 marker는 `FRONTIER_UNIT_107_CLOSED_VALID_NO_TRANSFERABLE_DIRECT_FORMULA_INTERACTION_NO_SEED`; Optimization Goal은 전체 맵 비교로 복귀했다.
