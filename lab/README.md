...
- `research/market-stop-reverse-lot-sizing-implementation-correction-v1/`: 활성 Program 5 / micro→macro Unit 062 구현교정 sibling; 원래 경제 계약 전체를 그대로 상속한 채 격리 소스·EX5·설정·물리 runtime과 causal HCS/TKC·symbols·contract 지문을 동결 완료. 순수 state-slot/event-handle/줄바꿈 교정은 같은 실험 내부이며 valid FIT은 아직 `0/1`, fitted median·candidate SET·selection/latest·경제판정은 미개봉
