...
- `research/us100-realized-variance-asymmetry-response-authoritative-history-v1/`: 열린 Program 1 / meso Unit 096 단일 실험 family; Unit 032의 경제 미판정 계약을 threshold·window·horizon·기간 변경 없이 그대로 고정했다. 자체 source/config/EX5와 neutral Lab에서 만든 lean `rva96-portable`은 build 6140 `0 errors / 0 warnings`, 링크·다른-family 코드·parity fault 0으로 동결됐고 Tester/경제결과는 아직 미개봉이다. 첫 P1 동기화가 환경 지문을 바꾸면 같은 실험 안에서 비경제 attempt를 보존·재동결한 뒤 4경로 전체를 P1부터 재시작한다
