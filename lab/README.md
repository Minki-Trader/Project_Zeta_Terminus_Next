...
현재 active research family는 Program 3 / meso→macro Unit 111 `research/us500-turn-of-month-cash-session-rebalancing-v1/`이다. 2026년 독립 논문이 고정한 마지막 4개·첫 4개 공식 NYSE 거래일에 US500 0.01 lot LONG을 09:30→16:00 ET로 보유하는 단 하나의 cash-session transfer를 선언했고 경제값은 아직 열지 않았다. CFD 휴일 바가 아니라 공식 NYSE 세션으로 순위를 매기고, 소비하지 않는 중간 바 완전성 대신 정확 09:30/15:45 경계 가용성을 쓰도록 두 설계 오류를 경제값 전 교정했다. P1-P5 primary density는 `142/95/95/40/16`이고 PRE4·POST4가 모든 기간에 존재한다. 선언 커밋 뒤 단 한 번의 source-free 집계로 전체 interval bundle을 닫고 Optimization whole map으로 즉시 복귀한다. 최신 marker는 `FRONTIER_UNIT_111_US500_TURN_OF_MONTH_CASH_SESSION_REBALANCING_DECLARED_ECONOMICS_UNOPENED`다.
