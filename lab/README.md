...
현재 active research family는 Program 7 / meso→macro Unit 107 `dual-portfolio-formula-interaction-spillover-causality-v1` 하나다. 닫힌 59차원 Optimization 산식 격자가 후선택 단서로만 남긴 exact point `9FD4A8BEEA47022BDF5C`와 `766FECB1566CBDB2A5A6`를 한 bundle로 고정하고, E4와 continuous June-July에서 changed-component 직접 경제와 unchanged-component 공유계좌 spillover를 분리한다. grid·neighbor·retune·MQL·Tester·MT5는 없고, 모든 fixed gate를 통과할 때만 우선순위상 exact Lab seed 하나를 보존한다. 이 interval은 formula interaction에는 unseen이지만 고정 portfolio weights에는 clean OOS가 아니라는 한계를 명시한다. 최신 marker는 `FRONTIER_UNIT_107_FORMULA_INTERACTION_SPILLOVER_DECLARED_LATER_OUTCOMES_UNOPENED`; Optimization Goal은 이 단일 Lab 판단 뒤 즉시 전체 맵으로 복귀한다.
...
- `research/dual-portfolio-formula-interaction-spillover-causality-v1/`: active source-free Program 7 / meso→macro Unit 107 family; exact formula interaction 두 점만 동결하고 formula-unseen E4/June-July의 direct changed-component uplift와 unchanged-component shared-account spillover를 한 번 판정. at most one exact Lab seed, no grid/neighbor/retune/MQL/Tester/MT5/Live authority
