...
현재 active research family는 없다. 완전한 paired-month anchor lifecycle이 새로 제공한 전역-대-component 성장자본 소유권을 Unit 101 `paired-anchor-component-capital-subsidy-v1` source-free Program 5 / meso→macro 단위로 닫았다. `COMPONENT_LOCAL_NO_ADD`는 관측 volume step의 `82.934705%`를 제거해 raw closed DD를 `20.401029% → 17.815650%`로 낮췄지만 actual/stressed profit을 `+$5,786.63 / +$5,477.524 → +$837.206701 / +$784.255010`으로 붕괴시켰다. 다섯 component와 네 epoch 모두 손익 delta가 음수였으므로 유효 비확증, retained seed·후속 후보·Lab MQL·Live 권한은 없다. 최신 scheduler marker는 `FRONTIER_UNIT_101_CLOSED_VALID_COMPONENT_CAPITAL_SUBSIDY_NONCONFIRMATION_NO_SEED`; Optimization Goal은 전체 맵 재비교로 복귀한다.
