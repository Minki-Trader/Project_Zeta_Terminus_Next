...
현재 active research family는 없다. Program 7 / meso→macro Unit 106 `paired-month-lifecycle-concentration-robustness-v1`은 과거 Unit 075가 불완전 `564/2,235` 장부 때문에 판정하지 못했던 수익 집중도 역할을, 이후 자연스럽게 생긴 고정 후보의 완전 selection `1,428/1,428`·forward `54/54` 장부로 독립 판정했다. selection component top-10 양의 총이익 점유율은 `4/5`가 `30%` 이상이고 중앙값 `51.8291%`; 각 top-10 제거 뒤 `4/5` component와 전체 stressed `-$2,300.692`가 음수였지만 component 순위는 `0/10` 역전·tau `1.0`으로 그대로였다. verdict는 `AMBIGUOUS_SELECTION_COMPONENT_OUTCOME_CONCENTRATION`; forward top-3 제거 `-$8.878`은 소표본 경고뿐이다. 고정 후보·전 lifecycle·가중치·위험은 보존하고 seed·MT5·재배분·Live 권한은 없다. 최신 scheduler marker는 `FRONTIER_UNIT_106_CLOSED_VALID_AMBIGUOUS_LIFECYCLE_CONCENTRATION_NO_POLICY_CHANGE`; Optimization Goal은 전체 맵 재비교로 복귀한다.
