...
현재 active research family는 Program 2 / macro→meso Unit 113 `research/us500-blockbuster-release-next-week-mood-v1/`이다. Hong·Wei (2025)의 exact source rule대로 월요일-일요일 주간에 개봉 당일 4,000개를 **초과한** 미국 영화 편수를 세고 바로 다음 주 US500 정규 cash session을 편수당 0.01 lot LONG한다. P1/P2/P3/P4/latest event-week 밀도는 `25/18/10/4/2`, boundary-eligible day는 `119/84/45/19/10`이며 target OHLC·spread·경제값은 아직 열지 않았다. 선언 commit/push 뒤 단 한 번의 source-free doubled-spread 집계로 닫고 즉시 Optimization 전체 지도로 복귀한다. 고정 후보와 Live에는 권한이 없다. 최신 marker는 `FRONTIER_UNIT_113_HONG_WEI_BLOCKBUSTER_RELEASE_NEXT_WEEK_MOOD_DECLARED_ECONOMICS_UNOPENED`다.
