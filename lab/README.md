...
현재 active research family는 Program 2 / meso→macro Unit 110 `research/us500-epu-volatility-disconnect-cash-session-v1/` 하나다. Federal Reserve FEDS 2021-063의 five-day EPU / realized-volatility disconnect와 mean+0.5-SD 상태를 prior-only 26-week warmup으로 만들고, publisher의 30-day revision window를 넘는 five-week lag 뒤 `HIGH_D_LOW_V` US500 09:30→16:00 ET 0.01-lot LONG을 한 번 판정한다. Predictor-only density는 P1-P5 `112/70/160/43/35` days이고 양 2023 half가 존재한다. target return·P/L·PF·DD·gate·verdict는 unopened이며, declaration commit/push 뒤 one source-free doubled-spread aggregation만 허용된다. 최신 marker는 `FRONTIER_UNIT_110_US500_EPU_VOLATILITY_DISCONNECT_CASH_SESSION_DECLARED_ECONOMICS_UNOPENED`; fixed Optimization candidate와 Live는 unchanged다.
