...
- `research/cross-relative-convergence-sleeve-v1/`: 활성 Program 1 / meso→macro Unit 093 단일 실험 family; 환경 사전실행, 외부심볼 청산호가 회계 결함 pair, 그리고 마지막 latest DUAL_APPEND의 일시적 state-slot open 결함 matrix를 모두 경제 미사용 격리했다. 기존 A/B 이중화의 alternate-slot 쓰기만 추가한 build 6140 `0/0` EX5와 환경 V4를 재동결했고, 유효경로 `0/4`에서 6전략 CONTROL과 고정 US30+US500 DUAL_APPEND의 binding-long·isolated-latest 전체 4경로를 다시 처음부터 함께 판정한다. 순수 구현교정 횟수 제한은 없고 개별 symbol·threshold·clock·hold 구제는 없음
