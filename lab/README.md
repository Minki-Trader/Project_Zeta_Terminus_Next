...
현재 active research family는 없다. Optimization의 완전한 native router 비확증에서 나온 label-versus-account-contribution 단서를 Unit 100 `router-account-contribution-label-alignment-v1` source-free Program 7 / meso→macro 단위로 닫았다. `1,428 / 1,428` anchor close를 router decision에 정확히 결합한 결과 기존 virtual-net rank의 pooled Spearman은 `0.01503`, prior-quarter shared-account stressed-R label로만 바꾼 rank는 `0.00456`이었다. 후자는 상·하위 평균차 `0.02540R`, 하위군 stressed `+$568.183`, 비양수 epoch `2/4`로 다섯 필수 정보·경제 관문을 모두 실패했다. 엔지니어링 결함 없이 유효하게 비확증됐으며 seed·후속 후보·Lab MQL·Live 권한은 없다. 최신 scheduler marker는 `FRONTIER_UNIT_100_CLOSED_VALID_ACCOUNT_CONTRIBUTION_LABEL_NONCONFIRMATION_NO_SEED`; Optimization Goal은 전체 맵 재비교로 복귀한다.
