...
현재 active research family는 없다. Unit 099 `legacy-market-state-router-optimization-eligibility-v1`을 source-free Program 1 / meso→macro 단위로 닫았다. lineage-pinned `zt-market-state-router-economic-v1`의 exact quarterly pooled Ridge identity와 과거 native 경제경로를 확인했고, 현재 anchor candidate ledger의 `12,265`행 중 `7,352`행이 기존 노출 때문에 신호 미평가라 exact source-free counterfactual이 불가능함을 outcome-free로 확정했다. 이는 실패가 아니라 native-path 요구조건이며, tuning 없는 exact router 한 개만 별도 Optimization 정보 seed로 보존한다. 현재 공유계좌 경제성·native DD·Lab MQL·Live 권한은 열지 않았다. 최신 scheduler marker는 `FRONTIER_UNIT_099_CLOSED_RETAIN_EXACT_MARKET_STATE_ROUTER_NATIVE_OPTIMIZATION_SEED`; 구현·환경 교정은 후속 한 Optimization 실험 내부 단계다.
