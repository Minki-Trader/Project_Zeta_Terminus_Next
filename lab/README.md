...
- `research/round-number-crossing-response-v1/`: 열린 Program 1 / micro→meso Unit 094 단일 실험 family; 무거래 observer와 12개 고정 경로는 build 6140 `0/0`. 첫 선택 때 US30·US100 HCS가 차례로 동기화되어 두 미완성 매트릭스를 경제 미열람 폐기했고, 환경 V3에서 전체 P1을 다시 시작한다. 이후 raw는 경로 직후 보존하며 구현·환경 교정은 별도 실험이 아니다
