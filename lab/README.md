...
현재 active research family는 없다. Program 2 / meso→macro Unit 116 `research/initial-release-gscpi-above-normal-portfolio-state-v1/`은 엄격히 직전에 공표된 New York Fed GSCPI 초기값의 자연 0 초과 상태를 `2,233 = 972 ABOVE_NORMAL + 1,252 AT_OR_BELOW_NORMAL + 9 UNAVAILABLE` lifecycle에 정확히 한 번 집계했다. 확인 P1–P3 centered effect는 `+0.008765R/+0.009366 stop`이고, 기간 `0/3`과 양 book `0/2`가 동일 방향으로 일치하지 않아 strong-null no-field로 닫혔다. 비교군이 8건인 P4는 결과 전 설계교정으로 descriptive-only로 격리했다. seed·Optimization candidate·MT5·Live 권한은 없으며 연속 Goal은 serial whole-map discovery로 복귀했다. 최신 marker는 `FRONTIER_UNIT_116_CLOSED_VALID_NO_INITIAL_RELEASE_GSCPI_ABOVE_NORMAL_PORTFOLIO_FIELD_NO_SEED_MAP_RECOMPARED`다.
