...
- `research/cross-relative-convergence-sleeve-v1/`: 활성 Program 1 / meso→macro Unit 093 단일 실험 family; 최초 환경 사전실행과 첫 binding CONTROL/DUAL_APPEND 쌍은 각각 환경 동기화와 입증된 외부심볼 청산호가 회계 결함으로 경제 미사용 격리됐고, 비경제적 교정·build 6140 `0/0` EX5·환경 V3를 재동결했다. 유효경로 `0/4`에서 6전략 CONTROL과 고정 US30+US500 DUAL_APPEND를 binding-long·isolated-latest 전체 4경로로 처음부터 함께 판정하며 개별 symbol·threshold·clock·hold 구제는 없음
