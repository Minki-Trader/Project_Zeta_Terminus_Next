...
현재 active research family는 없다. Unit 097의 정확한 `4%` base quantum reserve는 별도 Optimization selection에서 경제적으로 실패하여 소비됐고 인접 reserve 구제는 닫혔다. 이후 전 프로그램과 lineage를 다시 비교하여 source-free Program 1 / meso→macro Unit 098 `legacy-us500-h4-staged-overlay-optimization-eligibility-v1`을 한 단위로 닫았다. 이 단위는 legacy에서 `STAGED_EXTENSION_CANDIDATE_NOT_FINAL`로 남은 정확한 US500 H4 fixed-`0.01` identity와 현재 anchor의 도달 가능한 `$500` balance stage gate만 보존하며, 공유계좌 경제성·native DD·Live 권한은 열지 않았다. 최신 scheduler marker는 `FRONTIER_UNIT_098_CLOSED_RETAIN_EXACT_LEGACY_US500_H4_STAGED_OVERLAY_OPTIMIZATION_SEED`; feasibility와 구현 단계는 별도 실험으로 세지 않는다.
