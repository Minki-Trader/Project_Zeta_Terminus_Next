...
현재 active research family는 없다. Program 3 / meso→macro Unit 111 `research/us500-turn-of-month-cash-session-rebalancing-v1/`은 `VALID_NO_US500_TURN_OF_MONTH_CASH_SESSION_EFFECT_AFTER_COST_NO_SEED`로 닫혔다. 공식 PRE4+POST4 US500 cash-session 388일의 full gross/observed/stressed는 `-$6.3103/-$8.0013/-$9.6923`, PF `0.8334`, DD `13.6166%`였고 P2-P4는 비용 전부터 모두 음수였다. PRE4는 `-$9.3889`, POST4도 `-$0.3034`; prelatest primary mean `-$0.028303`은 ROM `+$0.024421`과 unconditional `+$0.004284`보다 낮다. Latest만 `+$0.8366`이지만 June이 거의 전부이고 모든 초기·확인·역할·대조군·full gate가 실패했다. No seed, Optimization candidate or MT5 shortlist survives; fixed candidate와 Live는 unchanged이고 Optimization whole map이 재개됐다. 최신 marker는 `FRONTIER_UNIT_111_CLOSED_VALID_NO_US500_TURN_OF_MONTH_CASH_SESSION_EFFECT_AFTER_COST_NO_SEED_MAP_RECOMPARED`다.
