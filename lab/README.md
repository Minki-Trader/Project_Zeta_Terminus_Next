...
현재 active research family는 없다. Program 2 / macro→meso Unit 115 `research/us30-house-recess-cash-long-v1/`은 House Clerk 공식 공지를 이용한 recess-only US30 cash LONG을 `1,017 = 337 RECESS + 680 OPEN`일에 정확히 한 번 집계했다. Full gross/observed/stressed는 `+$41.565/+$36.799/+$32.033`이지만 P1/P3, P1 두 split, `30.226%` DD, `2/5` weekday breadth, control efficiency와 157-cell matched difference가 실패했다. P4가 full의 `86.54%`를 공급했고 latest는 `+$0.0585`로 경제적으로 평평해 valid ambiguous no-seed로 닫혔다. House-open mean이 더 높고 unconditional net/DD도 더 높다. seed·Optimization candidate·MT5·Live 권한은 없으며 연속 Goal은 serial whole-map discovery로 복귀했다. 최신 marker는 `FRONTIER_UNIT_115_CLOSED_VALID_AMBIGUOUS_HOUSE_RECESS_US30_CASH_LONG_NO_SEED_MAP_RECOMPARED`다.
