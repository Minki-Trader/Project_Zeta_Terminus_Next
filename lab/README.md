...
- `research/cross-relative-convergence-sleeve-v1/`: 활성 Program 1 / meso→macro Unit 093 단일 실험 family; 이전 무효 시도들에 이어 세 번째 binding pair의 symbols DB drift와 DUAL_APPEND 일시적 event-segment open 결함도 경제 미사용 격리했다. 이벤트 CSV를 실행 중 유지·행별 flush하는 build 6140 V7 `0/0` EX5와 현재 동기화된 환경 V5를 재동결했고, 유효경로 `0/4`에서 6전략 CONTROL과 고정 US30+US500 DUAL_APPEND의 binding-long·isolated-latest 전체 4경로를 다시 처음부터 함께 판정한다. 구현교정은 실험 내부 감사 사건이며 횟수 제한은 없고 개별 symbol·threshold·clock·hold 구제는 없음
