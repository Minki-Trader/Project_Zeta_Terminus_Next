...
#property description "Project Zeta Terminus Next Lab one-hour adverse exit management"
#property description "Execution version: zt-next-lab-one-hour-adverse-exit-management-v1"
...
#include <ZetaOaem\Position\ZetaOneHourAdverseExit.mqh>
#include <ZetaTerminusNext\Persistence\ZetaStateAndEvents.mqh>

// The EA owns assembly and the inherited event ordering only.

void PrintOaem40ContractFingerprint(const string stage)
  {
   string symbols[2] = {"US30", "US100"};
   for(int index = 0; index < ArraySize(symbols); ++index)
     {
      const string symbol = symbols[index];
      PrintFormat("%s oaem40_contract stage=%s symbol=%s digits=%d point=%.10f tick_size=%.10f contract_size=%.4f trade_mode=%d calc_mode=%d execution_mode=%d filling_mode=%d stops_level=%d freeze_level=%d volume_min=%.4f volume_step=%.4f volume_max=%.4f spread_points=%d swap_mode=%d swap_long=%.8f swap_short=%.8f swap_rollover3=%d",
                  EXECUTION_VERSION,
                  stage,
                  symbol,
...
int OnInit()
  {
   ...
   if(_Symbol != "US30" || _Period != PERIOD_M30 ||
       ...
       InpOneHourAdverseExitKind < 0 || InpOneHourAdverseExitKind > 3)
      return(INIT_PARAMETERS_INCORRECT);
   FolderCreate("ZetaTerminusNext");
   FolderCreate("ZetaTerminusNext\\lab");
   FolderCreate("ZetaTerminusNext\\lab\\oaem40");
   if(tester_mode)
   ...
   if(!InitializeConnectedRuntime())
      PrintFormat("%s waiting for the saved FPMarkets connection",
                  EXECUTION_VERSION);
   PrintOaem40ContractFingerprint("START");
...
void OnDeinit(const int reason)
  {
   PrintOaem40ContractFingerprint("END");
   if(!tester_mode)
      EventKillTimer();
   if(execution_state.runtime_ready)
     {
   ...
   PrintFormat("%s tester_scheduler dispatched_ticks=%I64d "
               ...
               tester_transaction_dispatches,
               tester_retry_dispatches);
   PrintFormat("%s one_hour_adverse_exit kind=%d threshold_r=%.2f "
               "evaluations=%I64d triggers=%I64d successes=%I64d "
               "close_failures=%I64d data_faults=%I64d",
               EXECUTION_VERSION,
               InpOneHourAdverseExitKind,
               OneHourAdverseExitThresholdR(),
   ...
   for(int component = 0; component < COMPONENT_COUNT; ++component)
      PrintFormat("%s one_hour_adverse_exit_component component=%s triggers=%I64d",
                  EXECUTION_VERSION,
                  component_definitions[component].id,
