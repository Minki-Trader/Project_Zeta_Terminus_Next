...
#property description "Project Zeta Terminus Next Lab component removal counterfactual"
#property description "Execution version: zt-next-lab-component-removal-counterfactual-v1"
...
int OnInit()
  {
   ...
   if(InpComponentSetProfile < 0 || InpComponentSetProfile > 2)
   ...
   FolderCreate("ZetaTerminusNext\\lab\\crc81");
   const string profile_name =
      (InpComponentSetProfile == 0 ? "NATIVE" :
       (InpComponentSetProfile == 1 ? "WITHOUT_PASSIVE" :
                                      "WITHOUT_RETURN"));
   PrintFormat("%s component_set profile=%s input=%d enabled=%s",
               EXECUTION_VERSION,
               profile_name,
               InpComponentSetProfile,
               (InpComponentSetProfile == 0
                ? "RC16/RC4/Cross/Pressure/Return/Passive"
                : (InpComponentSetProfile == 1
                   ? "RC16/RC4/Cross/Pressure/Return"
                   : "RC16/RC4/Cross/Pressure/Passive")));
   if(tester_mode)
