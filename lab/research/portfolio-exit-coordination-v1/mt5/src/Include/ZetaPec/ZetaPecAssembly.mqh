#ifndef ZETA_PEC_ASSEMBLY_MQH
#define ZETA_PEC_ASSEMBLY_MQH
...
#include <ZetaPec\Portfolio\ZetaPecExitCoordination.mqh>
...
// Shared assembly for the four predeclared Tester-only PEC paths.
...
int OnInit()
  {
   ...
   FolderCreate("ZetaPec");
   FolderCreate(PEC_FOLDER);
   if(tester_mode)
   ...
   if(tester_mode)
     {
      ...
      const bool pec_exit_dispatch =
         PecTesterContinuousDispatchRequired();
      tester_retry_dispatch = tester_data_retry_active;
      if(!tester_clock_dispatch && !transaction_gate &&
         !tester_retry_dispatch && !pec_exit_dispatch)
         return;
