#!/usr/bin/env python3
"""Run the one frozen Unit 119 ADS-state economic aggregation.
...
FAMILY = REPO / "lab/research/ads-business-conditions-above-normal-portfolio-state-v1"
EVIDENCE = FAMILY / "evidence"
DECLARATION_PATH = (
    EVIDENCE / "ADS_BUSINESS_CONDITIONS_ABOVE_NORMAL_PORTFOLIO_STATE_DECLARATION_V1.json"
)
SCHEDULE_PATH = EVIDENCE / "ADS_REALTIME_RELEASE_STATE_SCHEDULE_V1.csv"
DERIVED_PATH = EVIDENCE / "ADS_BUSINESS_CONDITIONS_ABOVE_NORMAL_DERIVED_ROWS_V1.csv"
RESULT_PATH = EVIDENCE / "ADS_BUSINESS_CONDITIONS_ABOVE_NORMAL_PORTFOLIO_STATE_RESULT_V1.json"
CLOSURE_PATH = EVIDENCE / "ADS_BUSINESS_CONDITIONS_ABOVE_NORMAL_PORTFOLIO_STATE_CLOSURE_V1.json"
...
def verify_pins(declaration: dict[str, Any]) -> dict[str, Any]:
    ...
    pins = declaration["immutable_source_pins"] + declaration["immutable_portfolio_inputs"]
    for pin in pins:
        path = REPO / pin["path"]
        ...
        checked.append(
            {"path": pin["path"], "bytes": actual_bytes, "sha256": actual_sha}
        )
    require(len(checked) == 12, f"unexpected pin count: {len(checked)}")
    ...
    with SCHEDULE_PATH.open("r", encoding="utf-8", newline="") as handle:
        ...
        for row in reader:
            release = Release(
                ...
                value=finite_float(row["initial_value"], row["release_date"]),
                state=row["state"],
                source_column=int(row["source_column"]),
                value_count=int(row["value_count_from_2021_12"]),
            )
            ...
            require(
                ...
                f"noncausal observation date: {release.release_date}",
            )
            require(release.value_count > 0, "empty retained vintage")
            previous = release.release_date
            releases.append(release)
    require(len(releases) == 385, f"unexpected release count: {len(releases)}")
    require(releases[0].release_date == date(2022, 1, 6), "unexpected first release")
    require(releases[-1].release_date == date(2026, 8, 27), "unexpected last release")
    require(
        Counter(item.state for item in releases) == Counter({ABOVE: 208, BELOW: 177}),
        "unexpected release-state counts",
    )
    return releases
...
def reconstruct_lifecycles(
                ...
                if event == birth_event:
                    ...
                    lifecycles.append(
                        Lifecycle(
                            ...
                            ads_value=release.value if release else None,
    ...
    require(
        math.isclose(sum(item.actual_net for item in lifecycles), 444.19, rel_tol=0.0, abs_tol=1e-9),
        "unexpected actual net anchor",
    )
    require(
        math.isclose(sum(item.stressed_net for item in lifecycles), 407.0477, rel_tol=0.0, abs_tol=1e-9),
...
def center_outcomes(rows: list[Lifecycle]) -> list[Lifecycle]:
    ...
    for row in rows:
        require(row.state in STATES, "unexpected unavailable lifecycle")
...
def state_metrics(rows: list[Lifecycle], state: str) -> dict[str, Any]:
    ...
    require(
        all(row.centered_stressed_r is not None and row.centered_stop is not None for row in selected),
...
def contrast(rows: list[Lifecycle]) -> dict[str, Any]:
    ...
    return {
        ...
        BELOW: below,
        "above_minus_below": {
            "raw_mean_stressed_r": above["mean_stressed_r"] - below["mean_stressed_r"],
            "raw_stop_rate": above["stop_rate"] - below["stop_rate"],
            "centered_stressed_r": above["mean_centered_stressed_r"] - below["mean_centered_stressed_r"],
            "centered_stop_rate": above["mean_centered_stop"] - below["mean_centered_stop"],
        },
    }
...
def breakout(rows: list[Lifecycle], field: str, values: Iterable[str]) -> dict[str, dict[str, Any]]:
    output: dict[str, dict[str, Any]] = {}
    for value in values:
        selected = [row for row in rows if getattr(row, field) == value]
        states = Counter(row.state for row in selected)
        require(states[ABOVE] > 0 and states[BELOW] > 0, f"empty breakout state: {field}:{value}")
...
def write_derived(rows: list[Lifecycle]) -> None:
    fields = [
        "record_id", "period", "component", "book", "birth_server", "final_server",
        "state", "release_date", "observation_date", "ads_value", "planned_risk_usd",
        "actual_net_usd", "stressed_net_usd", "stressed_r", "stop",
        "centered_stressed_r", "centered_stop", "source_path", "source_birth_row",
        "source_final_row",
    ]
            ...
            writer.writerow(
                {
                    ...
                    "observation_date": row.observation_date.isoformat() if row.observation_date else "",
                    "ads_value": "" if row.ads_value is None else row.ads_value,
...
def main() -> None:
    ...
    require(
        declaration_sha == "106431AEF3A3B7EECB0280EF15813385F7B606074C65C1F473EA6C05726263DE",
        "declaration hash mismatch",
    )
    pins = verify_pins(declaration)
    releases = load_releases()
    lifecycles, event_counts, event_rows = reconstruct_lifecycles(declaration, releases)
    lifecycles = center_outcomes(lifecycles)

    state_counts = Counter(row.state for row in lifecycles)
    require(
        state_counts == Counter({BELOW: 1133, ABOVE: 1100}),
        f"unexpected state counts: {state_counts}",
    )
    expected_period_states = {
        "P1_2022H2_2023": Counter({ABOVE: 508, BELOW: 261}),
        "P2_2024": Counter({BELOW: 320, ABOVE: 234}),
        "P3_2025": Counter({BELOW: 322, ABOVE: 232}),
        "P4_2026_YTD": Counter({BELOW: 230, ABOVE: 126}),
    }
    ...
    for period in PERIODS:
        for book in BOOKS:
            for state in STATES:
                ...
                require(
                    cell_count >= 40,
                    f"thin period-book-state cell: {period}:{book}:{state}:{cell_count}",
                )
    ...
    primary = contrast(lifecycles)
    books = breakout(lifecycles, "book", BOOKS)
    periods = breakout(lifecycles, "period", PERIODS)
    components = breakout(lifecycles, "component", COMPONENTS)
    ...
    favorable = {
        ...
        "at_least_three_of_four_periods": period_directions["FAVORABLE_ABOVE_NORMAL"] >= 3,
        "at_least_four_components": component_directions["FAVORABLE_ABOVE_NORMAL"] >= 4,
    ...
    adverse = {
        ...
        "at_least_three_of_four_periods": period_directions["ADVERSE_ABOVE_NORMAL"] >= 3,
        "at_least_four_components": component_directions["ADVERSE_ABOVE_NORMAL"] >= 4,
        "period_contribution_cap_0_60": period_contribution["maximum_absolute_share"] <= 0.60,
        "component_contribution_cap_0_45": component_contribution["maximum_absolute_share"] <= 0.45,
    }
    ...
    strong_null = {
        ...
        "favorable_period_breadth_at_most_2": period_directions["FAVORABLE_ABOVE_NORMAL"] <= 2,
        "adverse_period_breadth_at_most_2": period_directions["ADVERSE_ABOVE_NORMAL"] <= 2,
        "favorable_component_breadth_at_most_3": component_directions["FAVORABLE_ABOVE_NORMAL"] <= 3,
        "adverse_component_breadth_at_most_3": component_directions["ADVERSE_ABOVE_NORMAL"] <= 3,
    }
    ...
    result = {
        "schema": "zeta-next-ads-business-conditions-above-normal-portfolio-state-result-v1",
        ...
        "integrity": {
            ...
            "mapped_lifecycles": len(lifecycles),
            ...
            "realtime_release_rows": len(releases),
            "same_release_date_excluded": True,
        },
        "state_counts": {ABOVE: state_counts[ABOVE], BELOW: state_counts[BELOW], UNAVAILABLE: 0},
        "confirmatory_periods": list(PERIODS),
        "primary": primary,
        "breakouts": {"books": books, "periods": periods, "components": components},
        "direction_breadth": {
            "books": dict(book_directions),
            "periods": dict(period_directions),
            "components": dict(component_directions),
        },
        "contribution_caps": {"periods": period_contribution, "components": component_contribution},
        "gates": {"favorable": favorable, "adverse": adverse, "strong_null": strong_null},
        "selected_candidate": None,
        "retained_lab_question": (
            "one_nonautomatic_whole_portfolio_ads_state_question"
    ...
    closure = {
        "schema": "zeta-next-ads-business-conditions-above-normal-portfolio-state-closure-v1",
        "created_at_local": "2026-08-30",
        "status": "CLOSED_VALID",
        "unit": declaration["unit"],
        "macro_program": declaration["macro_program"],
        "verdict": verdict,
        "primary_effect": primary["above_minus_below"],
        "state_counts": result["state_counts"],
        "direction_breadth": result["direction_breadth"],
        "contribution_caps": result["contribution_caps"],
        "gates": result["gates"],
        "declaration_sha256": declaration_sha,
        ...
        "next": "update_state_close_unit_119_recompare_whole_map_and_return_to_optimization_boundary",
    }
