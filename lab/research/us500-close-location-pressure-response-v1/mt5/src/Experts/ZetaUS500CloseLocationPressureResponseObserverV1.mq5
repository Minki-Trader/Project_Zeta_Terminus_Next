...
#property description "Trade-free US500 completed-bar close-location pressure response observer"

input int  InpRunCode = 1;
input long InpLastTriggerEpoch = 1767110400;

const string OBSERVER_ID = "ZETA-NEXT-US500-CLOSE-LOCATION-PRESSURE-RESPONSE-V1";
const string OUTPUT_ROOT = "US500CLP36V1";
const int CONTINUOUS_SECONDS = 900;
const int REQUIRED_RATES = 3;
const int HORIZON_MARKET_BARS = 4;
const double CLOSE_LOCATION_THRESHOLD = 0.75;
const double BODY_FRACTION_THRESHOLD = 0.50;
...
struct ClosePressureObservation
  {
   bool              active;
   long              opportunity_id;
...
ClosePressureObservation g_observation;
...
void WriteOpportunity(const MqlTick &tick)
  {
   ...
   const uint written=FileWrite(g_opportunity_handle,
                                OBSERVER_ID,
                                InpRunCode,
                                g_observation.opportunity_id,
                                TimeToString(g_observation.completed_bar_time,TIME_DATE|TIME_SECONDS),
                                TimeToString(g_observation.entry_time,TIME_DATE|TIME_SECONDS),
                                TimeToString((datetime)tick.time,TIME_DATE|TIME_SECONDS),
                                DoubleToString(g_observation.bar_open,_Digits),
                                DoubleToString(g_observation.bar_high,_Digits),
                                DoubleToString(g_observation.bar_low,_Digits),
                                DoubleToString(g_observation.bar_close,_Digits),
                                DoubleToString(g_observation.close_location,9),
                                DoubleToString(g_observation.body_fraction,9),
...
void EvaluateCandidate(const MqlTick &tick)
  {
   ...
   ENUM_ORDER_TYPE continuation_direction=ORDER_TYPE_BUY;
   ...
   if((long)(rates[0].time-rates[1].time)!=CONTINUOUS_SECONDS ||
      (long)(rates[1].time-rates[2].time)!=CONTINUOUS_SECONDS)
      return;

   const double bar_range=rates[1].high-rates[1].low;
   if(!MathIsValidNumber(rates[1].open) ||
      ...
      rates[1].open>rates[1].high ||
      rates[1].close<rates[1].low ||
      rates[1].close>rates[1].high)
     {
      g_rate_faults++;
      return;
     }
   ...
   close_location=(2.0*rates[1].close-rates[1].high-rates[1].low)/bar_range;
   body_fraction=(rates[1].close-rates[1].open)/bar_range;
   if(!MathIsValidNumber(close_location) || !MathIsValidNumber(body_fraction))
     {
      g_rate_faults++;
      return;
     }

   if(MathAbs(close_location)<CLOSE_LOCATION_THRESHOLD ||
      MathAbs(body_fraction)<BODY_FRACTION_THRESHOLD)
      return;

   if(close_location>0.0 && body_fraction>0.0)
      continuation_direction=ORDER_TYPE_BUY;
   else if(close_location<0.0 && body_fraction<0.0)
      continuation_direction=ORDER_TYPE_SELL;
   else
      return;
   ...
   g_observation.completed_bar_time=rates[1].time;
   g_observation.entry_time=(datetime)tick.time;
   g_observation.bar_open=rates[1].open;
   g_observation.bar_high=rates[1].high;
   g_observation.bar_low=rates[1].low;
   g_observation.bar_close=rates[1].close;
   g_observation.close_location=close_location;
   g_observation.body_fraction=body_fraction;
   g_observation.continuation_direction=continuation_direction;
...
void ProcessNewBar(const MqlTick &tick,const datetime current_bar_time)
  {
   ...
   if(!g_observation.active && current_bar_time<=g_last_trigger_time)
...
void WriteSummary(const int reason)
  {
   ...
   FileWrite(summary_handle,
             OBSERVER_ID,
             InpRunCode,
             TimeToString(g_last_trigger_time,TIME_DATE|TIME_SECONDS),
             g_eligible_days,
             g_eligible_evaluations,
             g_finalized_bars,
             g_valid_ticks,
   ...
   PrintFormat("%s STOP run=%d reason=%d cutoff=%s eligible_days=%I64d eligible_evaluations=%I64d finalized_bars=%I64d valid_ticks=%I64d triggers=%I64d resolved=%I64d unresolved=%d rate_faults=%I64d tick_faults=%I64d calc_faults=%I64d row_faults=%I64d",
               OBSERVER_ID,
               InpRunCode,
               reason,
               TimeToString(g_last_trigger_time,TIME_DATE|TIME_SECONDS),
...
int OnInit()
  {
   ...
   g_last_trigger_time=(datetime)InpLastTriggerEpoch;
   ...
   if(!MQLInfoInteger(MQL_TESTER) ||
      ...
      InpRunCode<1 ||
      InpRunCode>3 ||
      InpLastTriggerEpoch<=0)
     {
      PrintFormat("%s INIT_CONTRACT_FAILED tester=%d symbol=%s period=%d run=%d cutoff=%I64d",
                  OBSERVER_ID,
   ...
   g_initialized=true;
   PrintFormat("%s START run=%d symbol=%s close_location=%.2f body_fraction=%.2f horizon=%d cutoff=%s",
               OBSERVER_ID,
               InpRunCode,
               _Symbol,
               CLOSE_LOCATION_THRESHOLD,
               BODY_FRACTION_THRESHOLD,
               HORIZON_MARKET_BARS,
               TimeToString(g_last_trigger_time,TIME_DATE|TIME_SECONDS));
...
void OnTick()
  {
   ...
   ProcessNewBar(tick,current_bar_time);
  }
