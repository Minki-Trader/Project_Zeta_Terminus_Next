...
#property description "Project Zeta Terminus Next Lab cross-relative convergence sleeve"
#property description "Execution version: zt-next-lab-cross-relative-convergence-sleeve-v1"
...
#include <ZetaTerminusNext\Strategies\ZetaCrossConvergence.mqh>
...
string ResearchProfileName()
  {
   return(InpResearchProfile == CRCS93_DUAL_APPEND
          ? "DUAL_APPEND" : "CONTROL");
  }


bool ResearchComponentEnabled(const int component)
  {
   if(component < US30_CONVERGENCE)
      return(true);
   return(InpResearchProfile == CRCS93_DUAL_APPEND);
  }
...
void PrintRequiredSymbolContract(const string stage,
                                 const string symbol)
  {
   PrintFormat("%s CRCS93_CONTRACT stage=%s symbol=%s "
...
int OnInit()
  {
   ...
   if(_Symbol != "US30" || _Period != PERIOD_M30 ||
       ...
       (!tester_mode && InpAllowNewEntries &&
        InpExpectedLiveAccountLogin <= 0) ||
       InpEventCapacity < 256 || InpEventCapacity > 65536 ||
      InpSnapshotSeconds < 10 || InpSnapshotSeconds > 600 ||
      (InpResearchProfile != CRCS93_CONTROL &&
       InpResearchProfile != CRCS93_DUAL_APPEND))
      return(INIT_PARAMETERS_INCORRECT);
   FolderCreate("ZetaTerminusNext");
   FolderCreate("ZetaTerminusNext\\lab");
   FolderCreate("ZetaTerminusNext\\lab\\crcs93");
   if(tester_mode)
      ResetTesterArtifacts();
   PrintFormat("%s research_profile=%s input=%d "
               "enabled=%s",
               EXECUTION_VERSION,
               ResearchProfileName(),
               (int)InpResearchProfile,
               (InpResearchProfile == CRCS93_DUAL_APPEND
                ? "RC16/RC4/Cross/Pressure/Return/Passive/US30_CONVERGENCE/US500_CONVERGENCE"
                : "RC16/RC4/Cross/Pressure/Return/Passive"));
   for(int component = 0; component < COMPONENT_COUNT; ++component)
      PrintFormat("%s component_identity ordinal=%d id=%s symbol=%s "
                  "timeframe=%s hold=%d magic=%I64u enabled=%s",
                  EXECUTION_VERSION,
                  component,
                  component_definitions[component].id,
                  component_definitions[component].symbol,
                  EnumToString(component_definitions[component].timeframe),
                  component_definitions[component].hold_bars,
                  component_definitions[component].magic,
                  (ResearchComponentEnabled(component) ? "true" : "false"));
                             ...
                             const datetime us100_m15_bar)
  {
   ...
   const bool fixed_data_retry =
      ((component_states[RC4_BOTH].entry_check_result == "DATA_UNAVAILABLE" &&
       ...
       (InpResearchProfile == CRCS93_DUAL_APPEND &&
        component_states[US30_CONVERGENCE].entry_check_result == "DATA_UNAVAILABLE" &&
        server.hour == 17 && server.min >= 0 && server.min <= 2) ||
       (InpResearchProfile == CRCS93_DUAL_APPEND &&
        component_states[US500_CONVERGENCE].entry_check_result == "DATA_UNAVAILABLE" &&
        server.hour == 17 && server.min >= 0 && server.min <= 2));
   ...
   if(NewEntriesOperationallyAllowed())
     {
      ...
      ProcessUS100Cross();
      if(InpResearchProfile == CRCS93_DUAL_APPEND)
        {
         ProcessUS30Convergence();
         ProcessUS500Convergence();
        }
     }
