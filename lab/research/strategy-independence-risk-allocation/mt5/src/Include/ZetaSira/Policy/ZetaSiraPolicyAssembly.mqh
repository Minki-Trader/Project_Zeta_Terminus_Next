#ifndef ZETA_SIRA_POLICY_ASSEMBLY_MQH
#define ZETA_SIRA_POLICY_ASSEMBLY_MQH

#include <Trade\Trade.mqh>

#include <ZetaSira\Policy\Domain\ZetaSiraPolicyDomain.mqh>
...
#include <ZetaSira\Policy\ZetaSiraAllocationPolicy.mqh>
#include <ZetaSira\Policy\Portfolio\ZetaSiraPolicyPortfolioRisk.mqh>
...
// Lab-only assembly. A thin wrapper selects either all six frozen strategies
// or exactly one strategy while preserving that strategy's causal behavior.

bool SiraComponentEnabled(const int component)
  {
   return(SIRA_SELECTED_COMPONENT < 0 ||
          SIRA_SELECTED_COMPONENT == component);
  }
...
int OnInit()
  {
   ...
   sira_policy_reservation_skips = 0;
   if(!tester_mode)
     {
   ...
   FolderCreate("ZetaSira");
   FolderCreate("ZetaSira\\sira");
   if(tester_mode)
...
void OnTick()
  {
   ...
   if(tester_mode)
     {
      ...
      if(SiraComponentEnabled(RC4_BOTH) &&
         execution_state.rc4_shadow_occupied)
   ...
   ProcessClosures();
   if(SiraComponentEnabled(US100_PASSIVE_LIMIT))
      ProcessPassiveLimit();
   if(SiraComponentEnabled(RC4_BOTH) && !tester_mode)
      ProcessRC4ShadowOccupancy();
   if(NewEntriesOperationallyAllowed())
     {
      if(SiraComponentEnabled(RC4_BOTH))
         ProcessRC4Both();
      if(SiraComponentEnabled(RC16_LONG))
         ProcessRC16Long();
      if(SiraComponentEnabled(US30_PRESSURE))
         ProcessUS30Pressure();
      if(SiraComponentEnabled(US30_RETURN_REV_LONG))
         ProcessUS30ReturnReversalLong();
      if(SiraComponentEnabled(US100_CROSS))
         ProcessUS100Cross();
     }
   if(SiraComponentEnabled(RC4_BOTH))
...
void OnDeinit(const int reason)
  {
   ...
   PrintFormat("%s sira_policy=%s reservation_skips=%I64d",
               EXECUTION_VERSION,
               SiraPolicyName(),
