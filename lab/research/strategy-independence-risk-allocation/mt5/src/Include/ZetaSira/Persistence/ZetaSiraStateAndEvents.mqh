#ifndef ZETA_SIRA_STATE_AND_EVENTS_MQH
#define ZETA_SIRA_STATE_AND_EVENTS_MQH

// Lab-only observation extension over the frozen V7 persistence behavior.

int SiraActiveComponentMask()
  {
   int mask = 0;
...
void ClearDecisionJournalState()
  {
   ...
   sira_current_opportunity_id = 0;
   sira_pre_active_mask = 0;
   sira_pre_aggregate_planned_risk = 0.0;
   sira_pre_equity = 0.0;
   sira_pre_margin = 0.0;
   sira_pre_risk_capital = 0.0;
   sira_pre_rc4_shadow_occupied = false;
  }
   ...
   if(tester_mode)
     {
      ...
      StringReplace(safe_detail, "|", "/");
      PrintFormat("SIRA_EVENT|server=%I64d|opportunity=%I64d|component=%d|name=%s|value_a=%.10f|value_b=%.10f|detail=%s|stressed_balance=%.10f|stage_balance=%.10f|equity=%.10f|margin=%.10f|sequence=%I64d",
                  (long)TimeCurrent(),
                  sira_current_opportunity_id,
                  component,
                  event_name,
                  value_a,
                  value_b,
...
bool PersistDecisionUntil(const int component,
                          ...
                          const datetime deadline)
  {
   ...
   decision_intent.stop_loss = 0.0;
   decision_intent.planned_risk_usd = 0.0;
   sira_current_opportunity_id = ++sira_opportunity_records;
   sira_pre_active_mask = SiraActiveComponentMask();
   sira_pre_aggregate_planned_risk = TrackedAggregatePlannedRisk();
   sira_pre_equity = AccountInfoDouble(ACCOUNT_EQUITY);
   sira_pre_margin = AccountInfoDouble(ACCOUNT_MARGIN);
   sira_pre_risk_capital = ConservativeRiskCapital();
   sira_pre_rc4_shadow_occupied = execution_state.rc4_shadow_occupied;
   ++journal_signal_decisions;
...
bool FinalizeDecisionJournal(const int component,
                             const string outcome)
  {
   ...
   string safe_outcome = outcome;
   StringReplace(safe_outcome, "|", "/");
   ...
   if(tester_mode)
      PrintFormat("SIRA_OPPORTUNITY|id=%I64d|server=%I64d|bar=%I64d|deadline=%I64d|selected=%d|component=%d|direction=%d|signal=%.10f|outcome=%s|stage=%s|ticket=%I64u|active_mask=%d|aggregate_risk=%.10f|risk_capital=%.10f|equity=%.10f|margin=%.10f|rc4_shadow=%d|order_price=%.10f|volume=%.10f|stop_loss=%.10f|planned_risk=%.10f",
                  sira_current_opportunity_id,
                  (long)TimeCurrent(),
                  (long)decision_intent.decision_bar,
                  (long)decision_intent.deadline,
                  SIRA_SELECTED_COMPONENT,
                  component,
                  decision_intent.direction,
                  decision_intent.signal_value,
                  safe_outcome,
                  stage_name,
                  decision_intent.adopted_ticket,
                  sira_pre_active_mask,
                  sira_pre_aggregate_planned_risk,
                  sira_pre_risk_capital,
                  sira_pre_equity,
                  sira_pre_margin,
                  (int)sira_pre_rc4_shadow_occupied,
                  component_states[component].entry_check_order_price,
                  component_states[component].entry_check_volume,
                  component_states[component].entry_check_stop_loss,
                  component_states[component].entry_check_planned_risk_usd);
