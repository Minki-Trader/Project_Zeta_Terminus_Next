#ifndef ZETA_PROFIT_MEMORY_ASSEMBLY_MQH
#define ZETA_PROFIT_MEMORY_ASSEMBLY_MQH
...
#include <ZetaProfitMemory\ZetaProfitMemoryObserver.mqh>
#endif

// The EA owns assembly and the inherited event ordering only.

bool PrintProfitMemoryContractEvidence(const string phase)
  {
   const string symbols[3] = {"US30", "US100", "US500"};
   for(int index = 0; index < 3; ++index)
     {
      const string symbol = symbols[index];
      if(!SymbolSelect(symbol, true))
         return(false);
      PrintFormat("%s PMO_CONTRACT phase=%s symbol=%s digits=%d point=%.10f "
                  "contract=%.10f tick_size=%.10f tick_value_profit=%.10f "
                  "tick_value_loss=%.10f volume_min=%.10f volume_max=%.10f "
                  "volume_step=%.10f calc_mode=%d swap_mode=%d "
                  "swap_long=%.10f swap_short=%.10f rollover3day=%d "
                  "currency_profit=%s currency_margin=%s",
                  EXECUTION_VERSION,
                  phase,
                  symbol,
...
int OnInit()
  {
   ...
   if(!PrintProfitMemoryContractEvidence("start"))
      return(INIT_FAILED);
   ...
   FolderCreate("ZetaTerminusNext\\lab");
    FolderCreate("ZetaTerminusNext\\lab\\profit-memory-observation-v1");
#ifdef ZETA_PMO_OBSERVER_VARIANT
    FolderCreate("ZetaTerminusNext\\lab\\profit-memory-observation-v1\\observer");
#else
    FolderCreate("ZetaTerminusNext\\lab\\profit-memory-observation-v1\\control");
#endif
    if(tester_mode)
       ResetTesterArtifacts();
#ifdef ZETA_PMO_OBSERVER_VARIANT
    if(!ResetProfitMemoryObserver())
       return(INIT_FAILED);
#endif
...
#ifdef ZETA_PMO_OBSERVER_VARIANT
    FinalizeProfitMemoryObserver();
#endif
    PrintProfitMemoryContractEvidence("end");
