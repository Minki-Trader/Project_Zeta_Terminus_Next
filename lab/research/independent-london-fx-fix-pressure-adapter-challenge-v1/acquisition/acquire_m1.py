...
def symbol_spec(symbol: str, fields: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "schema": "zeta-next-london-fx-fix-pressure-m1-symbol-spec-v1",
        "symbol": symbol,
...
def acquire_symbol(
    ...
    while cursor < end_exclusive:
        ...
        while True:
            ...
            if rates is None:
                ...
                if missing:
                    raise RuntimeError(f"missing {symbol} M1 columns: {missing}")
                frame = frame[
            ...
            if now >= next_progress:
                print(
                    f"waiting for {symbol} M1 {cursor:%Y-%m}: "
                    f"{last_observation}",
                    flush=True,
                )
        ...
        print(
            f"acquired {symbol} M1 {cursor:%Y-%m}: "
            f"{len(frame)} rows",
            flush=True,
        )
        cursor = batch_end
    ...
    if data["time"].duplicated().any() or not data["time"].is_monotonic_increasing:
        raise RuntimeError(f"duplicate or nonincreasing {symbol} M1 timestamps")
    if int(data["time"].iloc[0]) - start_epoch > 4 * 24 * 60 * 60:
        raise RuntimeError(f"first {symbol} M1 row is too far after request")
    if end_epoch - int(data["time"].iloc[-1]) > 4 * 24 * 60 * 60:
        raise RuntimeError(f"last {symbol} M1 row is too far before request end")
    for name in ("open", "high", "low", "close"):
        values = pd.to_numeric(data[name], errors="coerce")
        if not values.notna().all() or not values.map(math.isfinite).all():
            raise RuntimeError(f"nonfinite {symbol} M1 column: {name}")
        if not (values > 0).all():
            raise RuntimeError(f"nonpositive {symbol} M1 column: {name}")
    if not (data["high"] >= data[["open", "close", "low"]].max(axis=1)).all():
        raise RuntimeError(f"{symbol} M1 high invariant failed")
    if not (data["low"] <= data[["open", "close", "high"]].min(axis=1)).all():
        raise RuntimeError(f"{symbol} M1 low invariant failed")
    if (data["spread"] < 0).any() or (data["tick_volume"] < 0).any():
        raise RuntimeError(f"negative {symbol} M1 spread or volume")
...
def main() -> int:
    ...
    development_paths = {
        symbol: market_root / f"{symbol}_M1_DEVELOPMENT.csv" for symbol in SYMBOLS
    }
    locked_paths = {
        symbol: market_root / f"{symbol}_M1_LOCKED.csv" for symbol in SYMBOLS
    }
    combined_paths = {symbol: market_root / f"{symbol}_M1.csv" for symbol in SYMBOLS}
    ...
    staging = Path(tempfile.mkdtemp(prefix="zeta-f010-fix-m1-"))
    initialized = False
    try:
        ...
        if args.stage == "development":
            ...
            receipt = {
                "schema": "zeta-next-london-fx-fix-pressure-m1-development-acquisition-receipt-v1",
                ...
                "fix_features_directions_lifecycles_or_economics": 0,
            }
            ...
            receipt = {
                "schema": "zeta-next-london-fx-fix-pressure-m1-locked-acquisition-receipt-v1",
                "recorded_at_utc": datetime.now(tz=UTC).isoformat().replace(
                    "+00:00", "Z"
                ),
                ...
                "fix_features_directions_lifecycles_or_economics_in_acquisition": 0,
            }
            ...
            complete = {
                "schema": "zeta-next-london-fx-fix-pressure-m1-complete-acquisition-receipt-v1",
                "recorded_at_utc": datetime.now(tz=UTC).isoformat().replace(
                    "+00:00", "Z"
                ),
