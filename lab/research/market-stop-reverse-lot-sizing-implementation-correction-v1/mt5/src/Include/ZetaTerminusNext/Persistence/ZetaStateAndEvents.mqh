...
void ResetRuntimeState()
  {
   ...
   market_stop_native_atr_observations = 0;
   market_stop_atr_unavailable = 0;
   market_stop_sizing_attempts = 0;
   market_stop_distance_blocks = 0;
   market_stop_min_lot_blocks = 0;
   market_stop_accepted = 0;
   market_stop_non_native_volume = 0;
  }
...
bool RecordEvent(const int component,
                 ...
                 const string detail)
  {
   ...
   if(rotate || event_file_handle == INVALID_HANDLE ||
      event_file_segment != target_segment)
     {
      CloseEventFile();
      const int flags =
         (replace_segment
      ...
      event_file_handle = OpenFileWithRetry(path, flags, ',');
      if(event_file_handle == INVALID_HANDLE)
        {
      ...
      event_file_segment = target_segment;
      if(!replace_segment)
         FileSeek(event_file_handle, 0, SEEK_END);
      if(FileSize(event_file_handle) == 0)
        {
         const uint header_written =
            FileWrite(event_file_handle,
   ...
   const uint event_written =
      FileWrite(event_file_handle,
                ...
                AccountInfoDouble(ACCOUNT_MARGIN),
                state_sequence);
   FileFlush(event_file_handle);
   if(event_written == 0 || FileSize(event_file_handle) <= 0)
     {
      CloseEventFile();
...
void CloseEventFile()
  {
   if(event_file_handle == INVALID_HANDLE)
     {
      event_file_segment = -1;
      return;
     }
   FileFlush(event_file_handle);
   FileClose(event_file_handle);
   event_file_handle = INVALID_HANDLE;
   event_file_segment = -1;
  }
