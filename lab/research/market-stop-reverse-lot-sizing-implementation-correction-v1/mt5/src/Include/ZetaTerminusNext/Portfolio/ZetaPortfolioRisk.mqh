...
bool MarketStopTreatedComponent(const int component)
  {
   return(component == RC16_LONG ||
          component == RC4_BOTH ||
          component == US100_CROSS ||
          component == US30_PRESSURE ||
          component == US30_RETURN_REV_LONG);
  }


bool MarketStopCandidateActive()
  {
   return(InpMarketStopProfile == 1 &&
          TimeCurrent() >= InpMarketStopActivation);
  }


double MarketStopMultiplier(const int component)
  {
   if(component == RC16_LONG)
      return(InpMarketStopMultiplierRC16);
   if(component == RC4_BOTH)
      return(InpMarketStopMultiplierRC4);
   if(component == US100_CROSS)
      return(InpMarketStopMultiplierCross);
   if(component == US30_PRESSURE)
      return(InpMarketStopMultiplierPressure);
   if(component == US30_RETURN_REV_LONG)
      return(InpMarketStopMultiplierReturn);
   return(0.0);
  }


bool CompletedMarketStopAtr(const int component, double &atr)
  {
   atr = 0.0;
   if(!MarketStopTreatedComponent(component) ||
      InpMarketStopAtrPeriod <= 0)
      return(false);
   MqlRates rates[];
   ArraySetAsSeries(rates, true);
   const int required = InpMarketStopAtrPeriod + 1;
   const int copied =
      CopyRates(component_definitions[component].symbol,
   ...
   if(copied != required)
      return(false);
   double total = 0.0;
   for(int index = 0; index < InpMarketStopAtrPeriod; ++index)
     {
      const double high = rates[index].high;
      const double low = rates[index].low;
   ...
   atr = total / (double)InpMarketStopAtrPeriod;
   return(MathIsValidNumber(atr) && atr > 0.0);
  }


void RecordNativeStopAtr(const int component,
                         ...
                         const double volume)
  {
   if(!MarketStopTreatedComponent(component))
      return;
   double atr = 0.0;
   if(!CompletedMarketStopAtr(component, atr))
     {
      ++market_stop_atr_unavailable;
      RecordEvent(component,
                  "NATIVE_STOP_ATR_UNAVAILABLE",
                  entry_price,
                  stop_loss,
                  StringFormat("period=%d volume=%.8f",
                               InpMarketStopAtrPeriod,
                               volume));
      return;
     }
   const double distance = MathAbs(entry_price - stop_loss);
   const double ratio = distance / atr;
   if(!MathIsValidNumber(ratio) || ratio <= 0.0)
     {
      ++market_stop_atr_unavailable;
      RecordEvent(component,
                  "NATIVE_STOP_ATR_UNAVAILABLE",
                  distance,
                  atr,
                  "non-positive native stop/ATR ratio");
      return;
     }
   ++market_stop_native_atr_observations;
   RecordEvent(component,
               "NATIVE_STOP_ATR",
               ratio,
               atr,
               StringFormat("entry=%.8f stop=%.8f distance=%.8f volume=%.8f",
                            entry_price,
                            stop_loss,
                            distance,
                            volume));
...
bool CalculateMarketStopReverseLot(const int component,
                                   const string symbol,
                                   const int direction,
                                   const double entry_price,
                                   const double minimum_distance,
                                   ...
                                   double &planned_risk)
  {
   ...
   if(!CompletedMarketStopAtr(component, atr))
     {
      ...
      RecordEvent(component,
                  ...
                  entry_price,
                  minimum_distance,
                  StringFormat("period=%d", InpMarketStopAtrPeriod));
      return(false);
     }

   const double multiplier = MarketStopMultiplier(component);
   const double capital = ConservativeRiskCapital();
   ...
   if(!MathIsValidNumber(multiplier) || multiplier <= 0.0 ||
      capital <= 0.0 || tick_size <= 0.0 || entry_price <= 0.0 ||
      minimum_distance <= 0.0 || target_gross_risk <= 0.0)
     {
      ++protection_calculation_failures;
      RecordEvent(component,
                  "MARKET_STOP_CALC_FAIL",
                  capital,
                  target_gross_risk,
                  "invalid market-stop input");
      return(false);
     }

   const double raw_stop =
      entry_price - (double)direction * multiplier * atr;
   const double units = raw_stop / tick_size;
   stop_loss =
      NormalizeDouble((direction > 0
                       ? MathFloor(units + 1.0e-9) * tick_size
   ...
   const double actual_distance =
      (direction > 0 ? entry_price - stop_loss : stop_loss - entry_price);
   if(stop_loss <= 0.0 || actual_distance <= 0.0 ||
      actual_distance + 0.25 * tick_size < minimum_distance)
     {
      ++market_stop_distance_blocks;
      ++risk_admission_skips;
      RecordEvent(component,
                  "MARKET_STOP_DISTANCE_BLOCK",
                  actual_distance,
                  minimum_distance,
                  StringFormat("atr=%.8f multiplier=%.10f raw_stop=%.8f",
                               atr,
                               multiplier,
                               raw_stop));
      stop_loss = 0.0;
   ...
   if(minimum_volume <= 0.0 || maximum_volume < minimum_volume ||
      volume_step <= 0.0)
     {
      ++protection_calculation_failures;
      RecordEvent(component,
   ...
   if(!GrossStopRisk(symbol,
                     ...
                     minimum_gross_risk))
     {
      ...
      RecordEvent(component,
                  ...
                  minimum_volume,
                  stop_loss,
                  "minimum-volume stop risk unavailable");
      stop_loss = 0.0;
      return(false);
     }
   if(minimum_gross_risk > target_gross_risk + 1.0e-9)
     {
      ++market_stop_min_lot_blocks;
      ++risk_admission_skips;
      RecordEvent(component,
                  "MARKET_STOP_MIN_LOT_BLOCK",
                  minimum_gross_risk,
                  target_gross_risk,
                  StringFormat("min_volume=%.8f atr=%.8f multiplier=%.10f stop=%.8f",
                               minimum_volume,
                               atr,
                               multiplier,
                               stop_loss));
   ...
   const long maximum_step_index =
      (long)MathFloor((maximum_volume - minimum_volume) /
                      volume_step + 1.0e-9);
   long lower_step_index = 0;
   long upper_step_index = maximum_step_index;
   long best_step_index = 0;
   double best_gross_risk = minimum_gross_risk;
   while(lower_step_index <= upper_step_index)
     {
      const long middle_step_index =
         lower_step_index +
         (upper_step_index - lower_step_index) / 2;
      const double candidate_volume =
         NormalizeDouble(minimum_volume +
                         (double)middle_step_index * volume_step,
                         8);
      double candidate_gross_risk = 0.0;
      if(!GrossStopRisk(symbol,
                        direction,
                        candidate_volume,
                        entry_price,
                        stop_loss,
                        candidate_gross_risk))
        {
         upper_step_index = middle_step_index - 1;
         continue;
        }
      if(candidate_gross_risk <= target_gross_risk + 1.0e-9)
        {
         best_step_index = middle_step_index;
         best_gross_risk = candidate_gross_risk;
         lower_step_index = middle_step_index + 1;
        }
      else
         upper_step_index = middle_step_index - 1;
     }
   ...
   volume =
      NormalizeDouble(minimum_volume +
                      (double)best_step_index * volume_step,
                      8);
   double buffered_risk = 0.0;
   if(volume < minimum_volume - 1.0e-9 ||
      volume > maximum_volume + 1.0e-9 ||
      !BufferedPlannedRisk(symbol,
                           direction,
                           volume,
                           entry_price,
                           ...
                           buffered_risk))
     {
      ...
      RecordEvent(component,
                  ...
                  volume,
                  best_gross_risk,
                  "selected volume or buffered risk invalid");
      volume = 0.0;
      stop_loss = 0.0;
      return(false);
     }
   ...
   const double aggregate_budget =
      capital * InpMaximumAggregateRiskFraction;
   const double tolerance = 0.01;
   if(buffered_risk > position_budget + tolerance ||
      aggregate_after > aggregate_budget + tolerance)
     {
      ++risk_admission_skips;
      RecordEvent(component,
                  ...
                  buffered_risk,
                  aggregate_after,
                  StringFormat("position_cap=%.4f aggregate_cap=%.4f market_stop=true",
                               position_budget,
                               aggregate_budget));
      volume = 0.0;
      stop_loss = 0.0;
   ...
   const double native_volume = NormalizedVolume(symbol);
   if(MathAbs(volume - native_volume) > 0.5 * volume_step)
      ++market_stop_non_native_volume;
   ++market_stop_accepted;
   planned_risk = position_budget;
   RecordEvent(component,
               "MARKET_STOP_REVERSE_LOT",
               actual_distance / atr,
               volume,
               StringFormat("atr=%.8f multiplier=%.10f stop=%.8f gross=%.8f target=%.8f buffered=%.8f native_volume=%.8f",
                            atr,
                            multiplier,
                            stop_loss,
                            best_gross_risk,
...
bool CalculateEntryStopAndVolume(const int component,
                                 const string symbol,
                                 const int direction,
                                 const double entry_price,
                                 const double minimum_distance,
                                 ...
                                 double &planned_risk)
  {
   volume = NormalizedVolume(symbol);
   stop_loss = 0.0;
   planned_risk = 0.0;
   if(!MarketStopTreatedComponent(component) ||
      !MarketStopCandidateActive())
     {
      if(volume <= 0.0 ||
         !CalculateProtectiveStop(component,
                                  symbol,
                                  direction,
                                  volume,
                                  entry_price,
                                  ...
                                  planned_risk))
         return(false);
      RecordNativeStopAtr(component,
                          entry_price,
                          stop_loss,
                          volume);
      return(true);
     }
   return(CalculateMarketStopReverseLot(component,
                                        symbol,
                                        direction,
                                        entry_price,
                                        minimum_distance,
