...
#property description "Project Zeta Terminus Next Lab market-stop reverse-lot sizing correction"
#property description "Execution version: zt-next-lab-market-stop-reverse-lot-sizing-ic1"

#include <Trade\Trade.mqh>

#include <ZetaTerminusNext\Domain\ZetaDomain.mqh>
...
void PrintRequiredSymbolContract(const string stage,
                                 const string symbol)
  {
   PrintFormat("%s MSRL62IC1_CONTRACT stage=%s symbol=%s "
...
void PrintRequiredContracts(const string stage)
  {
   PrintRequiredSymbolContract(stage, "US30");
   PrintRequiredSymbolContract(stage, "US100");
   PrintRequiredSymbolContract(stage, "US500");
  }
...
int OnInit()
  {
   ...
   if(!tester_mode)
     {
      PrintFormat("%s is tester-only", EXECUTION_VERSION);
      return(INIT_FAILED);
     }
   if(_Symbol != "US30" || _Period != PERIOD_M30 ||
       ...
       (InpMarketStopProfile != 0 && InpMarketStopProfile != 1) ||
       InpMarketStopActivation != D'2026.04.01 00:00:00' ||
       InpMarketStopAtrPeriod != 14 ||
       (InpMarketStopProfile == 1 &&
        (!MathIsValidNumber(InpMarketStopMultiplierRC16) ||
         InpMarketStopMultiplierRC16 <= 0.0 ||
         !MathIsValidNumber(InpMarketStopMultiplierRC4) ||
         InpMarketStopMultiplierRC4 <= 0.0 ||
         !MathIsValidNumber(InpMarketStopMultiplierCross) ||
         InpMarketStopMultiplierCross <= 0.0 ||
         !MathIsValidNumber(InpMarketStopMultiplierPressure) ||
         InpMarketStopMultiplierPressure <= 0.0 ||
         !MathIsValidNumber(InpMarketStopMultiplierReturn) ||
         InpMarketStopMultiplierReturn <= 0.0)) ||
   ...
   FolderCreate("ZetaTerminusNext\\research\\msrl62ic1");
   if(tester_mode)
   ...
   if(!InitializeConnectedRuntime())
      PrintFormat("%s waiting for the saved FPMarkets connection",
                  EXECUTION_VERSION);
   PrintFormat("%s market_stop profile=%d activation=%s atr_period=%d "
               "multipliers=%.10f,%.10f,%.10f,%.10f,%.10f",
               EXECUTION_VERSION,
               InpMarketStopProfile,
               TimeToString(InpMarketStopActivation, TIME_DATE|TIME_MINUTES),
               InpMarketStopAtrPeriod,
               InpMarketStopMultiplierRC16,
               InpMarketStopMultiplierRC4,
               InpMarketStopMultiplierCross,
               InpMarketStopMultiplierPressure,
               InpMarketStopMultiplierReturn);
...
void OnDeinit(const int reason)
  {
   ...
   PrintFormat("%s market_stop native_atr_observations=%I64d "
               "atr_unavailable=%I64d sizing_attempts=%I64d "
               "distance_blocks=%I64d min_lot_blocks=%I64d "
               "accepted=%I64d non_native_volume=%I64d",
               EXECUTION_VERSION,
