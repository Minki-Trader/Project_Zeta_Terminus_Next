...
The complete family-owned adapter freezes authority-gated `precheck / development / confirmation`, causal channels and ATR, the exact cross-symbol event sequence, balance-compounded risk reservations, Bid/Ask and signed-swap economics, complete-period maturation, the same-process path audit, no-overwrite output and conditional locked/native escalation. Its candidate-local outcome-free precheck found `920 / 684` medium/slow signals over `530` active dates and evaluated zero future exit path, lifecycle, economic metric or improvement value.

The one frozen development process completed both variants and the audit. Medium `120/60` accepted `67` starts and returned actual/stressed `-$74.42949 / -$76.24129` with `76.35493%` closed-balance drawdown. Slow `240/120` accepted `50` starts and returned `-$44.42683 / -$45.96653` with `77.18248%` drawdown. Both failed both-year profitability, V8-derived profit and drawdown gates; slow also failed turnover and both failed the per-year start floor.

The same-process audit evaluated all `1,604 × 4 = 6,416` mature signal paths. Mean raw/stressed 12-H4 R was `-0.05005 / -0.06767` for medium and `-0.08545 / -0.11183` for slow. Neither year, asset class or both directions retained consistent broad edge; only one isolated symbol qualified in each variant. Broad headroom is false and no bounded seed remains.

Status: closed valid adverse development with no passer and no broad improvement headroom. Locked 2026, confirmation, EA source, compile and MT5 Tester were never opened. The complete channel/ATR/stop/exit/hold/symbol/direction/priority/risk/sizing bundle is frozen against in-family rescue.
