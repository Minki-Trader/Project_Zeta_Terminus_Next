...
#property description "Project Zeta Terminus Next Lab native hold schedule curve"
#property description "Execution version: zt-next-lab-native-hold-schedule-curve-v1"
...
int OnInit()
  {
   ...
   if(InpHoldScheduleProfile < 0 || InpHoldScheduleProfile > 2)
   ...
   FolderCreate("ZetaTerminusNext\\lab\\hsc60");
   const string profile_name =
      (InpHoldScheduleProfile == 0 ? "NATIVE" :
       (InpHoldScheduleProfile == 1 ? "SHORT_25" : "LONG_25"));
   PrintFormat("%s hold_schedule profile=%s input=%d vector=%d/%d/%d/%d/%d/%d",
               EXECUTION_VERSION,
               profile_name,
               InpHoldScheduleProfile,
               component_definitions[RC16_LONG].hold_bars,
               component_definitions[RC4_BOTH].hold_bars,
               component_definitions[US100_CROSS].hold_bars,
               component_definitions[US30_PRESSURE].hold_bars,
               component_definitions[US30_RETURN_REV_LONG].hold_bars,
               component_definitions[US100_PASSIVE_LIMIT].hold_bars);
   if(tester_mode)
