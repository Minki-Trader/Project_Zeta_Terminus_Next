   ...
   if(state == ORDER_STATE_EXPIRED)
     {
      ...
      const bool afterimage_recorded =
         RtfObserveExpiredPassive(expired_direction,
                                  observed_server);
      ...
      const bool state_saved = SaveState();
      return(afterimage_recorded && state_saved);
     }
   ...
   for(int component = 0; component < COMPONENT_COUNT; ++component)
     {
      ...
      if(component == US30_RETURN_REV_LONG)
         effective_hold_bars =
            RtfReturnHoldBars(identifier, effective_hold_bars);
      else if(component == US100_CROSS &&
              !RtfCrossEffectiveHoldBars(ticket,
                                         identifier,
                                         held_bars,
                                         effective_hold_bars,
                                         effective_hold_bars))
         continue;
      if(held_bars < effective_hold_bars)
         continue;
      if(!CloseComponent(component, ticket))
         continue;
      if(component == US30_RETURN_REV_LONG)
         RtfRecordReturnRelease(identifier);
      else if(component == US100_CROSS)
         RtfRecordCrossClose(identifier);
     }
  }
