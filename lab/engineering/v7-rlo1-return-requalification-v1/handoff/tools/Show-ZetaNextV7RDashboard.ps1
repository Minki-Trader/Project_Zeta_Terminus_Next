...
$windowTitle = 'Project Zeta Terminus Next V7R Live-Dev 대시보드'
$script:RenderingForGui = $false

foreach ($requiredPath in @($statePath, $statusScript)) {
    if (-not (Test-Path -LiteralPath $requiredPath -PathType Leaf)) {
        throw "Required Next V7R dashboard file is missing: $requiredPath"
    }
}

function Test-V7ROperatorAuthorized {
    $text = Get-Content -LiteralPath $statePath -Raw
    return $text -match 'Next V7R return new-entry authorization:\s+`ENABLED`'
}
...
function Get-ComponentView {
    ...
    # Display-only descriptions mirror the frozen active V7R source. They are
...
function Get-KoreanAlert {
    ...
    if ($Alert -eq 'next_v7r_live_authorization_missing') { return 'Next V7R 신규 주문 권한이 상태 문서에 없음' }
    if ($Alert -eq 'next_v7r_current_snapshot_unavailable') { return 'Next V7R 상태 파일을 아직 읽을 수 없음' }
    if ($Alert -eq 'next_v7r_ex5_missing' -or $Alert -eq 'next_v7r_ex5_hash_mismatch') { return '동결된 V7R EX5가 없거나 해시가 다름' }
    ...
    if ($Alert -like 'snapshot_identity=*') { return '실행 중 EA의 Next V7R 신분이 다름' }
...
function Render-Dashboard {
    ...
    $liveMode = ([string]$Status.operator_mode -eq 'NEXT_V7R_LIVE')
    ...
    Write-UiLine -Text '위험계약: 개별 4% · 총 한도 12% · 원래 V7 6개 전략 · Passive 고정 0.01 lot' -Color DarkCyan
...
function Show-GraphicalDashboard {
    ...
    $mutex = [System.Threading.Mutex]::new(
        $true,
        'Local\ProjectZetaTerminusNextV7RLiveDevDashboard',
        [ref]$createdNew
    )
    if (-not $createdNew) {
        $mutex.Dispose()
    ...
    $titleLabel.Text = 'PROJECT ZETA TERMINUS NEXT  ·  V7R LIVE-DEV'
    ...
    $modeLabel.Text = 'Next V7R Live-Dev 상태 확인 중'
        ...
        try {
            ...
            if ([bool]$status.healthy) {
                ...
                $modeLabel.Text = if ([int]$status.expected_new_entries -eq 1) {
                    '신규 주문 허용 · Next V7R Live-Dev'
                } else {
                    '신규 주문 차단 · Next V7R 점검 모드'
                }
