...
#property description "Project Zeta Terminus Next Lab research observation ledger"
#property description "Execution version: zt-next-lab-live-research-observation-ledger-v1"
   ...
   if(tester_mode)
     {
      ...
      ResetResearchObservationArtifacts();
     }
...
void OnTick()
  {
   ...
   // Research sampling is read-only and writes nothing on the order path.
   // It runs before the Tester scheduler return so open-position excursions
   // still observe every primary-symbol real tick.
   ResearchSampleOpenPositions();
