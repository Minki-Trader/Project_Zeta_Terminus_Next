...
`campaigns/dd20-mt5-calibrated-exposure-margin-proxy-v1/` is declared with all economics unopened. Its `16,807`-point local `0.1` exposure lattice uses the near miss as an anchor rather than rerunning it. Selection DD is projected from all three observed MT5 anchors and receives a `0.5` percentage-point reserve; full paired-forward DD uses two MT5 anchors plus a `0.25`-point reserve. Selection, full June/July, independent June and independent July gates may nominate at most one new MT5 hypothesis.
