...
The user's prospective objective remains maximum actual and doubled-cost-stressed profit inside a hard `20%` MT5 equity-drawdown budget. `campaigns/dd20-capital-composition-proxy-v1/` is the active serial stage. It holds four whole-portfolio gross budgets fixed, exhaustively reallocates each among the six components over `29,016` compositions, freezes at most three selection roles before opening the isolated later segment, and launches no MT5 during the proxy stage.
