...
`campaigns/dd20-native-gap-july-robustness-mt5-v1/` is compiled and ready with economics unopened. It materializes only weights `1.6 / 0.8 / 0.4 / 3.2 / 1.2 / 0` in a self-contained source/configuration/runtime family with unique identity, state, research, SET and report paths. MetaEditor build 6140 produced `0 errors / 0 warnings`; exactly one selection and one complete full June/July forward are permitted. Qualification additionally requires positive actual/stressed continuous June and July slices and native selection/forward equity DD `<=20%`.
