...
`campaigns/dd20-native-gap-july-robustness-mt5-v1/` is declared with compilation and economics unopened. It materializes only weights `1.6 / 0.8 / 0.4 / 3.2 / 1.2 / 0` in a self-contained source/configuration family with unique identity, state, research, SET and report paths. After compilation, exactly one selection and one complete full June/July forward are permitted; qualification additionally requires positive actual/stressed continuous June and July slices and native selection/forward equity DD `<=20%`.
