...
`campaigns/dd20-executable-volume-lattice-proxy-v1/` is the active serial stage. It searches exactly `112,752` component compositions while replaying the `0.01` lot lattice, `$150` daily sizing ladder and `0.04 / 0.18` position/aggregate risk admission from source birth/close order. Selection DD is calibrated upward-only from the preserved unweighted and high-exposure MT5 anchors. One maximum-stressed-profit role freezes before isolated-later access and can nominate at most one new MT5 candidate; the original 15 combinations remain untouched.
