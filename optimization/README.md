...
`campaigns/dd20-native-gap-july-robustness-proxy-v1/` is declared with all economics unopened. Its `49,049`-composition grid reduces range/cross exposure and expands intraday exposure. Selection and full-pair DD are charged the worst preserved positive native-MT5-minus-proxy gaps plus `0.75 / 0.5` percentage-point reserves. July actual/stressed net must remain positive after subtracting the worst observed continuous-run shortfalls `$6.82 / $6.8535` and a further `$1` reserve. At most one maximum-selection-profit role can be shortlisted, and no MT5 runs inside the proxy.
