...
`campaigns/dd20-capital-addition-ladder-proxy-v1/` is declared with economics unopened. It fixes qualified weights `1.6 / 0.8 / 0.4 / 3.2 / 1.2 / 0` and searches 50 new `$5`-spaced addition steps over `$50..$145` and `$155..$300`; closed `$150` is an appended calibration row only. Selection receives the same-composition DD gap plus `0.25pp`, only `50%` of incremental proxy profit plus a `$50` deduction, and must still exceed the qualified MT5 nets. Full-pair and July same-composition corrections and every prior chronological/month gate remain. Individual gate counts are predeclared; at most one MT5 hypothesis may emerge and no earlier path reruns.
