...
`campaigns/dd20-mt5-calibrated-exposure-margin-proxy-v1/` is corrected with all economics still unopened. Its `16,807`-point local `0.1` exposure lattice uses the near miss as an anchor rather than rerunning it. Selection DD is projected from all three observed MT5 anchors and receives a `0.5` percentage-point reserve. The first invocation stopped before output because paired raw DD was non-monotonic; full paired-forward calibration now fits the two anchors' observed-minus-raw gap against total exposure and retains the declared `0.25`-point reserve. Selection, full June/July, independent June and independent July gates may nominate at most one new MT5 hypothesis.
