...
`campaigns/dd20-deferred-profit-accelerator-mt5-v1/` is declared with source and configuration frozen, economics unopened. It derives once from the fully qualified MT5 family and changes only identity plus the causal sizing schedule: `$150` pre-activation, activation at `+$450` stressed closed profit, `$50` post-activation step, cap `14`. Thirteen Include modules remain byte-identical; Domain, Portfolio sizing and EA assembly are isolated in the new root. Exactly one long real-tick selection and one independently initialized June/July forward are permitted after a dedicated Portable and compile.
