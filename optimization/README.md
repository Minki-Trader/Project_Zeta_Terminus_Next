...
`campaigns/dd20-paired-month-stability-mt5-v1/` is active with source and configuration frozen and economics unopened. It materializes only weights `2 / 1.5 / 2 / 2.5 / 1.5 / 0` in a one-time self-contained copy of the final corrected executable-volume/protection path. One dedicated MT5 Portable may run exactly one real-tick selection and, after a valid completion, one full June/July forward. The original 15 points and prior rejected MT5 candidate remain closed without rerun.
