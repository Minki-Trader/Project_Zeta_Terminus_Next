...
`campaigns/dd20-qualified-profit-redistribution-proxy-v1/` is declared with economics unopened. Its `184,041` compositions expand range-61 and intraday while opening range-64, cross and return down to zero. Every candidate pays the worst five-anchor selection native-minus-raw DD gap `2.122151` points plus `0.25`, the worst proxy profit overstatement `+$659.7999 / +$615.1109` plus `$50`, the paired-forward DD correction and the continuous-July shortfall/reserve. Corrected selection actual/stressed must strictly beat the qualified MT5 anchor, all four epochs and June/July gates must pass, and at most one new MT5 hypothesis may emerge. No anchor or original combination reruns.
