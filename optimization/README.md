...
`campaigns/dd20-paired-month-stability-mt5-v1/` is active and compiled with economics unopened. It materializes only weights `2 / 1.5 / 2 / 2.5 / 1.5 / 0` in a one-time self-contained copy of the final corrected executable-volume/protection path. Its dedicated physical Portable has exact source/configuration parity and build 6140 produced `0 errors / 0 warnings`. It may now run exactly one real-tick selection and, after valid completion, one full June/July forward. The original 15 points and prior rejected MT5 candidate remain closed without rerun.
