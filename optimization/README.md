...
`campaigns/dd20-paired-month-stability-proxy-v1/` is the active serial stage with all outcomes unopened. It keeps the exact `112,752` executable-lattice search and long-selection `20%` DD gates, then requires every candidate to pass June and July independently from `$100` before one maximum-long-selection-stressed-profit role freezes. No proxy holdout remains afterward; one passing role may advance to one MT5 selection/full-forward hypothesis.
