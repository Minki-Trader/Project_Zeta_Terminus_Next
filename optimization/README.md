...
`campaigns/dd20-capital-composition-mt5-v1/` is the active serial stage. It materializes exactly that one hypothesis with fixed `0.04 / 0.18` risk caps and component risk multipliers, a unique tester-only identity and an independent physical Portable. It runs one selection plus one custom-forward pass only; the preserved 15 combinations and their parent comparator are not rerun.
