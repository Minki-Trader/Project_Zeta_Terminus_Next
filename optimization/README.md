...
`campaigns/dd20-deferred-profit-accelerator-mt5-v1/` is compiled and ready with economics unopened. A new physical Portable owns the exact source/configuration overlay, distinct terminal file identity and zero process owner; 19 campaign/runtime files match. One MetaEditor build 6140 invocation produced `0 errors / 0 warnings` and EX5 `F39D4465...B95A84`. Exactly one long real-tick selection and, after any complete valid selection, one independently initialized June/July forward are permitted. The qualified anchor and every previous path remain closed without rerun.
