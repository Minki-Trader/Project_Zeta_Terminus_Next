...
`campaigns/dd20-capped-profit-accelerator-proxy-v1/` is declared with economics unopened. It crosses the 18 closed profit-region steps `$50..$135` with caps `12,13,14,15,16,18,20,24,30,40,60`, retaining only a cap strictly below that step's observed uncapped maximum. The resulting `168` paths all bind and are therefore economically new; no constant-ladder row reruns. Selection applies the same DD gap/reserve and 50%-incremental-profit/$50-reserve contract, while every epoch, paired-forward, June and July gate stays unchanged. At most one maximum-conservative-profit path may advance to MT5.
