...
`campaigns/dd20-profit-protected-risk-ceiling-proxy-v1/` is declared with all economics unopened. It keeps the rejected accelerator's weights, `$150 → $50 after +$450` schedule and cap `14` fixed, and varies only a new fixed aggregate planned-risk dollar ceiling from `$30..$330` by `$10`. Every one of the `31` candidates must bind in the long selection while remaining nonbinding across the exact fresh forward, whose observed maximum aggregate planned risk was only `$20.79136`. Candidate selection DD pays the rejected accelerator's observed native-minus-retained-path gap plus reserve and cannot claim less than the qualified native-DD floor. No prior grid or MT5 path reruns; commit and push this declaration before one proxy invocation.
