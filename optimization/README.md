...
`campaigns/dd20-exposure-margin-mt5-v1/` is compiled and ready with that sole `1.8 / 1.3 / 1.4 / 2.6 / 1.5 / 0` hypothesis. Its source, identity, fixed SET and selection/paired-forward INIs are frozen in a new dedicated physical Portable; the normal build completed at `0 errors / 0 warnings`. Tester and all MT5 economics remain unopened. It will run exactly one real-tick selection and one full June/July forward; no closed candidate reruns.
