...
#property description "Execution version: zt-next-v7-causal-admission-learner-v1"

#include <Trade\Trade.mqh>

#include <ZetaV7CALearner\Domain\ZetaDomain.mqh>
#include <ZetaV7CALearner\Time\ZetaSessionClock.mqh>
#include <ZetaV7CALearner\Strategies\ZetaStrategyShared.mqh>
#include <ZetaV7CALearner\Strategies\ZetaRC16.mqh>
#include <ZetaV7CALearner\Strategies\ZetaRC4.mqh>
#include <ZetaV7CALearner\Strategies\ZetaCross.mqh>
#include <ZetaV7CALearner\Strategies\ZetaPressure.mqh>
#include <ZetaV7CALearner\Strategies\ZetaReturn.mqh>
#include <ZetaV7CALearner\Strategies\ZetaPassive.mqh>
#include <ZetaV7CALearner\Portfolio\ZetaPortfolioRisk.mqh>
#include <ZetaV7CALearner\Execution\ZetaOwnership.mqh>
#include <ZetaV7CALearner\Execution\ZetaOrders.mqh>
#include <ZetaV7CALearner\Execution\ZetaProtectionAndReconciliation.mqh>
#include <ZetaV7CALearner\Persistence\ZetaStateAndEvents.mqh>
#include <ZetaV7CALearner\Observation\ZetaResearchObservation.mqh>

#include <ZetaV7CALearner\Observation\ZetaReturnDiagnostics.mqh>

#include <ZetaV7CALearner\Learning\ZetaCausalAdmission.mqh>
...
bool InitializeConnectedRuntime()
  {
   ...
   if(tester_mode)
      PrintV7CARequiredContracts("START");
...
int OnInit()
  {
   ...
   if(InpCausalRunTag=="unassigned" || (InpCausalRole<1 || InpCausalRole>3)) return(INIT_PARAMETERS_INCORRECT);
   ...
   if(!CausalPreparePaths()) return(INIT_FAILED);
   if(tester_mode)
      ResetTesterArtifacts();
   if(!AcquireRuntimeOwnership())
      return(INIT_FAILED);
   ResetRuntimeState();
   if(!CausalInitialize()) return(INIT_FAILED);
...
double OnTester()
  {
   CausalFinish();
   PrintFormat("V7CA_NATIVE profit=%.6f equity_dd=%.6f equity_dd_relative_pct=%.9f balance_dd_relative_pct=%.9f trades=%.0f",
   ...
   PrintFormat("%s V7CA_UNAVAILABLE_SUMMARY aggregate=%I64d rows=%I64d "
   ...
   PrintV7CARequiredContracts("END");
   ...
   const bool correction_required =
      (causal_faults!=0 || portfolio_state.safety_stopped || persistence_failed ||
   ...
   if(correction_required)
     {
      PrintFormat("V7CA_RESULT status=CORRECTION_REQUIRED "
   ...
   PrintFormat("V7CA_RESULT status=ECONOMIC position_risk=%.6f "
