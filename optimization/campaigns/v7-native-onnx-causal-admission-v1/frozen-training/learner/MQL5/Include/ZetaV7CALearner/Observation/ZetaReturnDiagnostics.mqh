...
void ObserveArcUnavailability(const datetime current_bar)
  {
   ...
   if(available)
      ...
      PrintFormat("%s V7CA_UNAVAILABLE reason_class=%s reason_detail=%s "
...
void PrintV7CARequiredSymbolContract(const string stage,
                                     const string symbol)
  {
   PrintFormat("%s V7CA_CONTRACT stage=%s symbol=%s "
...
void PrintV7CARequiredContracts(const string stage)
  {
   PrintV7CARequiredSymbolContract(stage, "US30");
   PrintV7CARequiredSymbolContract(stage, "US100");
   PrintV7CARequiredSymbolContract(stage, "US500");
  }
