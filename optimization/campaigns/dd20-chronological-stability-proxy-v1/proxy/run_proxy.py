...
def main() -> None:
    ...
    selection_eligible_indices = np.flatnonzero(selection_eligible)
    ...
    development_weights = weights[selection_eligible_indices]
    development = simulate(development_events, development_weights, config, [])
    development_pass = (
    ...
    qualified_local_indices = np.flatnonzero(development_pass)
    qualified_pairs = [
        (int(selection_eligible_indices[local_index]), int(local_index))
        for local_index in qualified_local_indices
    ]

    def rank_key(pair: tuple[int, int]) -> tuple[Any, ...]:
        selection_index, development_index = pair
        return (
            -float(selection["stressed_net"][selection_index]),
            -float(development["stressed_net"][development_index]),
            -float(selection["actual_net"][selection_index]),
            -float(development["actual_net"][development_index]),
            -float(selection["epoch_stressed_net"][selection_index, -1]),
            float(calibrated_dd[selection_index]),
            tuple(float(value) for value in weights[selection_index]),
        )

    ranked = sorted(qualified_pairs, key=rank_key)
    winner_pair = ranked[0] if ranked else None
    top_count = int(config["output_top_selection_records"])
    top_development_qualified: list[dict[str, Any]] = []
    for selection_index, development_index in ranked[:top_count]:
        record = record_for_index(
            selection_index, weights, selection, calibrated_dd, epochs, components
        )
        ...
        top_development_qualified.append(record)

    confirmation_record = None
    july_confirmation_passed = False
    selection_winner = None
    if winner_pair is not None:
        winner_index, winner_development_index = winner_pair
        selection_winner = record_for_index(
            winner_index, weights, selection, calibrated_dd, epochs, components
        )
        selection_winner["development_june"] = record_for_index(
            winner_development_index,
            development_weights,
            development,
            None,
            [],
        ...
        confirmation_config = config["confirmation_period"]
        confirmation_events = extract_segment(
            lifecycle_path,
            int(config["input"]["later_segment_index"]),
            confirmation_config["expected"],
            components,
            datetime.strptime(confirmation_config["start"], TIME_FORMAT),
            datetime.strptime(confirmation_config["end"], TIME_FORMAT),
        )
        winner_weights = weights[winner_index : winner_index + 1]
        confirmation = simulate(confirmation_events, winner_weights, config, [])
        confirmation_record = record_for_index(
            0, winner_weights, confirmation, None, [], components
        )
        july_confirmation_passed = bool(
            confirmation["actual_net"][0] > 0.0
            and confirmation["stressed_net"][0] > 0.0
            and confirmation["minimum_balance"][0] > 0.0
            and confirmation["raw_drawdown_pct"][0] <= hard_dd + 1.0e-12
        )
    ...
    if len(selection_eligible_indices) == 0:
        status = "VALID_PROXY_COMPLETE_NO_SELECTION_ELIGIBLE"
    elif winner_pair is None:
        status = "VALID_PROXY_COMPLETE_NO_JUNE_STABLE_CANDIDATE"
    elif july_confirmation_passed:
        status = "VALID_PROXY_COMPLETE_ONE_MT5_SHORTLIST"
    else:
        status = "VALID_PROXY_COMPLETE_JUNE_STABLE_WINNER_FAILS_JULY_NO_MT5_SHORTLIST"

    result = {
        "schema": "zeta-dd20-chronological-stability-proxy-raw-result-v1",
        ...
        "selection_search": {
            ...
            "eligible_compositions": int(len(selection_eligible_indices)),
            "eligible_fraction": float(len(selection_eligible_indices) / len(weights)),
        ...
        "development_june": {
            ...
            "selection_eligible_candidates_evaluated": int(
                len(selection_eligible_indices)
            ),
            "development_qualified_candidates": int(len(qualified_pairs)),
            "development_qualified_fraction_of_selection_eligible": float(
                len(qualified_pairs) / max(1, len(selection_eligible_indices))
            ),
        ...
        "top_development_qualified": top_development_qualified,
        "confirmation_july": confirmation_record,
        "confirmation_july_opened_only_after_winner_froze": True,
        "confirmation_july_used_in_ranking_or_weight_tuning": False,
        "confirmation_july_passed": july_confirmation_passed,
        "mt5_shortlist": (
            [
                {
                    ...
                    "confirmation_july_stressed_net_usd": confirmation_record[
                        "stressed_net_usd"
                    ],
                    "confirmation_july_raw_drawdown_pct": confirmation_record[
                        "raw_closed_balance_drawdown_pct"
                    ],
                }
            ]
            if july_confirmation_passed
            else []
        ),
        "boundary": {
            "proxy_completed": True,
            "july_failed_candidate_rescue_or_retune": False,
            "mt5_launched": False,
