...
def candidate_triples(
    ...
    thresholds = np.asarray(
        config["candidate_activation_growth_thresholds_usd"], dtype=np.float64
    )
    post_steps = np.asarray(
        config["candidate_post_activation_addition_steps_usd"], dtype=np.float64
    )
    caps = np.asarray(
        config["candidate_maximum_day_multiplier_caps"], dtype=np.int64
    )
    baseline_step = float(config["baseline_addition_step_usd"])
    if (
        len(np.unique(thresholds)) != len(thresholds)
        or np.any(thresholds <= 0.0)
        or len(np.unique(post_steps)) != len(post_steps)
        or np.any(post_steps <= 0.0)
        or len(np.unique(caps)) != len(caps)
        or np.any(caps < 1)
    ):
        raise RuntimeError("deferred accelerator grids must be unique and positive")
    if np.any(np.abs(thresholds / baseline_step - np.round(thresholds / baseline_step)) > 1.0e-9):
        raise RuntimeError("activation thresholds must be exact baseline-step multiples")
    for threshold in thresholds:
        baseline_multiplier_at_activation = 1 + int(round(threshold / baseline_step))
        if np.any(caps <= baseline_multiplier_at_activation):
            raise RuntimeError("candidate cap removes pre-activation baseline headroom")

    for name, expected_campaign in (
        ("closed_constant_ladder_result_file", "dd20-capital-addition-ladder-proxy-v1"),
        ("closed_capped_accelerator_result_file", "dd20-capped-profit-accelerator-proxy-v1"),
    ):
        closed = json.loads(
            (input_root / str(config["input"][name])).read_text(encoding="utf-8")
        )
        if (
            closed.get("status") != "VALID_PROXY_COMPLETE_NO_SELECTION_ELIGIBLE"
            or closed.get("campaign") != expected_campaign
        ):
            raise RuntimeError("closed predecessor result identity mismatch")

    triples = [
        (float(threshold), float(post_step), int(cap))
        for threshold in thresholds
        for post_step in post_steps
        for cap in caps
    ]
    if len(triples) != int(config["expected_candidates"]):
        raise RuntimeError("deferred accelerator triple count does not match contract")
    if len(set(triples)) != len(triples):
        raise RuntimeError("deferred accelerator triples must be unique")
    return (
        np.asarray([value[0] for value in triples], dtype=np.float64),
        np.asarray([value[1] for value in triples], dtype=np.float64),
        np.asarray([value[2] for value in triples], dtype=np.int64),
    )


def add_anchor_triple(
    thresholds: np.ndarray,
    post_steps: np.ndarray,
    caps: np.ndarray,
    anchor_threshold: float,
    anchor_post_step: float,
    anchor_cap: int,
) -> tuple[np.ndarray, np.ndarray, np.ndarray, int]:
    matches = np.flatnonzero(
        np.isclose(thresholds, anchor_threshold)
        & np.isclose(post_steps, anchor_post_step)
        & (caps == anchor_cap)
    )
    if len(matches) != 0:
        raise RuntimeError("closed anchor triple appears in the candidate grid")
    return (
        np.append(thresholds, anchor_threshold),
        np.append(post_steps, anchor_post_step),
        np.append(caps, anchor_cap),
        len(thresholds),
    )
...
def simulate(
    events: list[LifecycleEvent],
    weights: np.ndarray,
    activation_growth_thresholds: np.ndarray,
    post_activation_addition_steps: np.ndarray,
    maximum_day_multiplier_caps: np.ndarray,
    config: dict[str, Any],
    declared_epochs: list[dict[str, Any]],
) -> dict[str, np.ndarray]:
    candidate_count, component_count = weights.shape
    if (
        len(activation_growth_thresholds) != candidate_count
        or len(post_activation_addition_steps) != candidate_count
        or len(maximum_day_multiplier_caps) != candidate_count
    ):
        raise RuntimeError("threshold, post-step, cap and weight row counts differ")
    if (
        np.any(activation_growth_thresholds <= 0.0)
        or np.any(post_activation_addition_steps <= 0.0)
        or np.any(maximum_day_multiplier_caps < 1)
    ):
        raise RuntimeError("deferred accelerator controls must be positive")
    ...
    baseline_addition_step = float(model["pre_activation_addition_step_usd"])
    ...
    for event in events:
        ...
        if event_day != current_day:
            ...
            baseline_growth = np.minimum(growth, activation_growth_thresholds)
            accelerated_growth = np.maximum(
                0.0, growth - activation_growth_thresholds
            )
            uncapped_day_multiplier = (
                1
                + np.floor(
                    baseline_growth / baseline_addition_step + 1.0e-9
                ).astype(np.int64)
                + np.floor(
                    accelerated_growth / post_activation_addition_steps + 1.0e-9
                ).astype(np.int64)
            )
            day_multiplier = np.minimum(
                np.maximum(1, uncapped_day_multiplier), maximum_day_multiplier_caps
            )
...
def record_for_index(
    ...
    record: dict[str, Any] = {
        "pre_activation_addition_step_usd": 150.0,
        "activation_growth_threshold_usd": float(
            activation_growth_thresholds[index]
        ),
        "post_activation_addition_step_usd": float(
            post_activation_addition_steps[index]
        ),
        "maximum_day_multiplier_cap": int(caps[index]),
...
def main() -> None:
    ...
    thresholds, post_steps, caps = candidate_triples(config, input_root)
    candidate_count = len(thresholds)
    (
        evaluation_thresholds,
        evaluation_post_steps,
        evaluation_caps,
        anchor_index,
    ) = add_anchor_triple(
        thresholds,
        post_steps,
        caps,
        float(config["baseline_anchor_activation_growth_threshold_usd"]),
        float(config["baseline_anchor_post_activation_addition_step_usd"]),
        int(config["baseline_anchor_maximum_day_multiplier_cap"]),
    )
    ...
    evaluation_weights = np.tile(fixed_weights, (len(evaluation_thresholds), 1))
    ...
    def final_rank_key(index: int) -> tuple[Any, ...]:
        ...
        return (
            ...
            int(evaluation_caps[index]),
            -float(evaluation_thresholds[index]),
            -float(evaluation_post_steps[index]),
        )
    ...
    result = {
        "schema": "zeta-dd20-deferred-profit-accelerator-proxy-raw-result-v1",
        ...
        "implementation": {
            ...
            "deferred_candidate_construction_gate": "PASS",
        },
        "search": {
            "candidate_triples": candidate_count,
            "activation_threshold_grid_values": int(
                len(config["candidate_activation_growth_thresholds_usd"])
            ),
            "post_activation_step_grid_values": int(
                len(config["candidate_post_activation_addition_steps_usd"])
            ),
            "maximum_day_multiplier_cap_grid_values": int(
                len(config["candidate_maximum_day_multiplier_caps"])
            ),
            "evaluation_rows_including_closed_anchor": int(
                len(evaluation_thresholds)
            ),
            "fixed_component_weights": [float(value) for value in fixed_weights],
            "pre_activation_addition_step_usd": float(
                config["baseline_addition_step_usd"]
            ),
            "closed_anchor_activation_growth_threshold_usd": None,
            "closed_anchor_post_activation_addition_step_usd": float(
                config["baseline_anchor_post_activation_addition_step_usd"]
            ),
            "closed_anchor_maximum_day_multiplier_cap": None,
            "closed_anchor_triple_in_candidate_grid": False,
            "all_activation_thresholds_are_baseline_step_multiples": True,
            "all_caps_preserve_pre_activation_baseline_headroom": True,
            ...
            [
                {
                    ...
                    "activation_growth_threshold_usd": winner[
                        "activation_growth_threshold_usd"
                    ],
                    "post_activation_addition_step_usd": winner[
                        "post_activation_addition_step_usd"
                    ],
                    "maximum_day_multiplier_cap": winner[
                        "maximum_day_multiplier_cap"
                    ],
        ...
        "boundary": {
            ...
            "closed_constant_ladder_grid_rerun": False,
            "closed_capped_accelerator_grid_rerun": False,
