...
def main() -> None:
    ...
    july_weights = weights[[pair[0] for pair in qualified_pairs]]
    july = simulate(july_events, july_weights, config, [])
    july_pass = (
        (july["actual_net"] > 0.0)
    ...
    paired_candidates = [
        (selection_index, june_index, int(july_index))
        for july_index, (selection_index, june_index) in enumerate(qualified_pairs)
        if bool(july_pass[july_index])
    ]

    def rank_key(candidate: tuple[int, int, int]) -> tuple[Any, ...]:
        selection_index, june_index, july_index = candidate
        weaker_month = min(
            float(development["stressed_net"][june_index]),
            float(july["stressed_net"][july_index]),
        )
        paired_month_sum = (
            float(development["stressed_net"][june_index])
            + float(july["stressed_net"][july_index])
    ...
    ranked = sorted(paired_candidates, key=rank_key)
    winner_candidate = ranked[0] if ranked else None
    top_count = int(config["output_top_selection_records"])
    top_paired_month_qualified: list[dict[str, Any]] = []
    for selection_index, june_index, july_index in ranked[:top_count]:
        ...
        top_paired_month_qualified.append(record)

    selection_winner = None
    if winner_candidate is not None:
        winner_index, winner_june_index, winner_july_index = winner_candidate
        ...
        selection_winner["development_july"] = record_for_index(
            winner_july_index,
            july_weights,
            july,
            None,
    ...
    elif len(qualified_pairs) == 0:
        status = "VALID_PROXY_COMPLETE_NO_JUNE_STABLE_CANDIDATE"
    elif winner_candidate is None:
        status = "VALID_PROXY_COMPLETE_NO_PAIRED_MONTH_STABLE_CANDIDATE"
    else:
        status = "VALID_PROXY_COMPLETE_ONE_MT5_SHORTLIST"

    result = {
        "schema": "zeta-dd20-paired-month-stability-proxy-raw-result-v1",
        ...
        "development_july": {
            ...
            "june_qualified_candidates_evaluated": int(len(qualified_pairs)),
            "paired_month_qualified_candidates": int(len(paired_candidates)),
            "paired_month_qualified_fraction_of_june_qualified": float(
                len(paired_candidates) / max(1, len(qualified_pairs))
            ),
        ...
        "top_paired_month_qualified": top_paired_month_qualified,
        "proxy_holdout_remaining_after_paired_month_gate": False,
        "mt5_shortlist": (
            [
                {
        ...
        "boundary": {
            ...
            "june_and_july_used_only_as_predeclared_paired_gates": True,
