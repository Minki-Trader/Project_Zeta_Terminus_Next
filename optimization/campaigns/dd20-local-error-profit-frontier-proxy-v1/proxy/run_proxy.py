...
OUTPUT_PATH = (
    ...
    / "artifacts"
    / "raw"
    / "dd20-local-error-profit-frontier-proxy-v1"
    / "output"
    / "proxy-result.json"
)
...
def frozen_l1_lipschitz_slope(
    anchor_weights: np.ndarray,
    anchor_values: list[float],
    frozen_expected_slope: float,
) -> float:
    if len(anchor_weights) != len(anchor_values) or len(anchor_values) < 2:
        raise RuntimeError("composition-local calibration anchors are incomplete")
    slope = 0.0
    for left in range(len(anchor_values)):
        for right in range(left + 1, len(anchor_values)):
            distance = float(
                np.sum(np.abs(anchor_weights[left] - anchor_weights[right]))
            )
            if distance <= 0.0:
                raise RuntimeError("distinct calibration anchors share one weight row")
            slope = max(
                slope,
                abs(float(anchor_values[left]) - float(anchor_values[right]))
                / distance,
            )
    if abs(slope - frozen_expected_slope) > 1.0e-7:
        raise RuntimeError("frozen composition-local Lipschitz slope mismatch")
    return slope


def composition_local_upper(
    ...
    distances = np.sum(
        np.abs(evaluation_weights[:, None, :] - anchor_weights[None, :, :]),
        axis=2,
    )
    bounds = np.asarray(anchor_values, dtype=np.float64)[None, :] + slope * distances
    return np.maximum(0.0, np.min(bounds, axis=1))


def require_anchor_upper_reproduction(
    evaluation_weights: np.ndarray,
    anchor_weights: np.ndarray,
    anchor_values: list[float],
    upper_values: np.ndarray,
) -> None:
    for weights, value in zip(anchor_weights, anchor_values, strict=True):
        index = find_weight_index(evaluation_weights, weights)
        expected = max(0.0, float(value))
        if abs(float(upper_values[index]) - expected) > 1.0e-7:
            raise RuntimeError("composition-local anchor upper envelope mismatch")
...
def main() -> None:
    ...
    selection_anchor_gaps = [
        float(observed - raw)
        for raw, observed in zip(
            raw_selection_anchors, observed_selection_anchors, strict=True
        )
    ]
    selection_anchor_weights = np.asarray(
        [anchor["weights"] for anchor in calibration["anchors"]],
        dtype=np.float64,
    )
    selection_gap_slope = frozen_l1_lipschitz_slope(
        selection_anchor_weights,
        selection_anchor_gaps,
        float(
            calibration[
                "frozen_l1_lipschitz_slope_percentage_points_per_weight"
            ]
        ),
    )
    local_selection_gap = composition_local_upper(
        evaluation_weights,
        selection_anchor_weights,
        selection_anchor_gaps,
        selection_gap_slope,
    )
    require_anchor_upper_reproduction(
        evaluation_weights,
        selection_anchor_weights,
        selection_anchor_gaps,
        local_selection_gap,
    )
    calibrated_dd = selection["raw_drawdown_pct"] + local_selection_gap
    ...
    selection_actual_error_slope = frozen_l1_lipschitz_slope(
        selection_anchor_weights,
        selection_actual_shortfalls,
        float(
            selection_profit_calibration[
                "frozen_actual_l1_lipschitz_slope_usd_per_weight"
            ]
        ),
    )
    selection_stressed_error_slope = frozen_l1_lipschitz_slope(
        selection_anchor_weights,
        selection_stressed_shortfalls,
        float(
            selection_profit_calibration[
                "frozen_stressed_l1_lipschitz_slope_usd_per_weight"
            ]
        ),
    )
    local_selection_actual_shortfall = composition_local_upper(
        evaluation_weights,
        selection_anchor_weights,
        selection_actual_shortfalls,
        selection_actual_error_slope,
    )
    local_selection_stressed_shortfall = composition_local_upper(
        evaluation_weights,
        selection_anchor_weights,
        selection_stressed_shortfalls,
        selection_stressed_error_slope,
    )
    require_anchor_upper_reproduction(
        evaluation_weights,
        selection_anchor_weights,
        selection_actual_shortfalls,
        local_selection_actual_shortfall,
    )
    require_anchor_upper_reproduction(
        evaluation_weights,
        selection_anchor_weights,
        selection_stressed_shortfalls,
        local_selection_stressed_shortfall,
    )
    ...
    paired_anchor_gaps = [
        float(observed - raw)
        for raw, observed in zip(
            raw_paired_anchors, observed_paired_anchors, strict=True
        )
    ]
    paired_anchor_weights = np.asarray(
        [anchor["weights"] for anchor in paired_calibration["anchors"]],
        dtype=np.float64,
    )
    paired_gap_slope = frozen_l1_lipschitz_slope(
        paired_anchor_weights,
        paired_anchor_gaps,
        float(
            paired_calibration[
                "frozen_l1_lipschitz_slope_percentage_points_per_weight"
    ...
    local_paired_gap = composition_local_upper(
        evaluation_weights,
        paired_anchor_weights,
        paired_anchor_gaps,
        paired_gap_slope,
    ...
    calibrated_paired_dd = paired["raw_drawdown_pct"] + local_paired_gap
    ...
    july_anchor_weights = np.asarray(
        [anchor["weights"] for anchor in july_calibration["anchors"]],
        dtype=np.float64,
    )
    july_actual_error_slope = frozen_l1_lipschitz_slope(
        july_anchor_weights,
        july_actual_shortfalls,
        float(
            july_calibration[
                "frozen_actual_l1_lipschitz_slope_usd_per_weight"
            ]
    ...
    july_stressed_error_slope = frozen_l1_lipschitz_slope(
        july_anchor_weights,
        july_stressed_shortfalls,
        float(
            july_calibration[
                "frozen_stressed_l1_lipschitz_slope_usd_per_weight"
            ]
    ...
    local_july_actual_shortfall = composition_local_upper(
        evaluation_weights,
        july_anchor_weights,
        july_actual_shortfalls,
        july_actual_error_slope,
    )
    local_july_stressed_shortfall = composition_local_upper(
        evaluation_weights,
        july_anchor_weights,
        july_stressed_shortfalls,
        july_stressed_error_slope,
    )
    require_anchor_upper_reproduction(
        evaluation_weights,
        july_anchor_weights,
        july_actual_shortfalls,
        local_july_actual_shortfall,
    )
    require_anchor_upper_reproduction(
        evaluation_weights,
        july_anchor_weights,
        july_stressed_shortfalls,
        local_july_stressed_shortfall,
    )
    july_reserve = float(july_calibration["uncertainty_reserve_usd"])
    conservative_july_actual = (
        july["actual_net"] - local_july_actual_shortfall - july_reserve
    )
    conservative_july_stressed = (
        july["stressed_net"] - local_july_stressed_shortfall - july_reserve
    )
    raw_july_eligible = (
        development_eligible
        & (july["actual_net"] > 0.0)
    ...
    def complete_record(index: int) -> dict[str, Any]:
        ...
        record["selection_composition_local_gap_upper_percentage_points"] = float(
            local_selection_gap[index]
        )
        record["selection_gap_l1_lipschitz_slope_percentage_points_per_weight"] = (
            selection_gap_slope
        )
        record["selection_drawdown_uncertainty_reserve_percentage_points"] = (
            selection_reserve
        )
        record["selection_composition_local_actual_profit_shortfall_usd"] = float(
            local_selection_actual_shortfall[index]
        )
        record["selection_composition_local_stressed_profit_shortfall_usd"] = float(
            local_selection_stressed_shortfall[index]
        )
        record["selection_actual_error_l1_lipschitz_slope_usd_per_weight"] = (
            selection_actual_error_slope
        )
        record["selection_stressed_error_l1_lipschitz_slope_usd_per_weight"] = (
            selection_stressed_error_slope
        )
        ...
        record["paired_full_forward"][
            "composition_local_gap_upper_percentage_points"
        ] = float(local_paired_gap[index])
        record["paired_full_forward"][
            "gap_l1_lipschitz_slope_percentage_points_per_weight"
        ] = paired_gap_slope
        record["paired_full_forward"][
        ...
        july_record["composition_local_actual_shortfall_usd"] = float(
            local_july_actual_shortfall[index]
        )
        july_record["composition_local_stressed_shortfall_usd"] = float(
            local_july_stressed_shortfall[index]
        )
        july_record["actual_error_l1_lipschitz_slope_usd_per_weight"] = (
            july_actual_error_slope
        )
        july_record["stressed_error_l1_lipschitz_slope_usd_per_weight"] = (
            july_stressed_error_slope
        )
    ...
    result = {
        "schema": "zeta-dd20-local-error-profit-frontier-proxy-raw-result-v1",
        ...
        "selection_search": {
            ...
            "gap_l1_lipschitz_slope_percentage_points_per_weight": selection_gap_slope,
            "candidate_local_gap_upper_min_percentage_points": float(
                np.min(local_selection_gap[:candidate_count])
            ),
            "candidate_local_gap_upper_max_percentage_points": float(
                np.max(local_selection_gap[:candidate_count])
            ),
            "uncertainty_reserve_percentage_points": selection_reserve,
            "actual_error_l1_lipschitz_slope_usd_per_weight": selection_actual_error_slope,
            "stressed_error_l1_lipschitz_slope_usd_per_weight": selection_stressed_error_slope,
            "candidate_local_actual_shortfall_min_usd": float(
                np.min(local_selection_actual_shortfall[:candidate_count])
            ),
            "candidate_local_actual_shortfall_max_usd": float(
                np.max(local_selection_actual_shortfall[:candidate_count])
            ),
            "candidate_local_stressed_shortfall_min_usd": float(
                np.min(local_selection_stressed_shortfall[:candidate_count])
            ),
            "candidate_local_stressed_shortfall_max_usd": float(
                np.max(local_selection_stressed_shortfall[:candidate_count])
            ),
        ...
        "paired_full_forward": {
            ...
            "gap_l1_lipschitz_slope_percentage_points_per_weight": paired_gap_slope,
            "candidate_local_gap_upper_min_percentage_points": float(
                np.min(local_paired_gap[:candidate_count])
            ),
            "candidate_local_gap_upper_max_percentage_points": float(
                np.max(local_paired_gap[:candidate_count])
            ),
        ...
        "development_july": {
            ...
            "actual_error_l1_lipschitz_slope_usd_per_weight": july_actual_error_slope,
            "stressed_error_l1_lipschitz_slope_usd_per_weight": july_stressed_error_slope,
            "candidate_local_actual_shortfall_min_usd": float(
                np.min(local_july_actual_shortfall[:candidate_count])
            ),
            "candidate_local_actual_shortfall_max_usd": float(
                np.max(local_july_actual_shortfall[:candidate_count])
            ),
            "candidate_local_stressed_shortfall_min_usd": float(
                np.min(local_july_stressed_shortfall[:candidate_count])
            ),
            "candidate_local_stressed_shortfall_max_usd": float(
                np.max(local_july_stressed_shortfall[:candidate_count])
            ),
        ...
        "calibration": {
            ...
            "selection_gap_l1_lipschitz_slope_percentage_points_per_weight": selection_gap_slope,
            ...
            "paired_forward_gap_l1_lipschitz_slope_percentage_points_per_weight": paired_gap_slope,
            "continuous_july_actual_error_l1_lipschitz_slope_usd_per_weight": july_actual_error_slope,
            "continuous_july_stressed_error_l1_lipschitz_slope_usd_per_weight": july_stressed_error_slope,
