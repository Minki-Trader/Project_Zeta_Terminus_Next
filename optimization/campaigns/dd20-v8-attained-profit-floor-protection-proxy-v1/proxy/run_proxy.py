...
def build_role_map(config: dict[str, Any]) -> dict[str, Any]:
    coarse = config["coarse_map"]
    activations = [float(value) for value in coarse["activation_r_coordinates"]]
    floors = [float(value) for value in coarse["floor_r_coordinates"]]
    separation = float(coarse["minimum_activation_floor_separation_r"])
    tolerance = float(config["numeric_tolerance"])
    ...
    for activation_index, activation in enumerate(activations):
        for floor_index, floor in enumerate(floors):
            ...
            coordinate_to_index[(activation_index, floor_index)] = index
    if len(roles) - 1 != int(coarse["expected_nonneutral_roles"]):
        raise RuntimeError("coarse nonneutral role count mismatch")
    if len(roles) != int(coarse["expected_total_roles"]):
        raise RuntimeError("coarse total role count mismatch")

    adjacency: list[set[int]] = [set() for _ in roles]
    edge_axes: list[dict[int, set[str]]] = [dict() for _ in roles]
    for (activation_index, floor_index), index in coordinate_to_index.items():
        for delta_a, delta_f, axis in (
            ...
            neighbor = coordinate_to_index.get(
                (activation_index + delta_a, floor_index + delta_f)
            )
            if neighbor is None:
                continue
...
def robust_points(
    ...
    minimum_neighbors = int(config["coarse_map"]["minimum_eligible_neighbors"])
    minimum_axes = int(config["coarse_map"]["minimum_neighbor_axes"])
...
def near_miss_points(
    ...
    if not bool(config["coarse_map"]["near_miss_targets_follow_active_tier"]):
...
def select_centers(
    ...
    maximum = int(config["coarse_map"]["maximum_stage_b_centers"])
    minimum_separation = int(
        config["coarse_map"]["minimum_center_axis_separation"]
    )
    ...
    for item in ranked:
        ...
        coordinate = (
            int(role["activation_coordinate_index"]),
            int(role["floor_coordinate_index"]),
        )
        if any(
            abs(coordinate[0] - prior[0]) + abs(coordinate[1] - prior[1])
            < minimum_separation
...
def structural_precheck() -> dict[str, Any]:
    ...
    return {
        ...
        "role_coordinates": [
            [role["activation_r"], role["floor_r"]] for role in role_map["roles"]
        ],
...
def main() -> None:
    ...
    if not selected:
        raise RuntimeError("Stage A failed to select mandatory Stage-B centers")

    result: dict[str, Any] = {
        "schema": "zeta-next-dd20-v8-attained-profit-floor-protection-proxy-result-v1",
        "recorded_at_utc": datetime.now(timezone.utc).isoformat().replace("+00:00", "Z"),
        "campaign": "dd20-v8-attained-profit-floor-protection-proxy-v1",
    ...
    result["status"] = (
        "VALID_PROXY_COMPLETE_STAGE_A_ROBUST_PROFIT_FLOOR_CENTERS_STAGE_B_REQUIRED_NO_MT5"
        if selection_kind == "ROBUST_ELIGIBLE"
        else "VALID_PROXY_COMPLETE_STAGE_A_NEAR_MISS_PROFIT_FLOOR_CENTERS_STAGE_B_REQUIRED_NO_MT5"
    )
        ...
        json.dumps(
            {
                ...
                "stage_b_center_count": len(result["stage_b_centers"]),
                "mt5_paths": 0,
            }
        )
    )
