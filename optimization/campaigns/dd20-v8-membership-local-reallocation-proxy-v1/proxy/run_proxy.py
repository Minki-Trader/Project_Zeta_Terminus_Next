...
def verify_authority_and_stage_a(
    config: dict[str, Any],
    ...
    expected_seeds = config["membership_centers"]
    raw_centers = payloads["stage_a_raw"].get("stage_b_membership_seeds", [])
    durable_centers = payloads["stage_a_durable"].get(
...
def build_lattice(config: dict[str, Any]) -> dict[str, Any]:
    ...
    reallocation = config["reallocation"]
    offsets = list(map(float, reallocation["offsets"]))
    lower = float(reallocation["active_lower_bound"])
    ...
    root_maps: list[dict[tuple[float, ...], int]] = []
    for root_index, center in enumerate(config["membership_centers"]):
        center_weights = list(map(float, center["weights"]))
        if len(center_weights) != component_count:
            raise RuntimeError("membership center component count mismatch")
        mask = int(center["membership_mask_integer"])
        if float(center["aggregate_risk_fraction"]) != aggregate_risk:
            raise RuntimeError("membership center aggregate cap mismatch")
        grids: list[list[float]] = []
        for axis, value in enumerate(center_weights):
            active = bool(mask & (1 << axis))
            if active != (value > 0.0):
                raise RuntimeError("membership center mask/weight mismatch")
            if not active:
                grids.append([0.0])
                continue
            grid = sorted(
                {
                    round(
                        min(max(value + offset, lower), source_weights[axis]),
                        9,
                    )
            ...
            grids.append(grid)
        local = sorted(tuple(map(float, row)) for row in itertools.product(*grids))
        expected = int(center["expected_unique_variants"])
        if len(local) != expected:
            raise RuntimeError("declared center-local reallocation count mismatch")
        local_counts.append(len(local))
        local_map: dict[tuple[float, ...], int] = {}
        for weights_tuple in local:
            if weights_tuple in local_map:
                raise RuntimeError("center-local duplicate reallocation vector")
            index = len(all_weights)
            local_map[weights_tuple] = index
            all_weights.append(weights_tuple)
            root_indices.append(root_index)
            masks.append(mask)
            provenance.append(
                {
                    "stage_a_rank": int(center["stage_a_rank"]),
                    "membership_mask_integer": mask,
                }
            )
        root_maps.append(local_map)

    expected_counts = list(map(int, reallocation["expected_center_local_unique_counts"]))
    if local_counts != expected_counts:
        raise RuntimeError("declared center-local count vector mismatch")
    if len(all_weights) != int(reallocation["expected_union_parameterizations"]):
        raise RuntimeError("declared reallocation union count mismatch")
    unique_count = len(set(all_weights))
    if len(all_weights) - unique_count != int(reallocation["expected_duplicates_removed"]):
        raise RuntimeError("declared reallocation duplicate count mismatch")
    if unique_count != len(all_weights):
        raise RuntimeError("unexpected cross-root reallocation duplicate")
    center_indices: list[int] = []
    for root_index, center in enumerate(config["membership_centers"]):
        center_key = tuple(map(float, center["weights"]))
        center_index = root_maps[root_index].get(center_key)
        if center_index is None:
            raise RuntimeError("frozen membership center absent from local lattice")
    ...
    adjacency: list[set[int]] = [set() for _ in all_weights]
    adjacency_axes: list[dict[int, int]] = [dict() for _ in all_weights]
    step = float(reallocation["fine_step"])
    for root_index, local_map in enumerate(root_maps):
        mask = int(config["membership_centers"][root_index]["membership_mask_integer"])
        for weights_tuple, index in local_map.items():
            for axis in range(component_count):
                if not mask & (1 << axis):
                    continue
                for delta in (-step, step):
                    probe = list(weights_tuple)
                    probe[axis] = round(probe[axis] + delta, 9)
                    neighbor = local_map.get(tuple(probe))
                    if neighbor is None:
                        continue
                    adjacency[index].add(neighbor)
                    adjacency_axes[index][neighbor] = axis
    if np.any(weights > source_weights[None, :] + 1.0e-12):
        raise RuntimeError("reallocation lattice violates exact-V8 weight ceiling")
    if np.any(np.sum(weights > 0.0, axis=1) == 0):
        raise RuntimeError("empty membership reached reallocation lattice")
    return {
        "values": values,
        "weights": weights,
        "masks": np.asarray(masks, dtype=np.int16),
        "root_indices": np.asarray(root_indices, dtype=np.int16),
...
def robust_membership_points(
    ...
    minimum_neighbors = int(config["gates"]["minimum_eligible_fine_neighbors"])
    minimum_axes = int(config["gates"]["minimum_active_weight_axes_spanned"])
        ...
        robust.append(
            {
                ...
                "eligible_active_weight_axis_count": len(axes),
...
def select_separated_then_fill(
    ...
    maximum = int(config["gates"]["maximum_stage_c_static_seeds"])
    separation = int(config["gates"]["minimum_membership_hamming_separation"])
        ...
        ranked.append(
            {
                ...
                "eligible_active_weight_axis_count": 0,
...
def main() -> None:
    ...
    config = load_config()
    authority_paths, stage_a_raw, stage_a_durable = verify_authority_and_stage_a(
        config
    )
    ...
    if not centers:
        raise RuntimeError("Stage B failed to select mandatory Stage-C seeds")

    result: dict[str, Any] = {
        "schema": "zeta-next-dd20-v8-membership-local-reallocation-proxy-result-v1",
        "recorded_at_utc": datetime.now(timezone.utc).isoformat().replace("+00:00", "Z"),
        "campaign": "dd20-v8-membership-local-reallocation-proxy-v1",
        ...
        "inputs": {
            ...
            "stage_a_raw": {
                "path": str(config["inputs"]["root"])
                + "/"
                + str(config["inputs"]["stage_a_raw"]["name"]),
                "bytes": authority_paths["stage_a_raw"].stat().st_size,
                "sha256": sha256(authority_paths["stage_a_raw"]),
                "status": stage_a_raw["status"],
            },
            "stage_a_durable": {
                "path": str(config["inputs"]["root"])
                + "/"
                + str(config["inputs"]["stage_a_durable"]["name"]),
                "bytes": authority_paths["stage_a_durable"].stat().st_size,
                "sha256": sha256(authority_paths["stage_a_durable"]),
                "status": stage_a_durable["status"],
            },
            "matched_lifecycles": len(lifecycles),
        },
        "reallocation_map": {
            "center_local_unique_counts": [
                int(value) for value in lattice["local_counts"]
            ],
        ...
        "limitations": [
            ...
            "Stage B uses development only. January-May validation and locked June-July remain unopened by declaration.",
        ],
    }
    ...
    for rank, (index, meta) in enumerate(zip(centers, selected_meta), start=1):
        ...
        item.update(
            {
                ...
                "membership_root_provenance": lattice["provenance"][index],
                ...
                "eligible_active_weight_axis_count": int(
                    meta["eligible_active_weight_axis_count"]
                ),
        ...
        result["stage_c_static_seeds"].append(
            {
                "development_rank": rank,
                "selection_kind": selection_kind,
                "lattice_index": index,
                ...
                "membership_root_provenance": item["membership_root_provenance"],
            }
        )

    result["status"] = (
        "VALID_PROXY_COMPLETE_STAGE_B_ROBUST_REALLOCATION_SEEDS_STAGE_C_REQUIRED_NO_MT5"
        if selection_kind == "ROBUST_ELIGIBLE"
        else "VALID_PROXY_COMPLETE_STAGE_B_NEAR_MISS_REALLOCATION_SEEDS_STAGE_C_REQUIRED_NO_MT5"
    )
        ...
        json.dumps(
            {
                ...
                "stage_c_seed_count": len(result["stage_c_static_seeds"]),
                "mt5_paths": 0,
            }
        )
    )
