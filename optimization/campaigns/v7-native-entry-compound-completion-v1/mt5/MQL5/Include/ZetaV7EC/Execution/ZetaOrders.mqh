   ...
   if(component == US100_PASSIVE_LIMIT)
   ...
   else
     {
      ...
      const double current_position_budget =
         ConservativeRiskCapital() * ECRiskFraction(component);
...
bool BuildMarketEntryPlan(const int component,
                          ...
                          MarketEntryOutcome &outcome)
  {
   ...
   const string symbol = component_definitions[component].symbol;
   if(!ECPrepare(component,direction,feature))
     { component_states[component].entry_check_result="EC_FAULT"; return(false); }
   const double volume = ECVolume(symbol);
   if(volume <= 0.0)
     {
...
bool PersistMarketEntryIntent(const MarketEntryPlan &plan,
                              MarketEntryOutcome &outcome)
  {
   if(!ECJournal(plan.component,plan.volume,plan.requested_stop_loss,plan.admitted_planned_risk)) return(false);
