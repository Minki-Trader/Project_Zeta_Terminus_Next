#define EC_ROLE 0
#define EC_ROLE_ROOT_LITERAL "ZetaV7ECControl"
#define EC_ROLE_LABEL "CONTROL"
#define EC_MAGIC_BASE 2609082300

#property strict
#property version   "7.23"
#property description "Project Zeta Terminus Next modular V7 portfolio"
#property description "Execution version: zt-next-v7-rlo1-return-portfolio-v1"

#include <Trade\Trade.mqh>

#include <ZetaV7EC\Domain\ZetaDomain.mqh>
#include <ZetaV7EC\Time\ZetaSessionClock.mqh>
#include <ZetaV7EC\Strategies\ZetaStrategyShared.mqh>
#include <ZetaV7EC\Strategies\ZetaRC16.mqh>
#include <ZetaV7EC\Strategies\ZetaRC4.mqh>
#include <ZetaV7EC\Strategies\ZetaCross.mqh>
#include <ZetaV7EC\Strategies\ZetaPressure.mqh>
#include <ZetaV7EC\Strategies\ZetaReturn.mqh>
#include <ZetaV7EC\Strategies\ZetaPassive.mqh>
#include <ZetaV7EC\Portfolio\ZetaPortfolioRisk.mqh>
#include <ZetaV7EC\Execution\ZetaOwnership.mqh>
#include <ZetaV7EC\Execution\ZetaOrders.mqh>
#include <ZetaV7EC\Execution\ZetaProtectionAndReconciliation.mqh>
#include <ZetaV7EC\Persistence\ZetaStateAndEvents.mqh>
#include <ZetaV7EC\Observation\ZetaResearchObservation.mqh>

#include <ZetaV7EC\Observation\ZetaReturnDiagnostics.mqh>
#include <ZetaV7EC\Learning\ZetaEntryCompound.mqh>
...
int OnInit()
  {
   ...
   if(!tester_mode || InpECRunTag=="unconfigured" || StringLen(InpECRunTag)<1 || StringLen(InpECRunTag)>48) return(INIT_PARAMETERS_INCORRECT);
   for(int t=0;t<StringLen(InpECRunTag);++t) {const ushort ch=StringGetCharacter(InpECRunTag,t); if(!((ch>='a'&&ch<='z')||(ch>='0'&&ch<='9')||ch=='-')) return(INIT_PARAMETERS_INCORRECT);}
   if(FileIsExist(STATE_PATH_A)||FileIsExist(STATE_PATH_B)) return(INIT_FAILED);
   ...
   FolderCreate(EC_ROLE_ROOT);
   FolderCreate(EC_ROOT);
   FolderCreate(EC_ROOT+"\\state");
   FolderCreate(EC_ROOT+"\\research");
   FolderCreate(EC_ROOT+"\\learning");
...
double OnTester()
  {
   const bool ec_complete=ECFinish();
   ...
   const bool correction_required =
      (portfolio_state.safety_stopped || persistence_failed ||
       ...
       closed_lifecycles <= 0 || !ec_complete);
