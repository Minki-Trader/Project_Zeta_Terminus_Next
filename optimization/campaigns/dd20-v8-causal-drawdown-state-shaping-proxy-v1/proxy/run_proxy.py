...
def verify_and_load_lifecycles(
    ...
    paths: dict[str, Path] = {}
    for role in ("lifecycle", "candidate", "stage_b"):
    ...
    return paths, lifecycles, stage_b_result
...
def main() -> None:
    ...
    config = load_config()
    input_paths, lifecycles, stage_b_result = verify_and_load_lifecycles(config)
    ...
    result: dict[str, Any] = {
        "schema": "zeta-next-dd20-v8-causal-drawdown-state-shaping-proxy-result-v1",
        "recorded_at_utc": datetime.now(timezone.utc).isoformat().replace("+00:00", "Z"),
        "campaign": "dd20-v8-causal-drawdown-state-shaping-proxy-v1",
    ...
    if not centers:
        ...
        if not passing_roles:
            ...
            maximum_stage_d_roles = int(
                config["stage_succession"]["maximum_stage_d_roles"]
            )
            for local_index in passing_roles[:maximum_stage_d_roles]:
                ...
                result["stage_d_roles"].append(
                    {
                        ...
                        "parameters": {
                            ...
                            "aggregate_risk_fraction": float(
                                lattice["aggregate_risk"][global_index]
                            ),
                            "static_seed_index": int(
                                lattice["seed_index"][global_index]
                            ),
                            "trigger_drawdown_fraction": float(
                                lattice["trigger_drawdown"][global_index]
                            ),
                            "future_birth_multiplier": float(
                                lattice["state_multiplier"][global_index]
                            ),
                            "release_hysteresis_fraction": float(
                                lattice["release_hysteresis"][global_index]
                            ),
                            "minimum_subsequent_accepted_closes": int(
                                lattice["minimum_hold_closes"][global_index]
                            ),
                        },
                    }
                )
