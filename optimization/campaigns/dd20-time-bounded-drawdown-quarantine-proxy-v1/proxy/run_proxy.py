...
OUTPUT_PATH = (
    ...
    / "artifacts"
    / "raw"
    / "dd20-time-bounded-drawdown-quarantine-proxy-v1"
    / "output"
    / "proxy-result.json"
)
...
def closed_path_metrics(
    ...
    for event in events:
        ...
        if index is None:
            raise RuntimeError("quarantine event is outside selection epochs")
...
def simulate_candidate(
    ...
    quarantine_birth_multiplier: float,
    minimum_quarantine_hours: int,
    rearm_drawdown: float,
    ...
    quarantine_minimum_end: datetime | None = None
    armed = True
    waiting_for_rearm = False
    completed_quarantine_exits = 0
    completed_rearms = 0
    ...
    for item in aligned:
        ...
        if (
            server_time >= activation_time
            and quarantine_active
            and quarantine_minimum_end is not None
            and server_time >= quarantine_minimum_end
            and not active_modifications
        ):
            quarantine_active = False
            waiting_for_rearm = True
            completed_quarantine_exits += 1
            active_record = quarantine_records[-1]
            active_record["exit_time_server"] = server_time.strftime(TIME_FORMAT)
            active_record["actual_quarantine_hours"] = (
                server_time
                - datetime.strptime(
                    str(active_record["entry_time_server"]), TIME_FORMAT
                )
            ).total_seconds() / 3600.0
            active_record["clean_modified_book_at_exit"] = True
            quarantine_minimum_end = None
        ...
        while birth_index < len(births):
            ...
            if trade.position_identifier in modified_ids:
                raise RuntimeError("quarantine birth was already modified")
            final_actual_delta = (
                (quarantine_birth_multiplier - 1.0) * trade.actual_net_usd
                - scaled_position_reserve
            )
            final_stressed_delta = (
                (quarantine_birth_multiplier - 1.0) * trade.stressed_net_usd
                - scaled_position_reserve
            )
            forfeited_positive_peak = -(
                (1.0 - quarantine_birth_multiplier)
            ...
            scale_by_id[trade.position_identifier] = quarantine_birth_multiplier
            ...
            scaled_birth_records.append(
                {
                    ...
                    "quarantine_multiplier": quarantine_birth_multiplier,
        ...
        if server_time < activation_time:
            continue
        if waiting_for_rearm and global_drawdown <= rearm_drawdown + 1.0e-12:
            waiting_for_rearm = False
            armed = True
            completed_rearms += 1
        if quarantine_active or not armed:
            continue
        ...
        if active_modifications:
            raise RuntimeError("quarantine trigger occurred before a clean-book exit")
        ...
        if abs(floating_actual - floating_stressed) > 1.0e-9:
            raise RuntimeError("actual/stressed floating P/L mismatch at quarantine entry")
        ...
        release_modification = Modification(
            modification_id=f"RELEASE-{len(quarantine_records) + 1}",
        ...
        for trade in affected:
            if trade.position_identifier in modified_ids:
                raise RuntimeError("quarantine entry affected an already modified lifecycle")
            modified_ids.add(trade.position_identifier)
        ...
        minimum_end = server_time + timedelta(hours=minimum_quarantine_hours)
        quarantine_records.append(
            {
                "episode": len(quarantine_records) + 1,
                "entry_time_server": server_time.strftime(TIME_FORMAT),
                "minimum_end_time_server": minimum_end.strftime(TIME_FORMAT),
                "minimum_quarantine_hours": minimum_quarantine_hours,
        ...
        quarantine_minimum_end = minimum_end
        armed = False
        waiting_for_rearm = False
    ...
    if (
        ...
        raise RuntimeError("time-bounded quarantine path delta identity mismatch")
    ...
    return {
        ...
        "quarantine_future_birth_multiplier": quarantine_birth_multiplier,
        "minimum_quarantine_hours": minimum_quarantine_hours,
        "rearm_drawdown_pct": rearm_drawdown,
        "quarantine_entries": len(quarantine_records),
        "completed_quarantine_exits": completed_quarantine_exits,
        "completed_rearms": completed_rearms,
        "final_quarantine_active": quarantine_active,
        "final_waiting_for_rearm": waiting_for_rearm,
        "released_unique_positions": sum(
            int(record["affected_open_positions"]) for record in quarantine_records
        ),
        "quarantine_scaled_births": len(scaled_birth_records),
        ...
        "first_quarantine_entry_time_server": (
            quarantine_records[0]["entry_time_server"]
            if quarantine_records
            else None
        ),
        "last_quarantine_entry_time_server": (
            quarantine_records[-1]["entry_time_server"]
            if quarantine_records
            else None
        ),
        "total_quarantine_action_reserve_usd": len(quarantine_records)
        * action_reserve,
...
def main() -> None:
    ...
    if (
        ...
        raise RuntimeError("time-bounded quarantine activation anchor mismatch")
    ...
    quarantine = config["time_bounded_drawdown_quarantine"]
    triggers = [
        float(value) for value in quarantine["candidate_drawdown_triggers_pct"]
    ]
    retained_fractions = [
        float(value)
    ...
    birth_multipliers = [
        float(value)
        for value in quarantine["candidate_quarantine_birth_multipliers"]
    ]
    quarantine_hours = [
        int(value)
        for value in quarantine["candidate_minimum_quarantine_hours"]
    ]
    if (
        len(triggers)
        * len(retained_fractions)
        * len(birth_multipliers)
        * len(quarantine_hours)
        != int(quarantine["expected_candidates"])
        ...
        or len(set(quarantine_hours)) != len(quarantine_hours)
        ...
        or any(value <= 0 for value in quarantine_hours)
    ):
        raise RuntimeError("time-bounded quarantine grid is invalid")
    ...
    rearm_drawdown = float(quarantine["rearm_drawdown_pct"])
    action_reserve = float(
        quarantine["release_action_and_proxy_reserve_usd_per_episode"]
    )
    scaled_position_reserve = float(
        quarantine["per_scaled_position_modeling_reserve_usd"]
    )
    global_profit_reserve = float(
        quarantine["selection_profit_uncertainty_reserve_usd"]
    )
    ...
    quarantine_dd_reserve = float(
        dd_config[
            "quarantine_state_transition_and_execution_reserve_percentage_points"
        ]
    )
    ...
    for trigger in triggers:
        for retained in retained_fractions:
            for multiplier in birth_multipliers:
                for duration_hours in quarantine_hours:
                    ...
                    capital_gate = (
                        ...
                        and float(record["minimum_stressed_closed_balance_usd"])
                        > 0.0
                        and float(record["minimum_actual_sampled_equity_usd"]) > 0.0
                        and float(record["minimum_stressed_sampled_equity_usd"])
                        > 0.0
                    )
                    epoch_gate = all(
                        value > 0.0 for value in record["epoch_actual"]
                    ) and all(value > 0.0 for value in record["epoch_stressed"])
                    ...
                    record["forward_invariant"] = {
                        "time_bounded_drawdown_quarantine_armed": False,
    ...
    result = {
        "schema": "zeta-dd20-time-bounded-drawdown-quarantine-proxy-raw-result-v1",
        ...
        "implementation": {
            ...
            "immutable_global_high_no_reset_gate": "PASS",
            "minimum_quarantine_duration_gate": "PASS",
            "clean_modified_book_exit_gate": "PASS",
            "full_exposure_resume_gate": "PASS",
            "post_exit_rearm_gate": "PASS",
        ...
        "search": {
            ...
            "quarantine_future_birth_multipliers": birth_multipliers,
            "minimum_quarantine_hours": quarantine_hours,
            "rearm_drawdown_pct": rearm_drawdown,
            "quarantine_entry_count_range": [
                min(int(record["quarantine_entries"]) for record in records),
                max(int(record["quarantine_entries"]) for record in records),
            ],
            "completed_quarantine_exit_count_range": [
                min(
                    int(record["completed_quarantine_exits"])
            ...
            "quarantine_scaled_birth_count_range": [
                min(
                    int(record["quarantine_scaled_births"])
                    for record in records
                ),
                max(
                    int(record["quarantine_scaled_births"])
            ...
            "all_candidates_enter_quarantine": all(
                int(record["quarantine_entries"]) > 0 for record in records
            ),
            "forward_time_bounded_quarantine_armed": False,
        ...
        "calibration": {
            ...
            "quarantine_state_transition_and_execution_reserve_percentage_points": quarantine_dd_reserve,
            ...
            [
                {
                    ...
                    "quarantine_future_birth_multiplier": winner[
                        "quarantine_future_birth_multiplier"
                    ],
                    "minimum_quarantine_hours": winner[
                        "minimum_quarantine_hours"
                    ],
                    ...
                    "quarantine_entries": winner["quarantine_entries"],
                    "quarantine_scaled_births": winner[
                        "quarantine_scaled_births"
                    ],
        ...
        "boundary": {
            ...
            "time_bounded_drawdown_quarantine": True,
            "global_high_reset": False,
            "full_exposure_resumes_after_clean_duration_exit": True,
            "later_original_births_preserved_at_scaled_exposure": True,
            "freed_capacity_synthetic_births": False,
            "persistent_global_regime_grid_rerun": False,
