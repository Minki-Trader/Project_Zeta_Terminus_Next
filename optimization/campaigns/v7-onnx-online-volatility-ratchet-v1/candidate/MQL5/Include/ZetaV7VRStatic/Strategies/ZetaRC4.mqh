...
void ProcessArcModifyRetry()
  {
   ...
   const bool legal =
      (MathAbs(direction) == 1 && entry > 0.0 && executable_tick &&
       ...
       (double)direction * (target_stop - broker_stop) >= tick_size - price_tolerance &&
...
void ProcessRC4AdverseRiskCompression()
  {
   ...
   if((double)direction * (new_stop - PositionGetDouble(POSITION_SL)) < tick_size - tolerance ||
      retained_loss < tick_size - tolerance ||
