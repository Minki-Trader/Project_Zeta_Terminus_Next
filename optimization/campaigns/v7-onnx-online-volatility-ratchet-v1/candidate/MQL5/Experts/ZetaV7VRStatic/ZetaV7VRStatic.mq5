...
#property description "Execution version: zt-opt-v7-volatility-ratchet-candidate-v1"

#include <Trade\Trade.mqh>

#include <ZetaV7VRStatic\Domain\ZetaDomain.mqh>
#include <ZetaV7VRStatic\Learning\ZetaVolatilityTypes.mqh>
#include <ZetaV7VRStatic\Time\ZetaSessionClock.mqh>
#include <ZetaV7VRStatic\Strategies\ZetaStrategyShared.mqh>
#include <ZetaV7VRStatic\Strategies\ZetaRC16.mqh>
#include <ZetaV7VRStatic\Strategies\ZetaRC4.mqh>
#include <ZetaV7VRStatic\Strategies\ZetaCross.mqh>
#include <ZetaV7VRStatic\Strategies\ZetaPressure.mqh>
#include <ZetaV7VRStatic\Strategies\ZetaReturn.mqh>
#include <ZetaV7VRStatic\Strategies\ZetaPassive.mqh>
#include <ZetaV7VRStatic\Portfolio\ZetaPortfolioRisk.mqh>
#include <ZetaV7VRStatic\Execution\ZetaOwnership.mqh>
#include <ZetaV7VRStatic\Execution\ZetaOrders.mqh>
#include <ZetaV7VRStatic\Execution\ZetaProtectionAndReconciliation.mqh>
#include <ZetaV7VRStatic\Persistence\ZetaStateAndEvents.mqh>
#include <ZetaV7VRStatic\Observation\ZetaResearchObservation.mqh>

#include <ZetaV7VRStatic\Observation\ZetaReturnDiagnostics.mqh>
#include <ZetaV7VRStatic\Observation\ZetaEconomicPath.mqh>
#include <ZetaV7VRStatic\Learning\ZetaVolatilityRatchet.mqh>
...
int OnInit()
  {
   ...
   if(!tester_mode) return(INIT_FAILED);
   ...
   FolderCreate("ZetaV7VRStatic");
   FolderCreate("ZetaV7VRStatic\\optimization");
   FolderCreate("ZetaV7VRStatic\\optimization\\volatility-ratchet");
   FolderCreate("ZetaV7VRStatic\\optimization\\volatility-ratchet\\state");
   FolderCreate("ZetaV7VRStatic\\optimization\\volatility-ratchet\\research");
   if(tester_mode)
      ResetTesterArtifacts();
   if(!AcquireRuntimeOwnership())
      return(INIT_FAILED);
   ResetRuntimeState();
   if(!InitializeEconomicPath()) { ReleaseRuntimeOwnership(); return(INIT_FAILED); }
   if(!InitializeVRModel())
     { CloseVRModel(); ReleaseRuntimeOwnership(); return(INIT_FAILED); }
   ...
   if(tester_mode)
     {
      ...
      const bool transaction_gate =
         (execution_state.pending_reconcile || execution_state.passive_cancel_pending ||
          execution_state.arc_modify_retry_pending || VRHasPending());
...
double OnTester()
  {
   CompleteEconomicPath();
   PrintFormat("V7VR_MODEL forecasts=%I64d missing_prefixes=%I64d decisions=%I64d intents=%I64d adoptions=%I64d refusals=%I64d errors=%I64d pending=%d", vr_forecasts, vr_missing_prefixes, vr_decisions, vr_modify_intents, vr_modify_adoptions, vr_modify_refusals, vr_errors, (VRHasPending() ? 1 : 0));
   PrintFormat("V7RR1_NATIVE profit=%.6f equity_dd=%.6f equity_dd_relative_pct=%.9f balance_dd_relative_pct=%.9f trades=%.0f",
      TesterStatistics(STAT_PROFIT), TesterStatistics(STAT_EQUITY_DD),
      TesterStatistics(STAT_EQUITY_DDREL_PERCENT), TesterStatistics(STAT_BALANCE_DDREL_PERCENT),
      TesterStatistics(STAT_TRADES));
   PrintFormat("%s V7RR1_UNAVAILABLE_SUMMARY aggregate=%I64d rows=%I64d "
               "complete_zero_range=%I64d complete_zero_variance=%I64d "
               "session_boundary=%I64d invalid_direction=%I64d "
               "short_copy=%I64d invalid_price=%I64d invalid_tick=%I64d "
               "nonfinite=%I64d unclassified=%I64d",
               EXECUTION_VERSION,
   ...
   const long unavailability_reason_total =
      ...
      arc_unavailability_nonfinite +
      arc_unavailability_unclassified;
   const bool invalid_unavailability =
      (arc_data_unavailable != arc_unavailability_rows ||
       arc_unavailability_rows != unavailability_reason_total ||
   ...
   const bool correction_required =
      (economic_path_failed || vr_errors != 0 || VRHasPending() || vr_forecasts <= 0 ||
       ...
       research_dropped_records != 0 || invalid_unavailability ||
       closed_lifecycles <= 0);
   if(correction_required)
     {
      PrintFormat("V7RR1_RESULT status=CORRECTION_REQUIRED "
                  ...
                  "research_dropped=%I64d unavailable_invalid=%d "
                  "unavailable_aggregate=%I64d unavailable_rows=%I64d "
                  "unavailable_reasons=%I64d final_server=%s",
                  ...
                  (invalid_unavailability ? 1 : 0),
                  arc_data_unavailable,
                  arc_unavailability_rows,
                  unavailability_reason_total,
   ...
   PrintFormat("V7RR1_RESULT status=ECONOMIC position_risk=%.6f "
