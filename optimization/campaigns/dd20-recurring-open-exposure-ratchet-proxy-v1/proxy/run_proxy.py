...
def activation_from_closes(
    ...
    raise RuntimeError("selection never reaches governor arming threshold")
...
def active_positions(
    closes: list[CloseTrade], item: AlignedSnapshot, affected_ids: set[str]
) -> list[CloseTrade]:
    ...
    return [
        ...
        and trade.position_identifier not in affected_ids
...
def closed_path_metrics(
    closes: list[CloseTrade],
    retained_fraction_by_id: dict[str, float],
    ...
    for trade in closes:
        scale = retained_fraction_by_id.get(trade.position_identifier, 1.0)
        events.append(
            NetEvent(
    ...
    for event in events:
        ...
        if index is None:
            raise RuntimeError("ratchet economic event is outside selection epochs")
...
def simulate_candidate(
    ...
    released_fraction = 1.0 - retained_fraction
    settled_actual_shift = 0.0
    settled_stressed_shift = 0.0
    pending: dict[str, Any] | None = None
    local_stressed_peak: float | None = None
    ...
    for item in aligned:
        server_time = item.snapshot.server_time
        if pending is not None and server_time > pending["last_affected_close"]:
            settled_actual_shift += float(pending["final_actual_delta"])
            settled_stressed_shift += float(pending["final_stressed_delta"])
            pending = None
            local_stressed_peak = None
            rearm_count += 1

        if pending is not None:
            actual_shift = settled_actual_shift + float(pending["transition_actual_delta"])
            stressed_shift = settled_stressed_shift + float(
                pending["transition_stressed_delta"]
            )
            candidate_actual_equity.append(item.snapshot.account_equity + actual_shift)
            candidate_stressed_equity.append(item.stressed_equity + stressed_shift)
            continue

        current_actual_equity = item.snapshot.account_equity + settled_actual_shift
        current_stressed_equity = item.stressed_equity + settled_stressed_shift
        ...
        if server_time < activation_time:
            continue
        if local_stressed_peak is None:
            local_stressed_peak = current_stressed_equity
        else:
            local_stressed_peak = max(local_stressed_peak, current_stressed_equity)
        local_drawdown = 100.0 * (
            local_stressed_peak - current_stressed_equity
        ) / local_stressed_peak
        if local_drawdown + 1.0e-12 < trigger:
            continue

        affected = active_positions(closes, item, affected_ids)
        if not affected:
        ...
        if abs(floating_actual - floating_stressed) > 1.0e-9:
            raise RuntimeError("actual/stressed floating P/L mismatch at recurring release")
        ...
        release_actual = released_fraction * floating_actual - action_reserve
        release_stressed = (
            released_fraction * floating_stressed
            - released_fraction * affected_drag
            - action_reserve
        )
        final_actual_delta = release_actual - released_fraction * affected_actual
        final_stressed_delta = release_stressed - released_fraction * affected_stressed
        transition_actual_delta = min(-action_reserve, final_actual_delta)
        transition_stressed_delta = min(-action_reserve, final_stressed_delta)
        last_affected_close = max(trade.close_time for trade in affected)
        for trade in affected:
            if trade.position_identifier in retained_fraction_by_id:
                raise RuntimeError("a lifecycle was affected by more than one episode")
            affected_ids.add(trade.position_identifier)
            retained_fraction_by_id[trade.position_identifier] = retained_fraction
        ...
        actions.append(
            {
                "episode": len(actions) + 1,
                "release_time_server": server_time.strftime(TIME_FORMAT),
                "release_stage": item.snapshot.stage,
                "release_result": item.snapshot.result,
                "local_stressed_equity_high_usd": local_stressed_peak,
                "local_stressed_equity_drawdown_pct": local_drawdown,
                "release_account_balance_usd": item.snapshot.account_balance
                + settled_actual_shift,
                "release_account_equity_usd": current_actual_equity,
                "release_stressed_balance_usd": item.stressed_balance
                + settled_stressed_shift,
                "release_stressed_equity_usd": current_stressed_equity,
                ...
                "final_actual_delta_usd": final_actual_delta,
                "final_stressed_delta_usd": final_stressed_delta,
        ...
        pending = {
            "last_affected_close": last_affected_close,
        ...
        candidate_actual_equity[-1] = (
            ...
            + settled_actual_shift
            + transition_actual_delta
        )
        candidate_stressed_equity[-1] = (
            item.stressed_equity
            + settled_stressed_shift
            + transition_stressed_delta
        )
        local_stressed_peak = None

    if not actions:
        raise RuntimeError("declared recurring candidate never releases open exposure")
    metrics = closed_path_metrics(
        closes,
        retained_fraction_by_id,
        action_events,
        reference,
        epochs,
    )
    cumulative_actual_delta = sum(float(item["final_actual_delta_usd"]) for item in actions)
    cumulative_stressed_delta = sum(
        float(item["final_stressed_delta_usd"]) for item in actions
    )
    ...
    if (
        abs(metrics["actual_net_usd"] - baseline_actual - cumulative_actual_delta)
        > 1.0e-7
        or abs(
            metrics["stressed_net_usd"]
            - baseline_stressed
            - cumulative_stressed_delta
        )
        > 1.0e-7
    ):
        raise RuntimeError("recurring release path delta identity mismatch")
    ...
    return {
        ...
        "retained_open_exposure_fraction": retained_fraction,
        "released_open_exposure_fraction": released_fraction,
        "release_episodes": len(actions),
        "completed_cooldown_rearms": rearm_count,
        "affected_unique_positions": len(affected_ids),
        "zero_exposure_trigger_observations": zero_exposure_trigger_observations,
        "first_release_time_server": actions[0]["release_time_server"],
        "last_release_time_server": actions[-1]["release_time_server"],
        "total_action_reserve_usd": len(actions) * action_reserve,
        "cumulative_actual_delta_usd": cumulative_actual_delta,
        "cumulative_stressed_delta_usd": cumulative_stressed_delta,
...
def main() -> None:
    ...
    if (
        ...
        raise RuntimeError("governor activation anchor mismatch")
    ...
    governor = config["recurring_open_exposure_ratchet"]
    triggers = [float(value) for value in governor["candidate_drawdown_triggers_pct"]]
    retained_fractions = [
        float(value)
        for value in governor["candidate_retained_open_exposure_fractions"]
    ]
    if (
        len(triggers) * len(retained_fractions)
        != int(governor["expected_candidates"])
        ...
        raise RuntimeError("recurring governor grid is invalid")
    ...
    action_reserve = float(
        governor["release_action_and_proxy_reserve_usd_per_episode"]
    )
    global_profit_reserve = float(
        governor["selection_profit_uncertainty_reserve_usd"]
    )
    ...
    recurring_dd_reserve = float(
        dd_config[
            "recurring_state_transition_and_execution_reserve_percentage_points"
        ]
    )
    ...
    for trigger in triggers:
        for retained in retained_fractions:
            ...
            budgeted_dd = max(
                ...
                + native_gap
                + recurring_dd_reserve,
            )
            conservative_actual = float(record["actual_net_usd"]) - global_profit_reserve
            ...
            record["forward_invariant"] = {
                "recurring_governor_armed": False,
    ...
    result = {
        "schema": "zeta-dd20-recurring-open-exposure-ratchet-proxy-raw-result-v1",
        ...
        "implementation": {
            ...
            "candidate_action_delta_identity_gate": "PASS",
            "lifecycle_clear_rearm_gate": "PASS",
        ...
        "search": {
            ...
            "release_episode_count_range": [
                min(int(record["release_episodes"]) for record in records),
                max(int(record["release_episodes"]) for record in records),
            ],
            "affected_unique_position_count_range": [
                min(int(record["affected_unique_positions"]) for record in records),
                max(int(record["affected_unique_positions"]) for record in records),
            ],
            "all_candidates_release_open_exposure": all(
                int(record["release_episodes"]) > 0 for record in records
            ),
            "forward_recurring_governor_armed": False,
        ...
        "calibration": {
            ...
            "recurring_state_transition_and_execution_reserve_percentage_points": recurring_dd_reserve,
            ...
            [
                {
                    ...
                    "release_episodes": winner["release_episodes"],
        ...
        "boundary": {
            ...
            "recurring_lifecycle_clear_open_exposure_ratchet": True,
            "later_original_births_enter_at_full_exposure": True,
