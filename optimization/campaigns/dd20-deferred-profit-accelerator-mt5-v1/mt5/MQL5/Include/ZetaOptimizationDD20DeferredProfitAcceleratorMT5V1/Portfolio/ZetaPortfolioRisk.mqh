...
void UpdateSizingDay()
  {
   ...
   const double baseline_growth = MathMin(growth, InpActivationGrowthUSD);
   const double accelerated_growth =
      MathMax(0.0, growth - InpActivationGrowthUSD);
   portfolio_state.day_volume_multiplier =
      1 + (int)MathFloor(baseline_growth / InpAdditionStepUSD + 1.0e-9) +
      (int)MathFloor(accelerated_growth / InpPostActivationStepUSD + 1.0e-9);
   portfolio_state.day_volume_multiplier = MathMax(1, portfolio_state.day_volume_multiplier);
   portfolio_state.day_volume_multiplier =
      MathMin(InpMaximumDayVolumeMultiplier,
              portfolio_state.day_volume_multiplier);
...
double ComponentRiskMultiplier(const int component)
  {
   if(component == RC16_LONG)
      return(InpRC16RiskMultiplier);
   if(component == RC4_BOTH)
      return(InpRC4RiskMultiplier);
   if(component == US100_CROSS)
      return(InpUS100CrossRiskMultiplier);
   if(component == US30_PRESSURE)
      return(InpUS30PressureRiskMultiplier);
   if(component == US30_RETURN_REV_LONG)
      return(InpUS30ReturnRiskMultiplier);
   if(component == US100_PASSIVE_LIMIT)
      return(InpUS100PassiveRiskMultiplier);
   return(-1.0);
  }
   ...
   if(risk_multiplier <= 1.0e-9)
     {
      ...
      RecordEvent(component,
                  ...
                  risk_multiplier,
                  0.0,
                  "frozen proxy composition multiplier is zero");
      return(false);
     }
