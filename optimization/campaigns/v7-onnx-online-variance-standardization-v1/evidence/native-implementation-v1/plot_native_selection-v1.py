...
def load_equity_path(path: Path) -> dict[str, object]:
    ...
    with path.open("r", encoding=text_encoding(path), errors="replace", newline="") as stream:
        ...
        if not required.issubset(header):
            fail(f"{rel(path)} lacks the authored account-path columns")
        for raw in reader:
            raw_rows += 1
            epoch = integer(raw.get("server_time_epoch", ""), f"{rel(path)} row {raw_rows} epoch")
            if previous_epoch is not None and epoch < previous_epoch:
                fail(f"{rel(path)} native epochs are not nondecreasing")
            equity = finite_float(raw.get("actual_equity", ""), f"{rel(path)} row {raw_rows} equity")
            mark_known = raw.get("conservative_mark_known", "").strip() == "1"
            if mark_known:
                mark = finite_float(
                    raw.get("conservative_stressed_mark", ""),
                    f"{rel(path)} row {raw_rows} conservative mark",
                )
            else:
                mark = math.nan
                unknown_mark_rows += 1
    ...
    if raw_rows == 0 or last_observation is None:
        fail(f"{rel(path)} contains no account observations")
...
def load_daily_birth_volume(path: Path) -> dict[str, object]:
    ...
    with path.open("r", encoding=text_encoding(path), errors="replace", newline="") as stream:
        ...
        header = clean_header(reader)
        if not required.issubset(header):
            fail(f"{rel(path)} lacks the authored lifecycle columns")
        for raw in reader:
            if raw.get("event", "").strip() != "BIRTH":
                continue
            birth_rows += 1
            observed = parse_server_datetime(
                raw.get("server_time", "").strip(), f"{rel(path)} BIRTH row {birth_rows}"
            )
            try:
                volume = Decimal(raw.get("volume", "").strip())
            except InvalidOperation:
                fail(f"{rel(path)} BIRTH row {birth_rows} has unreadable volume")
            if not volume.is_finite() or volume < 0:
                fail(f"{rel(path)} BIRTH row {birth_rows} has invalid volume")
            day = observed.date()
