...
FALLBACK_PATTERN = re.compile(
    r"(?:(?:tick|history).{0,80}\bfallback\b|\bfallback\b.{0,80}(?:tick|history)|using\s+generated\s+ticks?|generated\s+ticks?\s+(?:used|substitut)|"
    r"ticks?\s+(?:were\s+)?generated\s+(?:instead|from)|real\s+ticks?.{0,80}(?:unavailable|missing|absent|discarded|not\s+found)|"
...
def log_report(paths: list[Path]) -> dict[str, object]:
    markers: dict[str, tuple[str, Callable[[str], bool]]] = {
        "v7rr1_contract": ("V7RR1_CONTRACT ", lambda line: "V7RR1_CONTRACT " in line),
