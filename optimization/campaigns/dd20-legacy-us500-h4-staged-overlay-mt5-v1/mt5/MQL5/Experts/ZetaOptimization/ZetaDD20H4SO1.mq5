...
#property description "Project Zeta Terminus Next isolated DD20 legacy US500 H4 staged-overlay MT5 candidate"
#property description "Execution version: zt-opt-live-v7-dd20-legacy-us500-h4-staged-overlay-mt5-v1"

#include <Trade\Trade.mqh>

#include <ZetaOptDD20H4SO1\Domain\ZetaDomain.mqh>
#include <ZetaOptDD20H4SO1\Time\ZetaSessionClock.mqh>
#include <ZetaOptDD20H4SO1\Strategies\ZetaStrategyShared.mqh>
#include <ZetaOptDD20H4SO1\Strategies\ZetaRC16.mqh>
#include <ZetaOptDD20H4SO1\Strategies\ZetaRC4.mqh>
#include <ZetaOptDD20H4SO1\Strategies\ZetaCross.mqh>
#include <ZetaOptDD20H4SO1\Strategies\ZetaPressure.mqh>
#include <ZetaOptDD20H4SO1\Strategies\ZetaReturn.mqh>
#include <ZetaOptDD20H4SO1\Strategies\ZetaPassive.mqh>
#include <ZetaOptDD20H4SO1\Portfolio\ZetaPortfolioRisk.mqh>
#include <ZetaOptDD20H4SO1\Execution\ZetaOwnership.mqh>
#include <ZetaOptDD20H4SO1\Execution\ZetaOrders.mqh>
#include <ZetaOptDD20H4SO1\Execution\ZetaProtectionAndReconciliation.mqh>
#include <ZetaOptDD20H4SO1\Persistence\ZetaStateAndEvents.mqh>
#include <ZetaOptDD20H4SO1\Observation\ZetaResearchObservation.mqh>
#include <ZetaOptDD20H4SO1\Strategies\ZetaUS500H4StagedOverlay.mqh>
...
int OnInit()
  {
   ...
   if(!tester_mode || InpAllowNewEntries ||
       _Symbol != "US30" || _Period != PERIOD_M30 ||
       ...
       MathAbs(InpRC16BaseWeight - 1.6) > 1.0e-9 ||
       MathAbs(InpRC4BaseWeight - 0.8) > 1.0e-9 ||
       MathAbs(InpUS100CrossBaseWeight - 0.4) > 1.0e-9 ||
       MathAbs(InpUS30PressureBaseWeight - 3.2) > 1.0e-9 ||
       MathAbs(InpUS30ReturnBaseWeight - 1.2) > 1.0e-9 ||
       MathAbs(InpUS100PassiveBaseWeight) > 1.0e-9 ||
       InpComponentStateLookback != 40 ||
       MathAbs(InpComponentStateMeanBandR - 0.10) > 1.0e-9 ||
       MathAbs(InpComponentStateLossOverlay) > 1.0e-9 ||
       MathAbs(InpComponentStateGainOverlay - 1.125) > 1.0e-9 ||
    ...
    ResetComponentEquityState();
    ResetH4OverlayState();
   FolderCreate("ZetaOptimization");
   FolderCreate("ZetaOptimization\\dd20-h4so1");
   FolderCreate("ZetaOptimization\\dd20-h4so1\\state");
   FolderCreate("ZetaOptimization\\dd20-h4so1\\research");
...
void OnTradeTransaction(const MqlTradeTransaction &transaction,
  ...
  {
   ObserveH4OverlayTradeTransaction(transaction);
...
double OnTester()
  {
   const bool h4_pending_exit_applied = ApplyPendingH4OverlayExit();
   ulong h4_open_ticket = 0;
   datetime h4_opened_at = 0;
   const int h4_open_positions =
      CountH4OverlayPositions(h4_open_ticket, h4_opened_at);
   long closed_lifecycles = 0;
   bool state_counts_match = true;
   for(int component = 0; component < COMPONENT_COUNT; ++component)
     {
      closed_lifecycles += component_states[component].closed_trades;
      if(component_equity_state_full_closes[component] !=
         component_states[component].closed_trades)
         state_counts_match = false;
      PrintFormat("COMPONENT_EQUITY_STATE component=%s base_weight=%.3f "
                  "lookback=%d mean_band_r=%.3f loss_overlay=%.3f "
                  "gain_overlay=%.3f full_closes=%I64d deque_count=%d "
                  "mean_r=%.9f next_overlay=%.3f partial_exits=%I64d "
                  "volume_evaluations=%I64d loss_evaluations=%I64d "
                  "neutral_evaluations=%I64d gain_evaluations=%I64d",
                  component_definitions[component].id,
                  ComponentBaseWeight(component),
                  InpComponentStateLookback,
                  InpComponentStateMeanBandR,
                  InpComponentStateLossOverlay,
                  InpComponentStateGainOverlay,
                  component_equity_state_full_closes[component],
                  component_equity_state_count[component],
                  ComponentEquityStateMeanR(component),
                  ComponentEquityStateOverlay(component),
   ...
   const bool correction_required =
      (portfolio_state.safety_stopped || persistence_failed ||
       ...
       research_dropped_records != 0 || component_equity_state_faults != 0 ||
       !h4_pending_exit_applied || h4_overlay_pending_exit_deal != 0 ||
       h4_overlay_faults != 0 || h4_overlay_entry_failures != 0 ||
       h4_overlay_exit_failures != 0 ||
       h4_overlay_partial_or_placed_outcomes != 0 ||
       h4_overlay_completed_entries != h4_overlay_completed_exits ||
       h4_open_positions != 0 || !state_counts_match ||
       closed_lifecycles <= 0);
   if(correction_required)
     {
      PrintFormat("OPTIMIZATION_RESULT status=CORRECTION_REQUIRED "
                  "position_risk=%.6f aggregate_risk=%.6f "
                  "base_weights=%.3f/%.3f/%.3f/%.3f/%.3f/%.3f "
                  "state=lookback%d-band%.3f-loss%.3f-gain%.3f "
                  "closed=%I64d state_faults=%I64d state_counts_match=%d "
                  "safety=%d persistence=%d broker=%d foreign=%d "
                  "protection_calc=%I64d protection_mismatch=%I64d "
                   "research_dropped=%I64d h4_entries=%I64d h4_exits=%I64d "
                   "h4_open=%d h4_pending=%I64u h4_faults=%I64d "
                   "h4_entry_failures=%I64d h4_exit_failures=%I64d "
                   "h4_partial_or_placed=%I64d final_server=%s",
                  InpMaximumPositionRiskFraction,
                  InpMaximumAggregateRiskFraction,
                  InpRC16BaseWeight,
                  InpRC4BaseWeight,
                  InpUS100CrossBaseWeight,
                  InpUS30PressureBaseWeight,
                  InpUS30ReturnBaseWeight,
                  InpUS100PassiveBaseWeight,
                  InpComponentStateLookback,
                  InpComponentStateMeanBandR,
                  InpComponentStateLossOverlay,
                  InpComponentStateGainOverlay,
   ...
   PrintFormat("OPTIMIZATION_RESULT status=ECONOMIC "
               "position_risk=%.6f aggregate_risk=%.6f "
               "base_weights=%.3f/%.3f/%.3f/%.3f/%.3f/%.3f "
               "state=lookback%d-band%.3f-loss%.3f-gain%.3f "
               "actual_net=%.6f stressed_net=%.6f actual_dd=%.6f "
               "stressed_dd=%.6f robust_net=%.6f robust_recovery=%.9f "
               "closed=%I64d state_faults=%I64d risk_skips=%I64d "
                "stop_exits=%I64d h4_entries=%I64d h4_exits=%I64d "
                "h4_actual=%.6f h4_stressed=%.6f h4_margin_skips=%I64d "
                "h4_stage_blocks=%I64d h4_state_unavailable=%I64d "
                "h4_late_windows=%I64d final_server=%s",
               InpMaximumPositionRiskFraction,
               InpMaximumAggregateRiskFraction,
               InpRC16BaseWeight,
               InpRC4BaseWeight,
...
void OnDeinit(const int reason)
  {
   ...
   for(int component = 0; component < COMPONENT_COUNT; ++component)
    ...
    const int h4_open_positions =
       CountH4OverlayPositions(h4_open_ticket, h4_opened_at);
    PrintFormat("%s H4_OVERLAY final candidate=%s magic=%I64u "
                "entries=%I64d exits=%I64d open=%d actual=%.6f "
                "stressed=%.6f margin_skips=%I64d stage_blocks=%I64d "
                "state_unavailable=%I64d late_windows=%I64d "
                "entry_failures=%I64d exit_failures=%I64d "
                "partial_or_placed=%I64d faults=%I64d pending=%I64u",
                EXECUTION_VERSION,
