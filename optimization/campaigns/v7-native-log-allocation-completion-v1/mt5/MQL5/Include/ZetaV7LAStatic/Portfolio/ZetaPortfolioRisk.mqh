...
bool NewEntriesOperationallyAllowed()
  {
   return(LACReady() && NewEntriesAuthorized() &&
...
double NormalizedVolume(const string symbol,const int component)
  {
   ...
   if(step <= 0.0)
      return(0.0);
   const double requested = InpBaseVolume * portfolio_state.day_volume_multiplier * LACWeight(component);
...
bool CalculateProtectiveStop(const int component,
                             ...
                             double &planned_risk)
  {
   ...
   const double position_budget =
      capital * InpMaximumPositionRiskFraction * LACWeight(component);
