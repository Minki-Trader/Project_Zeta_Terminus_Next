...
#property description "Project Zeta Terminus Next isolated paired clean-history requalification V2"
#property description "Execution version: zt-opt-live-v8-dd20-paired-clean-requal-mt5-v2"

#include <Trade\Trade.mqh>

#include <ZetaOptimizationDD20PairedCleanRequalMT5V2\Domain\ZetaDomain.mqh>
#include <ZetaOptimizationDD20PairedCleanRequalMT5V2\Time\ZetaSessionClock.mqh>
#include <ZetaOptimizationDD20PairedCleanRequalMT5V2\Strategies\ZetaStrategyShared.mqh>
#include <ZetaOptimizationDD20PairedCleanRequalMT5V2\Strategies\ZetaRC16.mqh>
#include <ZetaOptimizationDD20PairedCleanRequalMT5V2\Strategies\ZetaRC4.mqh>
#include <ZetaOptimizationDD20PairedCleanRequalMT5V2\Strategies\ZetaCross.mqh>
#include <ZetaOptimizationDD20PairedCleanRequalMT5V2\Strategies\ZetaPressure.mqh>
#include <ZetaOptimizationDD20PairedCleanRequalMT5V2\Strategies\ZetaReturn.mqh>
#include <ZetaOptimizationDD20PairedCleanRequalMT5V2\Strategies\ZetaPassive.mqh>
#include <ZetaOptimizationDD20PairedCleanRequalMT5V2\Portfolio\ZetaPortfolioRisk.mqh>
#include <ZetaOptimizationDD20PairedCleanRequalMT5V2\Execution\ZetaOwnership.mqh>
#include <ZetaOptimizationDD20PairedCleanRequalMT5V2\Execution\ZetaOrders.mqh>
#include <ZetaOptimizationDD20PairedCleanRequalMT5V2\Execution\ZetaProtectionAndReconciliation.mqh>
#include <ZetaOptimizationDD20PairedCleanRequalMT5V2\Persistence\ZetaStateAndEvents.mqh>
#include <ZetaOptimizationDD20PairedCleanRequalMT5V2\Observation\ZetaResearchObservation.mqh>

// The EA owns assembly and the inherited event ordering only.

void PrintPCR2RequiredSymbolContract(const string stage,
                                     const string symbol)
  {
   PrintFormat("%s PCR2_CONTRACT stage=%s symbol=%s "
...
void PrintPCR2RequiredContracts(const string stage)
  {
   PrintPCR2RequiredSymbolContract(stage, "US30");
   PrintPCR2RequiredSymbolContract(stage, "US100");
   PrintPCR2RequiredSymbolContract(stage, "US500");
  }
...
bool InitializeConnectedRuntime()
  {
   ...
   PrintFormat("%s initialized portfolio=%s recovered=%s "
               ...
               state_sequence);
   PrintPCR2RequiredContracts("START");
   return(true);
  }
...
int OnInit()
  {
   ...
   const bool control_tuple =
      (MathAbs(InpRC16RiskMultiplier - 1.0) <= 1.0e-9 &&
       MathAbs(InpRC4RiskMultiplier - 1.0) <= 1.0e-9 &&
       MathAbs(InpUS100CrossRiskMultiplier - 1.0) <= 1.0e-9 &&
       MathAbs(InpUS30PressureRiskMultiplier - 1.0) <= 1.0e-9 &&
       MathAbs(InpUS30ReturnRiskMultiplier - 1.0) <= 1.0e-9 &&
       MathAbs(InpUS100PassiveRiskMultiplier - 1.0) <= 1.0e-9);
   const bool candidate_tuple =
   ...
   FolderCreate("ZetaTerminusNext\\optimization");
   FolderCreate("ZetaTerminusNext\\optimization\\dd20-pcr2");
   FolderCreate("ZetaTerminusNext\\optimization\\dd20-pcr2\\state");
   FolderCreate("ZetaTerminusNext\\optimization\\dd20-pcr2\\research");
...
void OnDeinit(const int reason)
  {
   ...
   PrintFormat("%s PCR2_UNAVAILABLE_SUMMARY aggregate=%I64d rows=%I64d "
               ...
               arc_unavailability_nonfinite,
               arc_unavailability_unclassified);
   PrintPCR2RequiredContracts("END");
   ReleaseRuntimeOwnership();
  }
