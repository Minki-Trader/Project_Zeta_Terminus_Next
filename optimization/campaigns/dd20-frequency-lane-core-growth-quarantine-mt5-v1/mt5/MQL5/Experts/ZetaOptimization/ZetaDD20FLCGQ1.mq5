...
#property description "Project Zeta Terminus Next isolated DD20 core-growth quarantine MT5 candidate"
#property description "Execution version: zt-opt-live-v7-dd20-frequency-lane-core-growth-quarantine-mt5-v1"

#include <Trade\Trade.mqh>

#include <ZetaOptDD20FLCGQ1\Domain\ZetaDomain.mqh>
#include <ZetaOptDD20FLCGQ1\Time\ZetaSessionClock.mqh>
#include <ZetaOptDD20FLCGQ1\Strategies\ZetaStrategyShared.mqh>
#include <ZetaOptDD20FLCGQ1\Strategies\ZetaRC16.mqh>
#include <ZetaOptDD20FLCGQ1\Strategies\ZetaRC4.mqh>
#include <ZetaOptDD20FLCGQ1\Strategies\ZetaCross.mqh>
#include <ZetaOptDD20FLCGQ1\Strategies\ZetaPressure.mqh>
#include <ZetaOptDD20FLCGQ1\Strategies\ZetaReturn.mqh>
#include <ZetaOptDD20FLCGQ1\Strategies\ZetaPassive.mqh>
#include <ZetaOptDD20FLCGQ1\Portfolio\ZetaPortfolioRisk.mqh>
#include <ZetaOptDD20FLCGQ1\Execution\ZetaOwnership.mqh>
#include <ZetaOptDD20FLCGQ1\Execution\ZetaOrders.mqh>
#include <ZetaOptDD20FLCGQ1\Execution\ZetaProtectionAndReconciliation.mqh>
#include <ZetaOptDD20FLCGQ1\Persistence\ZetaStateAndEvents.mqh>
#include <ZetaOptDD20FLCGQ1\Observation\ZetaResearchObservation.mqh>
#include <ZetaOptDD20FLCGQ1\Strategies\ZetaUS500H4StagedOverlay.mqh>
...
int OnInit()
  {
   ...
   FolderCreate("ZetaOptimization\\dd20-flcgq1");
   FolderCreate("ZetaOptimization\\dd20-flcgq1\\state");
   FolderCreate("ZetaOptimization\\dd20-flcgq1\\research");
...
double OnTester()
  {
   ...
   long core_closed_lifecycles = 0;
   for(int component = 0; component < COMPONENT_COUNT; ++component)
      core_closed_lifecycles += component_states[component].closed_trades;
   ...
   const bool correction_required =
      (portfolio_state.safety_stopped || persistence_failed ||
       ...
       h4_overlay_faults != 0 || h4_overlay_core_ledger_mutations != 0 ||
       ...
       h4_open_positions != 0 || core_closed_lifecycles <= 0);
   if(correction_required)
     {
      PrintFormat("OPTIMIZATION_RESULT status=CORRECTION_REQUIRED "
                  ...
                  "core_closed=%I64d safety=%d persistence=%d broker=%d "
                  "foreign=%d protection_calc=%I64d "
                  "protection_mismatch=%I64d research_dropped=%I64d "
                  "h4_entries=%I64d h4_exits=%I64d h4_open=%d "
                  "h4_pending=%I64u h4_faults=%I64d "
                  "h4_core_mutations=%I64d h4_entry_failures=%I64d "
                  "h4_exit_failures=%I64d h4_partial_or_placed=%I64d "
                  "final_server=%s",
                  ...
                  h4_overlay_faults,
                  h4_overlay_core_ledger_mutations,
   ...
   const double core_actual_net =
      portfolio_state.project_realized_net;
   const double core_stressed_net =
      portfolio_state.stressed_balance - InpReferenceCapitalUSD;
   const double total_actual_net =
      core_actual_net + h4_overlay_actual_net;
   const double total_stressed_net =
      core_stressed_net + h4_overlay_stressed_net;
   const double core_actual_dd =
      portfolio_state.account_maximum_drawdown;
   const double core_stressed_dd =
      portfolio_state.stressed_maximum_closed_drawdown;
   const double robust_net =
      MathMin(total_actual_net, total_stressed_net);
   const double robust_dd =
      MathMax(0.01, MathMax(core_actual_dd, core_stressed_dd));
   ...
   PrintFormat("OPTIMIZATION_RESULT status=ECONOMIC position_risk=%.6f "
               ...
               "core_actual_net=%.6f core_stressed_net=%.6f "
               "h4_actual_net=%.6f h4_stressed_net=%.6f "
               "total_actual_net=%.6f total_stressed_net=%.6f "
               "core_actual_dd=%.6f core_stressed_dd=%.6f "
               "robust_net=%.6f robust_recovery=%.9f "
               "core_closed=%I64d risk_skips=%I64d stop_exits=%I64d "
               "h4_entries=%I64d h4_exits=%I64d h4_margin_skips=%I64d "
               "h4_stage_blocks=%I64d h4_state_unavailable=%I64d "
               "h4_late_windows=%I64d h4_core_mutations=%I64d "
               "final_server=%s",
               InpMaximumPositionRiskFraction,
               InpMaximumAggregateRiskFraction,
               InpRC16RiskMultiplier,
...
void OnDeinit(const int reason)
  {
   ...
   PrintFormat("%s H4_OVERLAY final candidate=%s magic=%I64u "
               ...
               "stressed=%.6f core_actual=%.6f core_stressed=%.6f "
               "total_actual=%.6f total_stressed=%.6f "
               "margin_skips=%I64d stage_blocks=%I64d "
               "state_unavailable=%I64d late_windows=%I64d "
               "entry_failures=%I64d exit_failures=%I64d "
               "partial_or_placed=%I64d faults=%I64d "
               "core_mutations=%I64d pending=%I64u",
               EXECUTION_VERSION,
               ...
               portfolio_state.project_realized_net,
               portfolio_state.stressed_balance - InpReferenceCapitalUSD,
               portfolio_state.project_realized_net + h4_overlay_actual_net,
               portfolio_state.stressed_balance - InpReferenceCapitalUSD +
                  h4_overlay_stressed_net,
               h4_overlay_margin_skips,
               h4_overlay_stage_gate_blocks,
