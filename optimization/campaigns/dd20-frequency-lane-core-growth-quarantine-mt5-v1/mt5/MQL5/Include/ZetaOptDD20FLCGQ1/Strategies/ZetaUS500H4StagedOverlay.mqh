#ifndef ZETA_OPT_DD20_FLCGQ1_US500_H4_STAGED_OVERLAY_MQH
#define ZETA_OPT_DD20_FLCGQ1_US500_H4_STAGED_OVERLAY_MQH

// Exact lineage-pinned ZT-H4-US500-V2-VOLATILITY-EXP-b4d28831f9
// translated into the quarantined shared-account tester executable. The fixed
// overlay owns one US500 H4 position and remains outside all six-component
// planned-risk, state, slot and growth-ledger accounting. Actual account
// balance, equity, margin, execution and safety remain shared.
...
void ResetH4OverlayState()
  {
   ...
   h4_overlay_faults = 0;
   h4_overlay_core_ledger_mutations = 0;
...
bool ApplyH4OverlayExitDeal(const ulong deal,
                            ...
                            const bool sampled_tick_known)
  {
   ...
   const double core_actual_before =
      portfolio_state.project_realized_net;
   const double core_stressed_before =
      portfolio_state.stressed_balance;
   h4_overlay_actual_net += deal_net;
   h4_overlay_stressed_net += stressed_net;
   const double core_actual_after =
      portfolio_state.project_realized_net;
   const double core_stressed_after =
      portfolio_state.stressed_balance;
   if(MathAbs(core_actual_after - core_actual_before) > 1.0e-9 ||
      MathAbs(core_stressed_after - core_stressed_before) > 1.0e-9)
     {
      ++h4_overlay_core_ledger_mutations;
      MarkH4OverlayFault("H4 close mutated quarantined core growth ledger");
      return(false);
     }
   ...
   PrintFormat("%s H4_OVERLAY close candidate=%s deal=%I64u "
               ...
               "core_actual_before=%.6f core_actual_after=%.6f "
               "core_stressed_before=%.6f core_stressed_after=%.6f",
               EXECUTION_VERSION,
               ...
               core_actual_before,
               core_actual_after,
               core_stressed_before,
               core_stressed_after);
   RecordEvent(-1,
               "H4_OVERLAY_CLOSE",
               deal_net,
               stressed_net,
               StringFormat("candidate=%s deal=%I64u core_actual=%.6f "
                            "core_stressed=%.6f",
                            US500_H4_STAGED_OVERLAY_CANDIDATE_ID,
                            deal,
                            core_actual_after,
...
bool OpenH4OverlayPosition(const int direction, const double state)
  {
   ...
   PrintFormat("%s H4_OVERLAY open candidate=%s direction=%d "
               ...
               "deal=%I64u balance=%.2f core_stage=%.2f",
               EXECUTION_VERSION,
               US500_H4_STAGED_OVERLAY_CANDIDATE_ID,
               direction,
               state,
