...
#property description "Project Zeta Terminus Next isolated DD20 market-state router"
#property description "Execution version: zt-opt-live-v7-dd20-market-state-router-mt5-v1"

#include <Trade\Trade.mqh>

#include <ZetaOptimizationDD20MSR1\Domain\ZetaDomain.mqh>
#include <ZetaOptimizationDD20MSR1\Time\ZetaSessionClock.mqh>
#include <ZetaOptimizationDD20MSR1\Strategies\ZetaStrategyShared.mqh>
#include <ZetaOptimizationDD20MSR1\Strategies\ZetaRC16.mqh>
#include <ZetaOptimizationDD20MSR1\Strategies\ZetaRC4.mqh>
#include <ZetaOptimizationDD20MSR1\Strategies\ZetaCross.mqh>
#include <ZetaOptimizationDD20MSR1\Strategies\ZetaPressure.mqh>
#include <ZetaOptimizationDD20MSR1\Strategies\ZetaReturn.mqh>
#include <ZetaOptimizationDD20MSR1\Strategies\ZetaPassive.mqh>
#include <ZetaOptimizationDD20MSR1\Portfolio\ZetaPortfolioRisk.mqh>
#include <ZetaOptimizationDD20MSR1\Execution\ZetaOwnership.mqh>
#include <ZetaOptimizationDD20MSR1\Execution\ZetaOrders.mqh>
#include <ZetaOptimizationDD20MSR1\Execution\ZetaProtectionAndReconciliation.mqh>
#include <ZetaOptimizationDD20MSR1\Persistence\ZetaStateAndEvents.mqh>
#include <ZetaOptimizationDD20MSR1\Observation\ZetaResearchObservation.mqh>
#include <ZetaOptimizationDD20MSR1\Router\ZetaMarketStateRouter.mqh>
...
int OnInit()
  {
   ...
   if(!tester_mode || _Symbol != "US30" || _Period != PERIOD_M30 ||
       ...
       InpRouterActualTradingStart < 0 ||
       MathAbs(InpRouterMinimumPercentile - 0.25) > 1.0e-9 ||
   ...
   FolderCreate("ZetaOptimization\\dd20-market-state-router-mt5-v1");
   FolderCreate("ZetaOptimization\\dd20-market-state-router-mt5-v1\\state");
   FolderCreate(RESEARCH_OBSERVATION_DIRECTORY);
   FolderCreate(ROUTER_DIRECTORY);
   if(tester_mode)
     {
      ResetTesterArtifacts();
      ResetMarketStateRouterArtifacts();
     }
   InitializeMarketStateRouter();
...
void OnTick()
  {
   ...
   // The exact router owns an independent virtual signal/lifecycle clock and
   // therefore runs on every primary tick before the core tester scheduler.
   ProcessMarketStateRouter();
   if(!MarketStateRouterEconomicStarted())
      return;
...
void OnDeinit(const int reason)
  {
   ...
   WriteMarketStateRouterSummary(reason);
