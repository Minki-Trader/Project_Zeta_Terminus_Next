...
def candidate_pairs(
    ...
    step_grid = np.asarray(config["candidate_addition_steps_usd"], dtype=np.float64)
    cap_grid = np.asarray(
        config["candidate_maximum_day_multiplier_caps"], dtype=np.int64
    )
    if (
        len(np.unique(step_grid)) != len(step_grid)
        or np.any(step_grid <= 0.0)
        or len(np.unique(cap_grid)) != len(cap_grid)
        or np.any(cap_grid < 1)
    ):
        raise RuntimeError("candidate step and cap grids must be unique and positive")

    closed_path = input_root / str(
        config["input"]["closed_constant_ladder_result_file"]
    )
    closed = json.loads(closed_path.read_text(encoding="utf-8"))
    if (
        closed.get("status") != "VALID_PROXY_COMPLETE_NO_SELECTION_ELIGIBLE"
        or closed.get("campaign") != "dd20-capital-addition-ladder-proxy-v1"
    ):
        raise RuntimeError("closed constant-ladder result identity mismatch")
    observed_maximum_by_step = {
        int(round(float(record["addition_step_usd"]))): int(
            record["maximum_day_multiplier"]
        )
        for record in closed["top_selection_diagnostic"]
    }
    declared_maximum_by_step = {
        int(step): int(value)
        for step, value in config[
            "closed_uncapped_maximum_day_multiplier_by_step"
        ].items()
    }
    if set(declared_maximum_by_step) != {
        int(round(value)) for value in step_grid
    }:
        raise RuntimeError("declared closed-step coverage differs from the step grid")
    for step, declared_maximum in declared_maximum_by_step.items():
        if observed_maximum_by_step.get(step) != declared_maximum:
            raise RuntimeError("closed uncapped maximum multiplier mismatch")

    pairs = [
        (float(step), int(cap))
        for step in step_grid
        for cap in cap_grid
        if int(cap) < declared_maximum_by_step[int(round(float(step)))]
    ]
    if len(pairs) != int(config["expected_candidates"]):
        raise RuntimeError("capped accelerator pair count does not match the contract")
    if len(set(pairs)) != len(pairs):
        raise RuntimeError("capped accelerator pairs must be unique")
    baseline_step = float(config["baseline_addition_step_usd"])
    baseline_cap = int(config["baseline_maximum_day_multiplier_cap"])
    if any(
        np.isclose(step, baseline_step) and cap == baseline_cap
        for step, cap in pairs
    ):
        raise RuntimeError("closed anchor pair appears in the candidate grid")
    return (
...
def add_anchor_pair(
    ...
    matches = np.flatnonzero(np.isclose(steps, anchor_step) & (caps == anchor_cap))
    if len(matches) != 0:
        raise RuntimeError("closed anchor pair appears in the candidate grid")
    return np.append(steps, anchor_step), np.append(caps, anchor_cap), len(steps)
...
def simulate(
    ...
    if (
        len(addition_steps) != candidate_count
        or len(maximum_day_multiplier_caps) != candidate_count
    ):
        raise RuntimeError("step, cap and weight row counts differ")
    if np.any(maximum_day_multiplier_caps < 1):
        raise RuntimeError("maximum day multiplier caps must be positive")
...
def main() -> None:
    ...
    steps, caps = candidate_pairs(config, input_root)
    candidate_count = len(steps)
    evaluation_steps, evaluation_caps, anchor_index = add_anchor_pair(
        steps,
        caps,
        float(config["baseline_addition_step_usd"]),
        int(config["baseline_maximum_day_multiplier_cap"]),
    )
    ...
    result = {
        "schema": "zeta-dd20-capped-profit-accelerator-proxy-raw-result-v1",
        ...
        "implementation": {
            ...
            "closed_constant_ladder_candidate_construction_gate": "PASS",
        },
        "search": {
            "candidate_pairs": candidate_count,
            "addition_step_grid_values": int(
                len(config["candidate_addition_steps_usd"])
            ),
            ...
            "closed_anchor_pair_in_candidate_grid": False,
            "all_candidate_caps_bind_on_their_closed_uncapped_paths": True,
