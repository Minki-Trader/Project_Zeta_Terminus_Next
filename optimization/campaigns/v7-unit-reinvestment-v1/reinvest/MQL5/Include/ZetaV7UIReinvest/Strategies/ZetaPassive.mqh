#ifndef ZETA_NEXT_MODULE_08_MQH
#define ZETA_NEXT_MODULE_08_MQH
...
bool PlacePassiveLimit(const int direction,
                       ...
                       const datetime expiration)
  {
   ...
   const double passive_volume = (UNIT_REINVESTMENT ? NormalizedVolume("US100") : InpBaseVolume);
   if(passive_volume <= 0.0)
     {
      component_states[US100_PASSIVE_LIMIT].entry_check_result = "VOLUME_BLOCKED";
      return(false);
     }
   component_states[US100_PASSIVE_LIMIT].entry_check_order_price = limit_price;
   component_states[US100_PASSIVE_LIMIT].entry_check_volume = passive_volume;
   if(!PassiveMarginAllows(direction, limit_price, passive_volume))
     {
   ...
   decision_intent.volume = passive_volume;
   unit_pending_volume = passive_volume;
   ...
   const bool requested =
      (direction > 0
       ? trade.BuyLimit(passive_volume,
                        ...
                        expiration,
                        "ZN 6 V7R")
       : trade.SellLimit(passive_volume,
                         limit_price,
                         "US100",
                         stop_loss,
                         0.0,
...
void ProcessPassiveLimit()
  {
   ...
   component_states[US100_PASSIVE_LIMIT].entry_check_order_price = limit_price;
   component_states[US100_PASSIVE_LIMIT].entry_check_volume = (UNIT_REINVESTMENT ? NormalizedVolume("US100") : InpBaseVolume);
   const datetime expiration =
