...
def build_lattice(
    ...
    quantiles = tuple(float(value) for value in lattice_config["coarse_quantile_coordinates"])
    component_count = len(config["components"])
    threshold_lookup = [
        {quantile: nearest_rank_threshold(populations[axis], quantile) for quantile in quantiles}
        for axis in range(component_count)
    ]
    raw_roles: list[dict[str, Any]] = []
    grouped: dict[tuple[float, ...], list[tuple[int, ...]]] = {}
    coordinate_values: dict[tuple[int, ...], tuple[float, ...]] = {}
    for coordinate in itertools.product(range(len(quantiles)), repeat=component_count):
        quantile_vector = tuple(quantiles[value] for value in coordinate)
        thresholds = tuple(
        ...
        coordinate_values[coordinate] = thresholds
        grouped.setdefault(thresholds, []).append(coordinate)
    if len(raw_roles) != int(lattice_config["raw_roles"]):
        raise RuntimeError("coarse strength raw role count mismatch")
    ...
    if len(unique_thresholds) != int(lattice_config["expected_unique_threshold_roles"]):
        raise RuntimeError("coarse strength unique threshold role count mismatch")

    coordinate_to_index = {
        coordinate: threshold_to_index[thresholds]
        for coordinate, thresholds in coordinate_values.items()
    }
    ...
    for coordinate, index in coordinate_to_index.items():
        for axis in range(component_count):
            for step in (-1, 1):
                value = coordinate[axis] + step
                if value < 0 or value >= len(quantiles):
                    continue
                neighbor_coordinate = list(coordinate)
                neighbor_coordinate[axis] = value
                neighbor = coordinate_to_index[tuple(neighbor_coordinate)]
                if neighbor == index:
                    continue
    ...
    primary_coordinates = [values[0] for values in provenance]
    quantile_vectors = np.asarray(
        [[quantiles[value] for value in coordinate] for coordinate in primary_coordinates],
        dtype=np.float64,
    )
    ...
    return {
        ...
        "integer_coordinates": np.asarray(primary_coordinates, dtype=np.int16),
...
def select_separated_then_fill(
    ...
    maximum = int(config["strength_lattice"]["maximum_stage_b_centers"])
    separation = int(
...
def structural_precheck() -> dict[str, Any]:
    ...
    threshold_summary = []
    quantiles = [float(value) for value in config["strength_lattice"]["coarse_quantile_coordinates"]]
...
def main() -> None:
    ...
    result: dict[str, Any] = {
        "schema": "zeta-next-dd20-v8-component-relative-signal-strength-suppression-proxy-result-v1",
        "recorded_at_utc": datetime.now(timezone.utc).isoformat().replace("+00:00", "Z"),
        "campaign": "dd20-v8-component-relative-signal-strength-suppression-proxy-v1",
            ...
            "component_thresholds": [
                {
                    ...
                    "q0": nearest_rank_threshold(populations[axis], 0.0),
                    "q25": nearest_rank_threshold(populations[axis], 0.25),
                    "q50": nearest_rank_threshold(populations[axis], 0.5),
                }
        ...
        "limitations": [
            ...
            "Stage A uses 2024-2025 development only. January-May validation and locked June-July remain unopened.",
        ],
    }
    ...
    for rank, (index, meta) in enumerate(zip(centers, selected_meta), start=1):
        ...
        item.update(
            {
                ...
                "quantile_provenance": [
                    [
                        float(config["strength_lattice"]["coarse_quantile_coordinates"][value])
                        for value in coordinate
                    ]
                    for coordinate in lattice["provenance"][index]
                ],
        ...
        result["stage_b_strength_seeds"].append(
            {
                "development_rank": rank,
                "selection_kind": selection_kind,
                "lattice_index": index,
    ...
    result["status"] = (
        "VALID_PROXY_COMPLETE_STAGE_A_ROBUST_STRENGTH_SEEDS_STAGE_B_REQUIRED_NO_MT5"
        if selection_kind == "ROBUST_ELIGIBLE"
        else "VALID_PROXY_COMPLETE_STAGE_A_NEAR_MISS_STRENGTH_SEEDS_STAGE_B_REQUIRED_NO_MT5"
    )
        ...
        json.dumps(
            {
                ...
                "stage_b_seed_count": len(result["stage_b_strength_seeds"]),
                "mt5_paths": 0,
            }
        )
    )
