...
def verify_and_load_lifecycles(
    config: dict[str, Any],
) -> tuple[dict[str, Path], list[Lifecycle], dict[str, Any]]:
    declared_inputs = config["inputs"]
    input_root = REPOSITORY_ROOT / str(declared_inputs["root"])
    paths: dict[str, Path] = {}
    for role in ("lifecycle", "candidate", "stage_a"):
    ...
    stage_a_declared = declared_inputs["stage_a"]
    stage_a_result = json.loads(paths["stage_a"].read_text(encoding="utf-8"))
    if stage_a_result.get("schema") != str(stage_a_declared["schema"]):
        raise RuntimeError("staged Stage-A schema mismatch")
    if stage_a_result.get("status") != str(stage_a_declared["status"]):
        raise RuntimeError("staged Stage-A status mismatch")
    observed_centers = stage_a_result.get("development_centers", [])
    declared_centers = config["refinement"]["centers"]
    if len(observed_centers) != len(declared_centers):
        raise RuntimeError("staged Stage-A center count mismatch")
    for observed, declared in zip(observed_centers, declared_centers):
        observed_values = list(map(float, observed["weights"]))
        declared_values = list(map(float, declared["weights"]))
        if len(observed_values) != len(declared_values):
            raise RuntimeError("staged Stage-A center width mismatch")
        if not np.allclose(observed_values, declared_values, atol=1.0e-12, rtol=0.0):
            raise RuntimeError("staged Stage-A center weight mismatch")
        if not math.isclose(
            float(observed["aggregate_risk_fraction"]),
            float(declared["aggregate_risk_fraction"]),
            abs_tol=1.0e-12,
            rel_tol=0.0,
        ):
            raise RuntimeError("staged Stage-A center aggregate-risk mismatch")
    ...
    return paths, lifecycles, stage_a_result


def build_lattice(config: dict[str, Any]) -> dict[str, np.ndarray]:
    refinement = config["refinement"]
    decimals = int(refinement["normalize_decimals"])
    components = config["components"]
    aggregate = config["aggregate_risk"]
    component_offsets = list(map(float, refinement["component_offsets"]))
    aggregate_offsets = list(map(float, refinement["aggregate_risk_offsets"]))
    union: set[tuple[float, ...]] = set()
    local_counts: list[int] = []
    for center in refinement["centers"]:
        seed = list(map(float, center["weights"]))
        if len(seed) != len(components):
            raise RuntimeError("refinement center component count mismatch")
        local: set[tuple[float, ...]] = set()
        for offsets in itertools.product(
            *([component_offsets] * len(components)), aggregate_offsets
        ):
            candidate: list[float] = []
            for axis, component in enumerate(components):
                value = min(
                    float(component["upper_bound"]),
                    max(float(component["lower_bound"]), seed[axis] + offsets[axis]),
                )
                candidate.append(round(value, decimals))
            aggregate_value = min(
                float(aggregate["upper_bound"]),
                max(
                    float(aggregate["lower_bound"]),
                    float(center["aggregate_risk_fraction"]) + offsets[-1],
                ),
            )
            candidate.append(round(aggregate_value, decimals))
            local.add(tuple(candidate))
        local_counts.append(len(local))
        union.update(local)
    if local_counts != list(map(int, refinement["expected_center_local_unique_counts"])):
        raise RuntimeError("declared center-local refinement count mismatch")
    if len(union) != int(refinement["expected_union_parameterizations"]):
        raise RuntimeError("declared refinement union size mismatch")
    values = np.asarray(sorted(union), dtype=np.float64)
    coordinates_rows: list[list[int]] = []
    for row in values:
        coordinate: list[int] = []
        for axis, component in enumerate(components):
            lower = float(component["lower_bound"])
            step = float(component["fine_step"])
            value = int(round((float(row[axis]) - lower) / step))
            if not math.isclose(lower + value * step, float(row[axis]), abs_tol=1.0e-9):
                raise RuntimeError("component refinement coordinate mismatch")
            coordinate.append(value)
        aggregate_coordinate = int(
            round(
                (float(row[-1]) - float(aggregate["lower_bound"]))
                / float(aggregate["fine_step"])
            )
        )
        if not math.isclose(
            float(aggregate["lower_bound"])
            + aggregate_coordinate * float(aggregate["fine_step"]),
            float(row[-1]),
            abs_tol=1.0e-9,
        ):
            raise RuntimeError("aggregate refinement coordinate mismatch")
        coordinate.append(aggregate_coordinate)
        coordinates_rows.append(coordinate)
    coordinates = np.asarray(coordinates_rows, dtype=np.int16)
    if len({tuple(map(int, row)) for row in coordinates}) != len(values):
        raise RuntimeError("refinement coordinate collision")
    component_count = len(config["components"])
    weights = values[:, :component_count]
    aggregate_risk = values[:, component_count]
    ...
    seed_coordinates: list[list[int]] = []
    for center in refinement["centers"]:
        seed_row = list(map(float, center["weights"])) + [
            float(center["aggregate_risk_fraction"])
        ]
        seed_coordinates.append(
            [
                int(round((seed_row[axis] - float(components[axis]["lower_bound"])) / float(components[axis]["fine_step"])))
                for axis in range(component_count)
            ]
            + [
                int(round((seed_row[-1] - float(aggregate["lower_bound"])) / float(aggregate["fine_step"])))
            ]
        )
    return {
        ...
        "center_local_unique_counts": np.asarray(local_counts, dtype=np.int32),
        "seed_coordinates": np.asarray(seed_coordinates, dtype=np.int16),
    }
        ...
        plateau.append(
            {
                ...
                "distance_from_stage_a_centers": int(
                    np.abs(seed_coordinates - coordinate[None, :]).sum(axis=1).min()
                ),
            }
        )
    plateau.sort(
        key=lambda item: (
            ...
            item["distance_from_stage_a_centers"],
            tuple(int(value) for value in coordinates[item["index"]]),
        )
    )
    maximum = int(config["gates"]["maximum_stage_b_centers"])
...
def main() -> None:
    ...
    config = load_config()
    input_paths, lifecycles, stage_a_result = verify_and_load_lifecycles(config)
    lattice = build_lattice(config)
    anchor = config["anchor_reproduction"]
    anchor_values = np.asarray(
        [list(map(float, anchor["weights"])) + [float(anchor["aggregate_risk_fraction"])]],
        dtype=np.float64,
    )
    ...
    centers, center_meta = choose_plateau_centers(
        eligible, lattice, development, config, lattice["seed_coordinates"]
    )

    result: dict[str, Any] = {
        "schema": "zeta-next-dd20-v8-clean-executable-basin-refinement-proxy-result-v1",
        "recorded_at_utc": datetime.now(timezone.utc).isoformat().replace("+00:00", "Z"),
        "campaign": "dd20-v8-clean-executable-basin-refinement-proxy-v1",
        ...
        "inputs": {
            ...
            "stage_a": {
                "path": str(config["inputs"]["root"])
                + "/"
                + str(config["inputs"]["stage_a"]["name"]),
                "bytes": input_paths["stage_a"].stat().st_size,
                "sha256": sha256(input_paths["stage_a"]),
                "status": str(stage_a_result["status"]),
            },
            "matched_lifecycles": len(lifecycles),
        },
        "lattice": {
            "declared_parameterizations": int(len(lattice["values"])),
            "center_local_unique_counts": [
                int(value) for value in lattice["center_local_unique_counts"]
            ],
        ...
        "stage_c_seed_centers": [],
        "mt5": {
            "shortlist_count": 0,
            "valid_economic_paths_run": 0,
            "stage_b_authorized_paths": int(config["mt5_budget"]["stage_b_authorized_paths"]),
            "maximum_valid_economic_paths_after_all_proxy_stages": int(
                config["mt5_budget"]["maximum_valid_economic_paths_after_all_proxy_stages"]
            )
        },
        "limitations": [
            "Accepted-source-path refinement replay only; freed-capacity opportunities are absent and receive zero credit.",
            ...
            "Stage B supplies static seeds only; mandatory causal Stage C and temporal/cost Stage D remain unopened.",
        ],
    }
        ...
        item.update(
            {
                ...
                "distance_from_stage_a_centers": int(
                    meta["distance_from_stage_a_centers"]
                ),
    ...
    if not centers:
        result["status"] = (
            "VALID_PROXY_COMPLETE_STAGE_B_NO_DEVELOPMENT_ELIGIBLE_STAGE_C_REQUIRES_STATIC_RESEED_NO_MT5"
            if int(eligible.sum()) == 0
            else "VALID_PROXY_COMPLETE_STAGE_B_NO_ROBUST_PLATEAU_STAGE_C_REQUIRES_STATIC_RESEED_NO_MT5"
        )
    else:
        ...
        if not passing_roles:
            result["status"] = "VALID_PROXY_COMPLETE_STAGE_B_VALIDATION_NONCONFIRMATION_STAGE_C_REQUIRES_STATIC_RESEED_NO_MT5"
        else:
            maximum_stage_c_seeds = int(
                config["stage_succession"]["maximum_stage_c_static_seeds"]
            )
            for local_index in passing_roles[:maximum_stage_c_seeds]:
                global_index = centers[local_index]
                result["stage_c_seed_centers"].append(
                    {
                        "development_rank": local_index + 1,
                        "lattice_index": int(global_index),
                        "parameters": {
                            "weights": [
                                float(value)
                                for value in lattice["values"][global_index, :-1]
                            ],
                            "base_position_risk_fraction": float(
                                config["base_position_risk_fraction"]
                            ),
                            "aggregate_risk_fraction": float(
                                lattice["values"][global_index, -1]
                            ),
                        },
                    }
                )
            ...
            if final_pass:
                result["status"] = "VALID_PROXY_COMPLETE_STAGE_B_SURVIVOR_STAGE_C_REQUIRED_NO_MT5"
            elif not holdout_pass:
                result["status"] = "VALID_PROXY_COMPLETE_STAGE_B_LOCKED_HOLDOUT_NONCONFIRMATION_STAGE_C_REQUIRED_NO_MT5"
            else:
                result["status"] = "VALID_PROXY_COMPLETE_STAGE_B_WHOLE_PATH_GATE_NONCONFIRMATION_STAGE_C_REQUIRED_NO_MT5"
        ...
        json.dumps(
            {
                ...
                "stage_c_seed_count": len(result["stage_c_seed_centers"]),
