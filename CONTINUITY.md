...
- Latest closed Lab follow-up: Unit 118 `fixed-candidate-scale-tail-mechanism-falsification-v1`, `VALID_SCALE_MECHANISM_NOT_CONFIRMED_NO_FACTOR_CAP_SEED`; `27` common pairs are scale-dominated and supply `84.43%` of uplift, but only `12/27` preserve the frozen stop-distance geometry. The August 25 Pressure/Cross LONG losses were separated by `789` seconds with zero holding overlap, only one STOP and no `0.12` cap binding, so no factor-cap seed or MT5 shortlist survived
- Active Lab detour: Program 2 / meso→macro Unit 119 `ads-business-conditions-above-normal-portfolio-state-v1`; the official real-time Philadelphia Fed ADS archive yields `385` strictly ordered vintages and causally maps all `2,233` lifecycles into `1,100 ABOVE_NORMAL / 1,133 AT_OR_BELOW_NORMAL`. Source, zero-state, strict-prior timing, density and one-shot economic gates are frozen before outcomes; no candidate, MT5 or Live authority exists
- Latest scheduler evidence: `optimization/evidence/OPTIMIZATION_POST_SCALE_TAIL_FALSIFICATION_WHOLE_MAP_AUDIT_V1.json`, `NO_READY_FACTOR_CAP_OR_OTHER_CONSULTANT_MECHANISM_FIXED_DEVELOPMENT_CANDIDATE_RETAINED`
- Active dedicated runtime: none; Unit 119 is source/evidence-only, and both closed August proxy/native runtimes remain inactive with useful evidence preserved and no owner process

This is the sole continuous economic-optimization stream and is physically independent from Live-Dev and Lab. The fixed paired-month replacement development candidate uses component weights `2 / 1.5 / 2 / 2.5 / 1.5 / 0` at position/aggregate risk `0.04 / 0.18`. Its native selection actual/stressed net remains `+$5,786.63 / +$5,477.524` at relative equity DD `20.256887565%`; its proxy-selected native June-July execution/cost confirmation remains `+$32.74 / +$30.626` at `18.675302130%`. Both months participated in proxy candidate gating and ranking, so this interval is not an independent statistical holdout. Lab Unit 117 closed the error-feedback quantization clue as economically adverse. Unit 118 directly falsified the consultant's exact factor-cap bridge. Unit 119 is the only current serial detour and tests a genuinely independent real-time business-conditions state from official ADS vintages; it cannot alter the candidate or open MT5 until complete economics and the next whole-map comparison support a distinct treatment question. Exact candidate source also disproves the alleged planned-risk accounting asymmetry; prospective attribution and pending-reservation/ladder discontinuity still require materially new natural evidence. Environment, engineering, configuration, history and report faults remain correction states with no economic verdict or retry cap. Every MT5 campaign is uniquely identified and tester-only, isolated from Master, Live and Lab. No result promotes automatically; explicit user authorization and a separate Lab engineering handoff are required before any possible Live change.
