...
- Latest closed campaign: `optimization/campaigns/dd20-uniform-entry-strength-floor-proxy-v1/`
- Latest verdict: `VALID_PROXY_COMPLETE_NO_SELECTION_ELIGIBLE_UNIFORM_FLOOR_DEGRADES_PROFIT_AND_DRAWDOWN`
...
This is the sole continuous economic-optimization stream and is physically independent from Live-Dev and Lab. The qualified paired-month anchor uses component weights `2 / 1.5 / 2 / 2.5 / 1.5 / 0` at position/aggregate risk `0.04 / 0.18`. The prior byte-equal RLO1 post-forward proxy supported that fixed anchor by `+$0.42 / +$0.375` over three complete Live closes but opened no candidate. The latest blind uniform entry-strength-floor grid then tested `1.00..1.50` on all five active native thresholds. Every stronger floor lost actual and stressed profit and worsened conservative DD; even `1.05` fell to `+$4,842.47 / +$4,555.237` at `25.422685%`, leaving zero selection-eligible factors and no holdout or MT5 shortlist. No component-specific, direction, period, subgroup or nearby-floor rescue continues. Environment, engineering, configuration, history and report faults are correction states with no economic verdict or retry cap. Every MT5 campaign is uniquely identified and tester-only, isolated from Master, Live and Lab. No result promotes automatically; explicit user authorization and a separate Lab engineering handoff are required before any possible Live change.
