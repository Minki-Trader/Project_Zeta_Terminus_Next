...
- Latest closed campaign: `optimization/campaigns/dd20-protective-stop-headroom-proxy-v1/`
- Latest verdict: `VALID_PROXY_COMPLETE_NO_SELECTION_ELIGIBLE_TIGHTER_PROTECTIVE_STOPS_DEGRADE_PROFIT_AND_DRAWDOWN`
- Active dedicated runtime: none; the source-free latest campaign used one hash-pinned lifecycle snapshot and one fixed-path proxy process

This is the sole continuous economic-optimization stream and is physically independent from Live-Dev and Lab. The qualified paired-month anchor uses component weights `2 / 1.5 / 2 / 2.5 / 1.5 / 0` at position/aggregate risk `0.04 / 0.18`. The byte-equal RLO1 post-forward proxy supported that fixed anchor but opened no candidate, and the global entry-strength floor then failed economically. The latest loss-side headroom grid tested the existing `InpStopPlacementHeadroomFraction` from `0.25..0.50` under two timing bounds with all other terms fixed. Every tighter value lost actual/stressed profit and worsened conservative DD; even `0.30` fell to `+$5,065.455772 / +$4,756.349772` at `23.190667%`, leaving zero selection-eligible factors and no holdout or MT5 shortlist. No component, symbol, direction, epoch, reserve, risk, volume or nearby-headroom rescue continues. Environment, engineering, configuration, history and report faults are correction states with no economic verdict or retry cap. Every MT5 campaign is uniquely identified and tester-only, isolated from Master, Live and Lab. No result promotes automatically; explicit user authorization and a separate Lab engineering handoff are required before any possible Live change.
