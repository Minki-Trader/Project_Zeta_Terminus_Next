...
- Latest closed campaign: `optimization/campaigns/dd20-paired-month-live-control-native-quality-recomparison-v1/`
- Latest verdict: `VALID_FIXED_DEVELOPMENT_CANDIDATE_NATIVE_QUALITY_TRADEOFF_SELECTION_MIXED_FORWARD_DOMINANT`
...
- Active dedicated runtime: none; the latest campaign and subsequent whole-map audit were source-free

This is the sole continuous economic-optimization stream and is physically independent from Live-Dev and Lab. The fixed paired-month replacement development candidate uses component weights `2 / 1.5 / 2 / 2.5 / 1.5 / 0` at position/aggregate risk `0.04 / 0.18`. Its selection actual/stressed net is `+$5,786.63 / +$5,477.524` at native relative equity DD `20.256887565%`; untouched forward is `+$32.74 / +$30.626` at `18.675302130%`. Direct Live-derived comparison now records cost resilience, `8.72x / 8.87x` net per close, `2.67x / 2.71x` net per native DD point, higher PF but lower long-path Recovery/Sharpe, and dominant forward native quality. The subsequent full-map audit found no independent ready mechanism in the complete candidate schema, unconsumed recorded lineage or unchanged natural RLO1 metadata. This observation boundary is not an economic failure or Goal completion. Environment, engineering, configuration, history and report faults remain correction states with no economic verdict or retry cap. Every MT5 campaign is uniquely identified and tester-only, isolated from Master, Live and Lab. No result promotes automatically; explicit user authorization and a separate Lab engineering handoff are required before any possible Live change.
