...
The fixed 2024 models and complete 2025 predictive output are preserved. Static and online pooled loss improve 16.2179% and 14.3389%, but Passive unconstrained signal supply is 151.2381% and 200%, so both fail the original 150% proxy ceiling and its original survivor remains null. `CORRECTION_01_NATIVE_ECONOMIC_SELECTION_V1.json` explicitly records a post-predictive design amendment: this extra proxy count cannot establish actual V7 occupancy, admission, risk or DD. The entire unchanged control/static/online bundle now receives complete full-2025 native economic selection, ordered adjacent control/static/control/online. No fitted model, threshold, risk rule or learning rule changes. Only a fully eligible native finalist can open the original continuous 2025-August 2026 confirmation. 2025 is adaptively consumed research data, not an untouched holdout; no economic improvement is claimed yet.

The three own physical Tester-only sources compile with zero errors and warnings. Their distinct namespaces, Magic, state, settings, observations and own Portable are installed; the original three entry denominators are the only strategy changes. Original position management and 150 USD staircase/fixed Passive quantity remain. Own native account and learning paths retain complete operating evidence. The new runtime contains only neutral platform/standard Include and canonical full 2024/2025 warmup/history; no closed custom EA/model/state/output or 2026 candidate values are imported. The first reader required a neutral private common.ini connection-selection correction; the corrected reader completed and exited normally. All 423 copied native files and full 2024/2025 M1 histories remain bound. SELECTION_INPUT_FREEZE_V1 fixes 1,297 exercised source/model/setting/native/input files before the ordered full-2025 matrix. About 32.55 GiB is free, with remaining full stage allowances funded above 30 GiB. No native economic result yet.

Lossless NTFS compression of closed ignored Optimization evidence has recovered about 3.07 GiB. All 1,297 planned files retain identical paths, logical sizes and SHA256; 32 long-path compression errors preserve their files unchanged, and the first failed argument-form attempt remains documented. No deletion or Live action. Full native preparation has a prospective 4.25 GiB runtime/1 GiB raw/52 MiB source allowance and retains the 30 GiB free-space floor. All 17 closed families and the separate unapproved old pyramid exception remain unchanged.
