...
After whole-map comparison, the only current source feasibility is original protective-stop event identity and learning-population availability, Program1, under optimization/evidence/V7_ML_POST_LOG_ALLOCATION_STOP_EVENT_COMPARE_V1.json. Inspect original2024/2025 lifecycle metadata only for four original market engines; RC4 already has native post-compression shadow behavior and Passive remains its original limit slot. Require64 attributable2024 full-stop events,16 per half/direction and3 engines with8 each before any new model or campaign. No prices/profits, changed event/clock, closed-model reuse, new candidate or native runtime is open. If unready, close without widening; if ready, freeze a complete distinct V7 response and native contract before outcomes.
