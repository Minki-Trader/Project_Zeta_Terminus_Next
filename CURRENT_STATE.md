...
Existing V7 improvement with ONNX and online learning continues until the user pauses it, inside Optimization only. Current status: `V7_WHOLE_MACRO_COMPLETE_EXACT_ENTRY_COMPOUND_NATIVE_EXCEPTION_PENDING`. Family22 `optimization/campaigns/v7-native-input-density-admission-v1/` is closed with complete valid native evidence and no eligible finalist. There is no active successor or native process. The complete Program1-5/7 comparison found no ready distinct successor among the examined proposals. A concrete funded proposal now covers only the initial Family1 unchanged static/online entry-risk and capital-proportional-lot model, which never received native economics. Its explicit closed-family exception remains pending; ordinary V7 Tester authority is already granted. No model was copied and no new family/source/runtime was opened. Do not infer consent from elapsed time or storage work. See optimization/evidence/V7_ML_POST_DENSITY_WHOLE_MAP_COMPARE_V1.json and V7_ENTRY_COMPOUND_NATIVE_COMPLETION_PROPOSAL_V1.json. External Program2 data remains excluded; Program6 remains outside the Goal.
