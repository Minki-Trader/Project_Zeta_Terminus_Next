...
The exact Live-Dev authorization, owner and entries-disabled/Live transition state are recorded only in `CURRENT_STATE.md`. The root Master launcher restores only the exact authorized V7 owner and Korean dashboard without duplicating them, through a current-user Task Scheduler host that keeps MT5, its in-process research recorder and the dashboard outside the Codex process lifetime. Running or compiling Lab material never changes Live.
